...
namespace xsimd
{
    ...
    template <class B>
    XSIMD_INLINE batch_type_t<B> exp(const simd_base<B>& x)
    {
        return detail::exp_kernel<batch_type_t<B>, exp_tag>::compute(x());
    }

    template <class B>
    XSIMD_INLINE batch_type_t<B> exp2(const simd_base<B>& x)
    {
        return detail::exp_kernel<batch_type_t<B>, exp2_tag>::compute(x());
    }

    template <class B>
    XSIMD_INLINE batch_type_t<B> exp10(const simd_base<B>& x)
    {
    ...
    namespace detail
    {
        ...
        template <class B>
        XSIMD_INLINE B expm1_real_impl(const B& x)
        {
        ...
        struct expm1_kernel<B, float>
        {
             ...
             * ====================================================
             */
            static XSIMD_INLINE B compute_impl(const B& a)
            {
    ...
    template <class B>
    XSIMD_INLINE batch_type_t<B> expm1(const simd_base<B>& x)
    {
