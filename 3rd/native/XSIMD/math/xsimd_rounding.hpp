...
namespace xsimd
{
    ...
    namespace impl
    {
        ...
        template <>
        XSIMD_INLINE batch<float, 4> ceil(const batch<float, 4>& x)
        {
            return _mm_ceil_ps(x);
        }

        template <>
        XSIMD_INLINE batch<double, 2> ceil(const batch<double, 2>& x)
        {
            return _mm_ceil_pd(x);
        }

        template <>
        XSIMD_INLINE batch<float, 4> floor(const batch<float, 4>& x)
        {
            return _mm_floor_ps(x);
        }

        template <>
        XSIMD_INLINE batch<double, 2> floor(const batch<double, 2>& x)
        {
            return _mm_floor_pd(x);
        }

        template <>
        XSIMD_INLINE batch<float, 4> trunc(const batch<float, 4>& x)
        {
            return _mm_round_ps(x, _MM_FROUND_TO_ZERO);
        }

        template <>
        XSIMD_INLINE batch<double, 2> trunc(const batch<double, 2>& x)
        {
            return _mm_round_pd(x, _MM_FROUND_TO_ZERO);
        }

        template <>
        XSIMD_INLINE batch<float, 4> nearbyint(const batch<float, 4>& x)
        {
            return _mm_round_ps(x, _MM_FROUND_TO_NEAREST_INT);
        }

        template <>
        XSIMD_INLINE batch<double, 2> nearbyint(const batch<double, 2>& x)
        {
        ...
        template <class T, std::size_t N>
        XSIMD_INLINE batch<T, N> ceil_impl(const batch<T, N>& x)
        {
        ...
        template <class T, std::size_t N>
        XSIMD_INLINE batch<T, N> floor_impl(const batch<T, N>& x)
        {
            using btype = batch<T, N>;
            btype tx = trunc(x);
            return select(tx > x, tx - btype(1), tx);
        ...
        template <class T, std::size_t N>
        XSIMD_INLINE batch<T, N> nearbyint_impl(const batch<T, N>& x)
        {
            using btype = batch<T, N>;
            btype s = bitofsign(x);
            btype v = x ^ s;
        ...
        template <>
        XSIMD_INLINE batch<float, 8> ceil(const batch<float, 8>& x)
        {
            return _mm256_round_ps(x, _MM_FROUND_CEIL);
        }

        template <>
        XSIMD_INLINE batch<double, 4> ceil(const batch<double, 4>& x)
        {
            return _mm256_round_pd(x, _MM_FROUND_CEIL);
        }

        template <>
        XSIMD_INLINE batch<float, 8> floor(const batch<float, 8>& x)
        {
            return _mm256_round_ps(x, _MM_FROUND_FLOOR);
        }

        template <>
        XSIMD_INLINE batch<double, 4> floor(const batch<double, 4>& x)
        {
            return _mm256_round_pd(x, _MM_FROUND_FLOOR);
        }

        template <>
        XSIMD_INLINE batch<float, 8> trunc(const batch<float, 8>& x)
        {
            return _mm256_round_ps(x, _MM_FROUND_TO_ZERO);
        }

        template <>
        XSIMD_INLINE batch<double, 4> trunc(const batch<double, 4>& x)
        {
            return _mm256_round_pd(x, _MM_FROUND_TO_ZERO);
        }

        template <>
        XSIMD_INLINE batch<float, 8> nearbyint(const batch<float, 8>& x)
        {
            return _mm256_round_ps(x, _MM_FROUND_TO_NEAREST_INT);
        }

        template <>
        XSIMD_INLINE batch<double, 4> nearbyint(const batch<double, 4>& x)
        {
        ...
        template <>
        XSIMD_INLINE batch<float, 16> ceil(const batch<float, 16>& x)
        {
        ...
        template <>
        XSIMD_INLINE batch<double, 8> ceil(const batch<double, 8>& x)
        {
        ...
        template <>
        XSIMD_INLINE batch<float, 16> floor(const batch<float, 16>& x)
        {
        ...
        template <>
        XSIMD_INLINE batch<double, 8> floor(const batch<double, 8>& x)
        {
        ...
        template <>
        XSIMD_INLINE batch<float, 16> trunc(const batch<float, 16>& x)
        {
        ...
        template <>
        XSIMD_INLINE batch<double, 8> trunc(const batch<double, 8>& x)
        {
        ...
        template <>
        XSIMD_INLINE batch<float, 16> nearbyint(const batch<float, 16>& x)
        {
        ...
        template <>
        XSIMD_INLINE batch<double, 8> nearbyint(const batch<double, 8>& x)
        {
        ...
        template <class T, std::size_t N>
        XSIMD_INLINE batch<T, N> ceil(const batch<T, N>& x)
        {
            XSIMD_FALLBACK_BATCH_UNARY_FUNC(std::ceil, x)
        }

        template <class T, std::size_t N>
        XSIMD_INLINE batch<T, N> floor(const batch<T, N>& x)
        {
            XSIMD_FALLBACK_BATCH_UNARY_FUNC(std::floor, x)
        }

        template <class T, std::size_t N>
        XSIMD_INLINE batch<T, N> trunc(const batch<T, N>& x)
        {
            XSIMD_FALLBACK_BATCH_UNARY_FUNC(std::trunc, x)
        }

        template <class T, std::size_t N>
        XSIMD_INLINE batch<T, N> nearbyint(const batch<T, N>& x)
        {
        ...
        template <class T, std::size_t N>
        XSIMD_INLINE batch<T, N> round(const batch<T, N>& x)
        {
            using btype = batch<T, N>;
            btype v = abs(x);
            btype c = ceil(v);
        ...
        template <class T, std::size_t N>
        XSIMD_INLINE batch<T, N> rint(const batch<T, N>& x)
        {
            return nearbyint(x);
        }
    }
    ...
    template <class B>
    XSIMD_INLINE batch_type_t<B> ceil(const simd_base<B>& x)
    {
        return impl::ceil(x());
    }

    template <class B>
    XSIMD_INLINE batch_type_t<B> floor(const simd_base<B>& x)
    {
        return impl::floor(x());
    }

    template <class B>
    XSIMD_INLINE batch_type_t<B> trunc(const simd_base<B>& x)
    {
        return impl::trunc(x());
    }

    template <class B>
    XSIMD_INLINE batch_type_t<B> round(const simd_base<B>& x)
    {
        return impl::round(x());
    }
    ...
    // are assume that the rounding mode is FE_TONEAREST
    template <class B>
    XSIMD_INLINE batch_type_t<B> nearbyint(const simd_base<B>& x)
    {
        return impl::nearbyint(x());
    }

    template <class B>
    XSIMD_INLINE batch_type_t<B> rint(const simd_base<B>& x)
    {
        return impl::rint(x());
    }
