...
namespace xsimd
{
    ...
    namespace detail
    {
        ...
        struct stirling_kernel<B, float>
        {
            ...
            static XSIMD_INLINE B split_limit()
            {
                return B(detail::caster32_t(uint32_t(0x41d628f6)).f);
            }

            static XSIMD_INLINE B large_limit()
            {
        ...
        template <class B>
        XSIMD_INLINE B stirling(const B& a)
        {
    ...
    namespace detail
    {
        ...
        template <class B>
        XSIMD_INLINE B tgamma_impl(const B& a)
        {
            auto nan_result = (a < B(0.) && is_flint(a));
#ifndef XSIMD_NO_INVALIDS
            nan_result = xsimd::xsimd_isnan(a) || nan_result;
#endif
            B q = abs(a);
            auto test = (a < B(-33.));
            B r = nan<B>();
    ...
    template <class B>
    XSIMD_INLINE batch_type_t<B> tgamma(const simd_base<B>& x)
    {
    ...
    namespace detail
    {
        ...
        {
            static XSIMD_INLINE B gammalnB(const B& x)
            {
                return horner<B,
            ...
            static XSIMD_INLINE B gammalnC(const B& x)
            {
                return horner<B,
            ...
            static XSIMD_INLINE B gammaln2(const B& x)
            {
                return horner<B,
        ...
        {
            static XSIMD_INLINE B gammaln1(const B& x)
            {
                return horner<B,
            ...
            static XSIMD_INLINE B gammalnA(const B& x)
            {
                return horner<B,
    ...
    namespace detail
    {
        ...
        private:
            ...
            static XSIMD_INLINE B negative(const B& q, const B& w)
            {
                B p = floor(q);
                B z = q - p;
                auto test2 = z < B(0.5);
            ...
            static XSIMD_INLINE B other(const B& x)
            {
                auto xlt650 = (x < B(6.5));
                B r0x = x;
                B r0z = x;
        ...
        private:
            ...
            static XSIMD_INLINE B large_negative(const B& q)
            {
                B w = lgamma(q);
                B p = floor(q);
                B z = q - p;
            ...
            static XSIMD_INLINE B other(const B& xx)
            {
                B x = xx;
                auto test = (x < B(13.));
                B r1 = B(0.);
    ...
    template <class B>
    XSIMD_INLINE batch_type_t<B> lgamma(const simd_base<B>& x)
    {
