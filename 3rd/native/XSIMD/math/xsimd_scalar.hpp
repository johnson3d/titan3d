...
namespace xsimd
{
    ...
    template <class T>
    bool xsimd_isnan(std::complex<T> var)
    {
    ...
    XSIMD_INLINE bool is_flint(const T& x)
    {
        return std::isnan(x - x) ? std::numeric_limits<T>::quiet_NaN() : x - /*std::*/trunc(x);
    }

    template <class T, class = typename std::enable_if<std::is_scalar<T>::value>::type>
    XSIMD_INLINE bool is_odd(const T& x)
    {
        return is_even(x - 1.);
    }

    template <class T, class = typename std::enable_if<std::is_scalar<T>::value>::type>
    XSIMD_INLINE bool is_even(const T& x)
    {
        return is_flint(x * T(0.5));
    }

    template <class T, class = typename std::enable_if<std::is_scalar<T>::value>::type>
    XSIMD_INLINE T exp10(const T& x)
    {
    ...
    {
        template <class C>
        XSIMD_INLINE C expm1_complex_scalar_impl(const C& val)
        {
    ...
    template <class T>
    XSIMD_INLINE std::complex<T> expm1(const std::complex<T>& val)
    {
    ...
    XSIMD_INLINE xtl::xcomplex<T, T, i3ec> expm1(const xtl::xcomplex<T, T, i3ec>& val)
    {
        return detail::expm1_complex_scalar_impl(val);
    }
#endif
    ...
    template <class T>
    XSIMD_INLINE auto bitofsign(T const& x) -> decltype(std::signbit(x))
    {
        return std::signbit(x);
    }

    template <class T>
    XSIMD_INLINE auto signbit(T const& v) -> decltype(bitofsign(v))
    {
        return bitofsign(v);
    }

    XSIMD_INLINE double sign(bool const &v)
    {
        return v;
    }

    template <class T, class = typename std::enable_if<std::is_scalar<T>::value>::type>
    XSIMD_INLINE T sign(const T& v)
    {
    ...
    {
        template <class C>
        XSIMD_INLINE C sign_complex_scalar_impl(const C& v)
        {
    ...
    template <class T>
    XSIMD_INLINE std::complex<T> sign(const std::complex<T>& v)
    {
...
#ifdef XSIMD_ENABLE_XTL_COMPLEX
    template <class T, bool i3ec>
    XSIMD_INLINE xtl::xcomplex<T, T, i3ec> sign(const xtl::xcomplex<T, T, i3ec>& v)
    {
        return detail::sign_complex_scalar_impl(v);
    }
#endif
...
#ifdef XSIMD_ENABLE_XTL_COMPLEX
    template <class T, bool i3ec>
    XSIMD_INLINE xtl::xcomplex<T, T, i3ec> log2(const xtl::xcomplex<T, T, i3ec>& val)
    {
        return log(val) / log(T(2));
    }
#endif
    ...
    {
        template <class C>
        XSIMD_INLINE C log1p_complex_scalar_impl(const C& val)
        {
            using T = typename C::value_type;
            C u = C(1.) + val;
    ...
    template <class T>
    XSIMD_INLINE std::complex<T> log1p(const std::complex<T>& val)
    {
...
#ifdef XSIMD_ENABLE_XTL_COMPLEX
    template <class T, bool i3ec>
    XSIMD_INLINE xtl::xcomplex<T, T, i3ec> log1p(const xtl::xcomplex<T, T, i3ec>& val)
    {
        return detail::log1p_complex_scalar_impl(val);
    }
#endif
    ...
    template <class T0, class T1>
    XSIMD_INLINE auto min(T0 const &self, T1 const &other) ->
    ...
    template <class T0, class T1>
    XSIMD_INLINE auto max(T0 const &self, T1 const &other) ->
        typename std::enable_if<std::is_scalar<T0>::value && std::is_scalar<T1>::value,
                                typename std::decay<decltype(self > other ? other : self)>::type>::type
    {
        return self < other ? other : self;
    ...
    template <class T>
    XSIMD_INLINE typename std::enable_if<std::is_scalar<T>::value, T>::type fma(const T& a, const T& b, const T& c)
    {
        return /*std::*/fma(a, b, c);
    }

    XSIMD_INLINE void sincos(float val, float&s, float& c)
    {
    ...
    XSIMD_INLINE void sincos(double val, double&s, double& c)
    {
#if defined(__APPLE__)
        __sincos(val, &s, &c);
#elif defined(_GNU_SOURCE)
    ...
    template <class T>
    XSIMD_INLINE void sincos(const std::complex<T>& val, std::complex<T>& s, std::complex<T>& c)
    {
        s = std::sin(val);
        c = std::cos(val);
    }
...
#ifdef XSIMD_ENABLE_XTL_COMPLEX
    template <class T>
    XSIMD_INLINE void sincos(const xtl::xcomplex<T>& val, xtl::xcomplex<T>& s, xtl::xcomplex<T>& c)
    {
        s = sin(val);
        c = cos(val);
    }
    ...
    template <class T>
    XSIMD_INLINE T frexp(T const& val, int& exp)
    {
        return std::frexp(val, &exp);
    }

    template <class T>
    XSIMD_INLINE decltype(abs(std::declval<T>())) norm(const T& val)
    {
        auto tmp = abs(val);
        return tmp * tmp;
    }
