...
namespace xsimd
{
    ...
    namespace detail
    {
        ...
        {
            static XSIMD_INLINE B cos_eval(const B& z)
            {
                B y = horner<B,
                             0x3d2aaaa5,
                             0xbab60619,
            ...
            static XSIMD_INLINE B sin_eval(const B& z, const B& x)
            {
                B y = horner<B,
                             0xbe2aaaa2,
                             0x3c08839d,
            ...
            static XSIMD_INLINE B base_tancot_eval(const B& z)
            {
                B zz = z * z;
                B y = horner<B,
                             0x3eaaaa6f,
            ...
            template <class BB>
            static XSIMD_INLINE B tan_eval(const B& z, const BB& test)
            {
            ...
            template <class BB>
            static XSIMD_INLINE B cot_eval(const B& z, const BB& test)
            {
                B y = base_tancot_eval(z);
        ...
        {
            static XSIMD_INLINE B reduce(const B& x, B& xr)
            {
                if (all(x <= pio4<B>()))
                {
                    xr = x;
    ...
    template <class B>
    XSIMD_INLINE batch_type_t<B> quadrant(const simd_base<B>& x)
    {
