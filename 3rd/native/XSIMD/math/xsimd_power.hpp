...
namespace xsimd
{
    ...
    XSIMD_INLINE typename std::enable_if<std::is_integral<T1>::value, batch_type_t<B>>::type
    ...
    namespace detail
    {
        ...
        struct pow_kernel
        {
            static XSIMD_INLINE B compute(const B& x, const B& y)
            {
                using b_type = B;
    ...
    template <class B>
    XSIMD_INLINE batch_type_t<B> pow(const simd_base<B>& x, const simd_base<B>& y)
    {
    ...
    template <class B>
    XSIMD_INLINE batch_type_t<B> cbrt(const simd_base<B>& x)
    {
    ...
    template <class B>
    XSIMD_INLINE batch_type_t<B> hypot(const simd_base<B>& x, const simd_base<B>& y)
    {
