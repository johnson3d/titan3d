...
namespace xsimd
{
     ...
     */
    template <class B>
    XSIMD_INLINE batch_type_t<B> fmod(const simd_base<B>& x, const simd_base<B>& y)
    {
     ...
     */
    template <class B>
    XSIMD_INLINE batch_type_t<B> remainder(const simd_base<B>& x, const simd_base<B>& y)
    {
     ...
     */
    template <class B>
    XSIMD_INLINE batch_type_t<B> fdim(const simd_base<B>& x, const simd_base<B>& y)
    {
     ...
     */
    template <class B>
    XSIMD_INLINE batch_type_t<B> clip(const simd_base<B>& x, const simd_base<B>& lo, const simd_base<B>& hi)
    {
    ...
    XSIMD_INLINE T clip(const T& x, const T& lo, const T& hi)
    {
    ...
    namespace detail
    {
        ...
        struct nextafter_kernel
        {
            ...
            static XSIMD_INLINE batch_type next(const batch_type& b) XSIMD_NOEXCEPT
            {
            ...
            static XSIMD_INLINE batch_type prev(const batch_type& b) XSIMD_NOEXCEPT
            {
    ...
    template <class X, class Y>
    XSIMD_INLINE batch_type_t<X> nextafter(const simd_base<X>& from, const simd_base<Y>& to)
    {
     ...
     */
    template <class B>
    XSIMD_INLINE typename simd_batch_traits<B>::batch_bool_type
    isfinite(const simd_base<B>& x)
    {
    ...
    is_flint(const simd_base<B>& x)
    {
        ...
        b_type frac = select(xsimd::xsimd_isnan(x - x), nan<b_type>(), x - trunc(x));
