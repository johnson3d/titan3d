...
namespace xsimd
{
    ...
    /*
      ...
      */
    template <class T, std::size_t N>
    XSIMD_INLINE batch<T, N> ldexp(const batch<T, N>& x, const batch<as_integer_t<T>, N>& e)
    {
     ...
     */
    template <class T, std::size_t N>
    XSIMD_INLINE batch<T, N> frexp(const batch<T, N>& arg, batch<as_integer_t<T>, N>& exp)
    {
