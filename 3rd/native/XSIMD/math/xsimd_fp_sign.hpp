...
namespace xsimd
{
    ...
    template <class B>
    XSIMD_INLINE batch_type_t<B> bitofsign(const simd_base<B>& x)
    {
        return x() & minuszero<batch_type_t<B>>();
    }

    template <class B>
    XSIMD_INLINE batch_type_t<B> copysign(const simd_base<B>& x1, const simd_base<B>& x2)
    {
    ...
    namespace detail
    {
        ...
        struct sign_impl<B, false>
        {
            static XSIMD_INLINE B compute(const B& a)
            {
                ...
                return r;
#else
                return select(xsimd::xsimd_isnan(a), nan<B>(), r);
#endif
            }
        };
    }
    ...
    template <class B>
    XSIMD_INLINE batch_type_t<B> sign(const simd_base<B>& x)
    {
    ...
    namespace detail
    {
        ...
        struct signnz_impl<B, true>
        {
            static XSIMD_INLINE B compute(const B& x)
            {
                ...
                return select(xsimd::xsimd_isnan(x), nan<B>(), B(1.) | (signmask<B>() & x));
#else
    ...
    template <class B>
    XSIMD_INLINE batch_type_t<B> signnz(const simd_base<B>& x)
    {
