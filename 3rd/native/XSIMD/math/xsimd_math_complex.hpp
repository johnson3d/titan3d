...
namespace xsimd
{
    ...
    namespace detail
    {
        ...
        XSIMD_INLINE batch<T, N> sign_complex_impl(const batch<T, N>& z)
        {
        ...
        struct sign_impl<batch<std::complex<T>, N>, false>
        {
            ...
            static XSIMD_INLINE batch_type compute(const batch_type& z)
            {
        ...
        template <class T, std::size_t N>
        XSIMD_INLINE batch<T, N> exp_complex_impl(const batch<T, N>& z)
        {
            using b_type = batch<T, N>;
            using r_type = typename b_type::real_batch;
            r_type icos, isin;
        ...
        template <class T, std::size_t N>
        XSIMD_INLINE batch<T, N> expm1_complex_impl(const batch<T, N>& z)
        {
            using b_type = batch<T, N>;
            using r_type = typename b_type::real_batch;
        ...
        template <class T, std::size_t N>
        XSIMD_INLINE batch<T, N> log_complex_impl(const batch<T, N>& z)
        {
        ...
        template <class T, std::size_t N>
        XSIMD_INLINE batch<T, N> logN_complex_impl(const batch<T, N>& z, typename batch<T, N>::real_value_type base)
        {
            using b_type = batch<T, N>;
        ...
        template <class T, std::size_t N>
        XSIMD_INLINE batch<T, N> log1p_complex_impl(const batch<T, N>& z)
        {
            using b_type = batch<T, N>;
            using r_type = typename b_type::real_batch;
            b_type u = b_type(1.) + z;
        ...
        template <class T, std::size_t N>
        XSIMD_INLINE batch<T, N> pow_complex_impl(const batch<T, N>& a, const batch<T, N>& z)
        {
        ...
        struct pow_kernel<batch<std::complex<T>, N>>
        {
            ...
            static XSIMD_INLINE batch_type compute(const batch_type& a, const batch_type& z)
            {
        ...
        template <class T, std::size_t N>
        XSIMD_INLINE void sincos_complex_impl(const batch<T, N>& z, batch<T, N>& si, batch<T, N>& co)
        {
            using b_type = batch<T, N>;
            using r_type = typename b_type::real_batch;
        ...
        template <class T, std::size_t N>
        XSIMD_INLINE batch<T, N> sin_complex_impl(const batch<T, N>& z)
        {
            return batch<T, N>(sin(z.real()) * cosh(z.imag()), cos(z.real()) * sinh(z.imag()));
        }

        template <class T, std::size_t N>
        XSIMD_INLINE batch<T, N> cos_complex_impl(const batch<T, N>& z)
        {
            return batch<T, N>(cos(z.real()) * cosh(z.imag()), -sin(z.real()) * sinh(z.imag()));
        }

        template <class T, std::size_t N>
        XSIMD_INLINE batch<T, N> tan_complex_impl(const batch<T, N>& z)
        {
            using b_type = batch<T, N>;
            using r_type = typename b_type::real_batch;
        ...
        struct trigo_kernel<batch<std::complex<T>, N>>
        {
            ...
            static XSIMD_INLINE batch_type sin(const batch_type& z)
            {
                return sin_complex_impl(z);
            }

            static XSIMD_INLINE batch_type cos(const batch_type& z)
            {
                return cos_complex_impl(z);
            }

            static XSIMD_INLINE batch_type tan(const batch_type& z)
            {
                return tan_complex_impl(z);
            }

            static XSIMD_INLINE void sincos(const batch_type& z, batch_type& si, batch_type& co)
            {
        ...
        struct invtrigo_kernel<batch<std::complex<T>, N>>
        {
            ...
            static XSIMD_INLINE batch_type asin(const batch_type& z)
            {
                return asin_complex_impl(z);
            }

            static XSIMD_INLINE batch_type acos(const batch_type& z)
            {
                return acos_complex_impl(z);
            }

            static XSIMD_INLINE batch_type atan(const batch_type& z)
            {
        ...
        template <class T, std::size_t N>
        XSIMD_INLINE batch<T, N> sinh_complex_impl(const batch<T, N>& z)
        {
            auto x = z.real();
            auto y = z.imag();
        ...
        template <class T, std::size_t N>
        XSIMD_INLINE batch<T, N> cosh_complex_impl(const batch<T, N>& z)
        {
            auto x = z.real();
            auto y = z.imag();
        ...
        template <class T, std::size_t N>
        XSIMD_INLINE batch<T, N> tanh_complex_impl(const batch<T, N>& z)
        {
        ...
        template <class T, std::size_t N>
        XSIMD_INLINE batch<T, N> asinh_complex_impl(const batch<T, N>& z)
        {
            using b_type = batch<T, N>;
        ...
        template <class T, std::size_t N>
        XSIMD_INLINE batch<T, N> acosh_complex_impl(const batch<T, N>& z)
        {
            using b_type = batch<T, N>;
            b_type w = acos(z);
            w = b_type(-w.imag(), w.real());
        ...
        template <class T, std::size_t N>
        XSIMD_INLINE batch<T, N> atanh_complex_impl(const batch<T, N>& z)
        {
            using b_type = batch<T, N>;
    ...
    {
        template <class T>
        T csqrt_scale_factor() XSIMD_NOEXCEPT;

        template <class T>
        T csqrt_scale() XSIMD_NOEXCEPT;

        template <>
        XSIMD_INLINE float csqrt_scale_factor<float>() XSIMD_NOEXCEPT
        {
            return 6.7108864e7f;
        }

        template <>
        XSIMD_INLINE float csqrt_scale<float>() XSIMD_NOEXCEPT
        {
            return 1.220703125e-4f;
        }

        template <>
        XSIMD_INLINE double csqrt_scale_factor<double>() XSIMD_NOEXCEPT
        {
            return 1.8014398509481984e16;
        }

        template <>
        XSIMD_INLINE double csqrt_scale<double>() XSIMD_NOEXCEPT
        {
    ...
    namespace detail
    {
        ...
        struct complex_batch_kernel
        {
            ...
            static batch_bool_type xsimd_isnan(const batch_type& z)
            {
                return batch_bool_type(xsimd::xsimd_isnan(z.real()) || xsimd::xsimd_isnan(z.imag()));
            }
        };
    ...
    template <class X>
    XSIMD_INLINE real_batch_type_t<X> real(const simd_base<X>& z)
    {
    ...
    template <class X>
    XSIMD_INLINE real_batch_type_t<X> imag(const simd_base<X>& z)
    {
        using batch_type = batch_type_t<X>;
        constexpr bool is_cplx = detail::is_complex<typename batch_type::value_type>::value;
    ...
    template <class X>
    XSIMD_INLINE real_batch_type_t<X> arg(const simd_base<X>& z)
    {
        return atan2(imag(z), real(z));
    }

    template <class X>
    XSIMD_INLINE batch_type_t<X> conj(const simd_base<X>& z)
    {
        return X(z().real(), -z().imag());
    }

    template <class X>
    XSIMD_INLINE real_batch_type_t<X> norm(const simd_base<X>& rhs)
    {
        return real(rhs) * real(rhs) + imag(rhs) * imag(rhs);
    }

    template <class X>
    XSIMD_INLINE batch_type_t<X> proj(const simd_base<X>& rhs)
    {
        using batch_type = batch_type_t<X>;
