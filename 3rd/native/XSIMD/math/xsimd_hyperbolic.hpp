...
namespace xsimd
{
    ...
    namespace detail
    {
        ...
        struct average_impl
        {
            static XSIMD_INLINE B compute(const B& x1, const B& x2)
            {
    ...
    template <class B>
    XSIMD_INLINE batch_type_t<B> average(const simd_base<B>& x1, const simd_base<B>& x2)
    {
    ...
    namespace detail
    {
        ...
        {
            static XSIMD_INLINE B compute(const B& x)
            {
                B sqrx = x * x;
                return horner<B,
                              0x3f800000,  // 1.0f
        ...
        struct sinh_kernel
        {
            static XSIMD_INLINE B compute(const B& a)
            {
                using b_type = B;
     ...
     */
    template <class B>
    XSIMD_INLINE batch_type_t<B> sinh(const simd_base<B>& a)
    {
    ...
    template <class B>
    XSIMD_INLINE batch_type_t<B> cosh(const simd_base<B>& a)
    {
    ...
    namespace detail
    {
        ...
        {
            static XSIMD_INLINE B tanh(const B& x)
            {
                B sqrx = x * x;
                return fma(horner<B,
            ...
            static XSIMD_INLINE B cotanh(const B& x)
            {
                return B(1.) / tanh(x);
            }
        };
        ...
        struct tanh_kernel_impl<B, double>
        {
            ...
            static XSIMD_INLINE B p(const B& x)
            {
                return horner<B,
            ...
            static XSIMD_INLINE B q(const B& x)
            {
                return horner1<B,
    ...
    template <class B>
    XSIMD_INLINE batch_type_t<B> tanh(const simd_base<B>& a)
    {
    ...
    namespace detail
    {
        ...
        struct asinh_kernel<B, float>
        {
            static XSIMD_INLINE B compute(const B& a)
            {
                ...
                return select(xsimd::xsimd_isnan(a), nan<B>(), select(lthalf, z, log(tmp) + log_2<B>()) ^ bts);
#else
    ...
    template <class B>
    XSIMD_INLINE batch_type_t<B> asinh(const simd_base<B>& x)
    {
    ...
    template <class B>
    XSIMD_INLINE batch_type_t<B> acosh(const simd_base<B>& a)
    {
    ...
    template <class B>
    XSIMD_INLINE batch_type_t<B> atanh(const simd_base<B>& a)
    {
