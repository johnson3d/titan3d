...
namespace xsimd
{
    ...
    namespace detail
    {
        ...
        struct trigo_kernel
        {
            ...
            static XSIMD_INLINE B sin(const B& a, Tag = Tag())
            {
                const B x = abs(a);
                B xr = nan<B>();
            ...
            static XSIMD_INLINE B cos(const B& a)
            {
                const B x = abs(a);
                B xr = nan<B>();
            ...
            static XSIMD_INLINE B tan(const B& a)
            {
                const B x = abs(a);
                B xr = nan<B>();
                const B n = trigo_reducer<B>::reduce(x, xr);
            ...
            static XSIMD_INLINE void sincos(const B& a, B& si, B& co)
            {
                const B x = abs(a);
                B xr = nan<B>();
                const B n = trigo_reducer<B>::reduce(x, xr);
    ...
    template <class B>
    XSIMD_INLINE batch_type_t<B> sin(const simd_base<B>& x)
    {
        return detail::trigo_kernel<batch_type_t<B>>::sin(x());
    }

    template <class B>
    XSIMD_INLINE batch_type_t<B> cos(const simd_base<B>& x)
    {
        return detail::trigo_kernel<batch_type_t<B>>::cos(x());
    }

    template <class B>
    XSIMD_INLINE void sincos(const simd_base<B>& x, batch_type_t<B>& si, batch_type_t<B>& co)
    {
        detail::trigo_kernel<batch_type_t<B>>::sincos(x(), si, co);
    }

    template <class B>
    XSIMD_INLINE batch_type_t<B> tan(const simd_base<B>& x)
    {
        return detail::trigo_kernel<batch_type_t<B>>::tan(x());
    }

    template <class B>
    XSIMD_INLINE batch_type_t<B> asin(const simd_base<B>& x)
    {
        return detail::invtrigo_kernel<batch_type_t<B>>::asin(x());
    }

    template <class B>
    XSIMD_INLINE batch_type_t<B> acos(const simd_base<B>& x)
    {
        return detail::invtrigo_kernel<batch_type_t<B>>::acos(x());
    }

    template <class B>
    XSIMD_INLINE batch_type_t<B> atan(const simd_base<B>& x)
    {
        return detail::invtrigo_kernel<batch_type_t<B>>::atan(x());
    }

    template <class B>
    XSIMD_INLINE batch_type_t<B> atan2(const simd_base<B>& y, const simd_base<B>& x)
    {
