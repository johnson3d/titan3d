...
namespace xsimd
{
    ...
    namespace detail
    {
        ...
        {
            XSIMD_INLINE static V set_simd(const T& value)
            {
            ...
            XSIMD_INLINE static V load_aligned(const T* src)
            {
                V res;
                return res.load_aligned(src);
            }

            XSIMD_INLINE static void load_aligned(const T* src, V& dst)
            {
                dst.load_aligned(src);
            }

            XSIMD_INLINE static V load_unaligned(const T* src)
            {
                V res;
                return res.load_unaligned(src);
            }

            XSIMD_INLINE static void load_unaligned(const T* src, V& dst)
            {
                dst.load_unaligned(src);
            }

            XSIMD_INLINE static void store_aligned(T* dst, const V& src)
            {
                src.store_aligned(dst);
            }

            XSIMD_INLINE static void store_unaligned(T* dst, const V& src)
            {
        ...
        {
            XSIMD_INLINE static V load_aligned(const T* real_src, const T* imag_src)
            {
                V res;
                return res.load_aligned(real_src, imag_src);
            }

            XSIMD_INLINE static void load_aligned(const T* real_src, const T* imag_src, V& dst)
            {
                dst.load_aligned(real_src, imag_src);
            }

            XSIMD_INLINE static V load_unaligned(const T* real_src, const T* imag_src)
            {
                V res;
                return res.load_unaligned(real_src, imag_src);
            }

            XSIMD_INLINE static void load_unaligned(const T* real_src, const T* imag_src, V& dst)
            {
                dst.load_unaligned(real_src, imag_src);
            }

            XSIMD_INLINE static void store_aligned(T* real_dst, T* imag_dst, const V& src)
            {
                src.store_aligned(real_dst, imag_dst);
            }

            XSIMD_INLINE static void store_unaligned(T* real_dst, T* imag_dst, const V& src)
            {
        ...
        {
            XSIMD_INLINE static T set_simd(const T& value)
            {
                return value;
            }

            XSIMD_INLINE static T load_aligned(const T* src)
            {
                return *src;
            }

            XSIMD_INLINE static void load_aligned(const T* src, T& dst)
            {
                dst = *src;
            }

            XSIMD_INLINE static T load_unaligned(const T* src)
            {
                return *src;
            }

            XSIMD_INLINE static void load_unaligned(const T* src, T& dst)
            {
                dst = *src;
            }

            XSIMD_INLINE static void store_aligned(T* dst, const T& src)
            {
                *dst = src;
            }

            XSIMD_INLINE static void store_unaligned(T* dst, const T& src)
            {
                *dst = src;
            }
        };
    ...
    template <class T1, class T2>
    XSIMD_INLINE simd_return_type<T1, T2> load_simd(const T1* src, aligned_mode)
    {
        return load_aligned<T1, T2>(src);
    }

    template <class T1, class T2>
    XSIMD_INLINE void load_simd(const T1* src, simd_type<T2>& dst, aligned_mode)
    {
        load_aligned<T1, T2>(src, dst);
    }

    template <class T1, class T2>
    XSIMD_INLINE simd_return_type<T1, T2> load_simd(const T1* real_src, const T1* imag_src, aligned_mode)
    {
        return load_aligned<T1, T2>(real_src, imag_src);
    }

    template <class T1, class T2>
    XSIMD_INLINE void load_simd(const T1* real_src, const T1* imag_src, simd_type<T2>& dst, aligned_mode)
    {
        load_aligned<T1, T2>(real_src, imag_src, dst);
    }

    template <class T1, class T2>
    XSIMD_INLINE simd_return_type<T1, T2> load_simd(const T1* src, unaligned_mode)
    {
        return load_unaligned<T1, T2>(src);
    }

    template <class T1, class T2>
    XSIMD_INLINE void load_simd(const T1* src, simd_type<T2>& dst, unaligned_mode)
    {
        load_unaligned<T1, T2>(src, dst);
    }

    template <class T1, class T2>
    XSIMD_INLINE simd_return_type<T1, T2> load_simd(const T1* real_src, const T1* imag_src, unaligned_mode)
    {
        return load_unaligned<T1, T2>(real_src, imag_src);
    }

    template <class T1, class T2>
    XSIMD_INLINE void load_simd(const T1* real_src, const T1* imag_src, simd_type<T2>& dst, unaligned_mode)
    {
        load_unaligned<T1, T2>(real_src, imag_src, dst);
    }

    template <class T1, class T2>
    XSIMD_INLINE void store_simd(T1* dst, const simd_type<T2>& src, aligned_mode)
    {
        store_aligned<T1, T2>(dst, src);
    }

    template <class T1, class T2>
    XSIMD_INLINE void store_simd(T1* dst, const simd_bool_type<T2>& src, aligned_mode)
    {
        store_aligned<T1, T2>(dst, src);
    }

    template <class T1, class T2>
    XSIMD_INLINE void store_simd(T1* dst, const simd_type<T2>& src, unaligned_mode)
    {
        store_unaligned<T1, T2>(dst, src);
    }

    template <class T1, class T2>
    XSIMD_INLINE void store_simd(T1* dst, const simd_bool_type<T2>& src, unaligned_mode)
    {
        store_unaligned<T1, T2>(dst, src);
    }

    template <class T1, class T2>
    XSIMD_INLINE void store_simd(T1* real_dst, T1* imag_dst, const simd_type<T2>& src, aligned_mode)
    {
        store_aligned<T1, T2>(real_dst, imag_dst, src);
    }

    template <class T1, class T2>
    XSIMD_INLINE void store_simd(T1* real_dst, T1* imag_dst, const simd_type<T2>& src, unaligned_mode)
    {
    ...
    template <class T>
    XSIMD_INLINE void prefetch(const T* /*address*/)
    {
    }
    ...
    template <>
    XSIMD_INLINE void prefetch<int32_t>(const int32_t* address)
    {
        _mm_prefetch(reinterpret_cast<const char*>(address), _MM_HINT_T0);
    }

    template <>
    XSIMD_INLINE void prefetch<int64_t>(const int64_t* address)
    {
        _mm_prefetch(reinterpret_cast<const char*>(address), _MM_HINT_T0);
    }

    template <>
    XSIMD_INLINE void prefetch<float>(const float* address)
    {
        _mm_prefetch(reinterpret_cast<const char*>(address), _MM_HINT_T0);
    }

    template <>
    XSIMD_INLINE void prefetch<double>(const double* address)
    {
        _mm_prefetch(reinterpret_cast<const char*>(address), _MM_HINT_T0);
    }
