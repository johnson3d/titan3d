...
#define XSIMD_FALLBACK_BOOL_CAST(T_OUT, X)  \
    ...
    template <typename T, std::size_t N>
    XSIMD_INLINE batch_bool<T, N>::batch_bool()
    {
    }

    template <typename T, std::size_t N>
    XSIMD_INLINE batch_bool<T, N>::batch_bool(bool b)
    ...
    XSIMD_INLINE batch_bool<T, N>::batch_bool(Args... exactly_N_bools)
        : m_value{ exactly_N_bools... }
    {
    }

    template <typename T, std::size_t N>
    XSIMD_INLINE batch_bool<T, N>::batch_bool(const std::array<bool, N>& rhs)
        : m_value(rhs)
    {
    }

    template <typename T, std::size_t N>
    XSIMD_INLINE batch_bool<T, N>& batch_bool<T, N>::operator=(const std::array<bool, N>& rhs)
    {
        m_value = rhs;
        return *this;
    }
    ...
    template <typename T, std::size_t N>
    XSIMD_INLINE batch_bool<T, N>::operator std::array<bool, N>() const
    {
        return m_value;
    }

    template <typename T, std::size_t N>
    XSIMD_INLINE const bool& batch_bool<T, N>::operator[](std::size_t index) const
    {
        return m_value[index];
    }

    template <typename T, std::size_t N>
    XSIMD_INLINE bool& batch_bool<T, N>::operator[](std::size_t index)
    {
        return m_value[index];
    }

    template <typename T, std::size_t N>
    XSIMD_INLINE const std::array<bool, N>& batch_bool<T, N>::get_value() const
    {
        return m_value;
    }
    ...
    template <typename T, std::size_t N>
    template <class... Args>
    XSIMD_INLINE batch_bool<T, N>& batch_bool<T, N>::load_values(Args... args)
    {
    ...
    template <typename T, std::size_t N>
    XSIMD_INLINE batch<T, N>::batch()
    {
    }

    template <typename T, std::size_t N>
    XSIMD_INLINE batch<T, N>::batch(T f)
    ...
    template <typename T, std::size_t N>
    template <typename... Args, typename Enable>
    XSIMD_INLINE batch<T, N>::batch(Args... exactly_N_scalars)
    ...
    template <typename T, std::size_t N>
    XSIMD_INLINE batch<T, N>::batch(const std::array<bool, N>& src)
    {
    ...
    template <typename T, std::size_t N>
    XSIMD_INLINE batch<T, N>::batch(const T* src)
        : batch(src, unaligned_mode())
    {
    }

    template <typename T, std::size_t N>
    XSIMD_INLINE batch<T, N>::batch(const T* src, aligned_mode)
        : batch(src, unaligned_mode())
    {
    }

    template <typename T, std::size_t N>
    XSIMD_INLINE batch<T, N>::batch(const T* src, unaligned_mode)
        : base_type(detail::array_from_pointer<T, N>(src))
    {
    }

    template <typename T, std::size_t N>
    XSIMD_INLINE batch<T, N>::batch(const std::array<T, N>& rhs)
        : base_type(rhs)
    {
    }

    template <typename T, std::size_t N>
    XSIMD_INLINE batch<T, N>::batch(const batch_bool_type& rhs)
    {
    ...
    template <typename T, std::size_t N>
    XSIMD_INLINE batch<T, N>& batch<T, N>::operator=(const std::array<T, N>& rhs)
    {
        this->m_value = rhs;
        return *this;
    }
    ...
    template <typename T, std::size_t N>
    XSIMD_INLINE batch<T, N>& batch<T, N>::operator=(const std::array<bool, N>& rhs)
    {
        using all_bits = detail::all_bits<std::is_integral<T>::value>;
    ...
    template <typename T, std::size_t N>
    XSIMD_INLINE batch<T, N>& batch<T, N>::operator=(const batch_bool_type& rhs)
    {
        std::transform(rhs.get_value().cbegin(), rhs.get_value().cend(), this->m_value.begin(),
                       [](bool b) -> T { return b ? T(1) : T(0); });
        return *this;
    ...
    template <typename T, std::size_t N>
    XSIMD_INLINE batch<T, N>::operator std::array<T, N>() const
    {
        return this->m_value;
    }
...
#define FALLBACK_DEFINE_LOAD_STORE(TYPE)                             \
    template <typename T, std::size_t N>                             \
    XSIMD_INLINE batch<T, N>& batch<T, N>::load_aligned(const TYPE* src)   \
    {                                                                \
        return this->load_unaligned_impl(src);                       \
    }                                                                \
    template <typename T, std::size_t N>                             \
    XSIMD_INLINE batch<T, N>& batch<T, N>::load_unaligned(const TYPE* src) \
    {                                                                \
        return this->load_unaligned_impl(src);                       \
    }                                                                \
    template <typename T, std::size_t N>                             \
    XSIMD_INLINE void batch<T, N>::store_aligned(TYPE* dst) const          \
    {                                                                \
        this->store_unaligned_impl(dst);                             \
    }                                                                \
    template <typename T, std::size_t N>                             \
    XSIMD_INLINE void batch<T, N>::store_unaligned(TYPE* dst) const        \
    {                                                                \
        this->store_unaligned_impl(dst);                             \
    }
...
#undef FALLBACK_DEFINE_LOAD_STORE
    ...
    template <typename T, std::size_t N>
    template <typename U>
    XSIMD_INLINE batch<T, N>& batch<T, N>::load_unaligned_impl(const U* src)
    {
        for(std::size_t i = 0; i < N; ++i)
        {
    ...
    template <typename T, std::size_t N>
    template <typename U>
    XSIMD_INLINE void batch<T, N>::store_unaligned_impl(U* dst) const
    {
        for(std::size_t i = 0; i < N; ++i)
        {
    ...
    template <typename T, std::size_t N>
    XSIMD_INLINE T& batch<T, N>::operator[](std::size_t index)
    {
        return this->m_value[index % base_type::size];
    }

    template <typename T, std::size_t N>
    XSIMD_INLINE const T& batch<T, N>::operator[](std::size_t index) const
    {
        return this->m_value[index % base_type::size];
    }
    ...
    template <typename T, std::size_t N>
    XSIMD_INLINE batch<T, N> operator<<(const batch<T, N>& lhs, int32_t rhs) {
    ...
    namespace detail
    {
        ...
        XSIMD_INLINE std::pair<B, B> load_complex_impl(const U* src)
        {
        ...
        template <std::size_t N, class B, class U>
        XSIMD_INLINE void store_complex_impl(const B& real, const B& imag, U* dst)
        {
            using value_type = typename U::value_type;
    ...
    template <class T, std::size_t N>
    template <class U>
    XSIMD_INLINE auto batch<std::complex<T>, N>::load_aligned(const U* src)
        -> typename std::enable_if<detail::is_complex<U>::value, self_type&>::type
    {
    ...
    template <class T, std::size_t N>
    template <class U>
    XSIMD_INLINE auto batch<std::complex<T>, N>::load_unaligned(const U* src)
        -> typename std::enable_if<detail::is_complex<U>::value, self_type&>::type
    {
    ...
    template <class T, std::size_t N>
    template <class U>
    XSIMD_INLINE void batch<std::complex<T>, N>::store_aligned(U* dst) const
    {
    ...
    template <class T, std::size_t N>
    template <class U>
    XSIMD_INLINE void batch<std::complex<T>, N>::store_unaligned(U* dst) const
    {
        store_aligned(dst);
    }
    ...
    template <class T, std::size_t N, bool i3ec>
    template <class U>
    XSIMD_INLINE auto batch<xtl::xcomplex<T, T, i3ec>, N>::load_aligned(const U* src)
        -> typename std::enable_if<detail::is_complex<U>::value, self_type&>::type
    {
        std::tie(this->m_real, this->m_imag) = detail::load_complex_impl<N, real_batch>(src);
        return *this;
    ...
    template <class T, std::size_t N, bool i3ec>
    template <class U>
    XSIMD_INLINE auto batch<xtl::xcomplex<T, T, i3ec>, N>::load_unaligned(const U* src)
        -> typename std::enable_if<detail::is_complex<U>::value, self_type&>::type
    {
        return load_aligned(src);
    }
    ...
    template <class T, std::size_t N, bool i3ec>
    template <class U>
    XSIMD_INLINE void batch<xtl::xcomplex<T, T, i3ec>, N>::store_aligned(U* dst) const
    {
        detail::store_complex_impl<N>(this->m_real, this->m_imag, dst);
    }
    ...
    template <class T, std::size_t N, bool i3ec>
    template <class U>
    XSIMD_INLINE void batch<xtl::xcomplex<T, T, i3ec>, N>::store_unaligned(U* dst) const
    {
        store_unaligned(dst);
    }
    ...
    template <std::size_t N>
    XSIMD_INLINE batch<int32_t, N> to_int(const batch<float, N>& x)
    {
        XSIMD_FALLBACK_BATCH_STATIC_CAST(int32_t, x)
    }

    template <std::size_t N>
    XSIMD_INLINE batch<int64_t, N> to_int(const batch<double, N>& x)
    {
        XSIMD_FALLBACK_BATCH_STATIC_CAST(int64_t, x)
    }

    template <std::size_t N>
    XSIMD_INLINE batch<float, N> to_float(const batch<int32_t, N>& x)
    {
        XSIMD_FALLBACK_BATCH_STATIC_CAST(float, x)
    }

    template <std::size_t N>
    XSIMD_INLINE batch<double, N> to_float(const batch<int64_t, N>& x)
    {
    ...
    template <std::size_t N>
    XSIMD_INLINE batch_bool<int32_t, N> bool_cast(const batch_bool<float, N>& x)
    {
        XSIMD_FALLBACK_BOOL_CAST(int32_t, x)
    }

    template <std::size_t N>
    XSIMD_INLINE batch_bool<int64_t, N> bool_cast(const batch_bool<double, N>& x)
    {
        XSIMD_FALLBACK_BOOL_CAST(int64_t, x)
    }

    template <std::size_t N>
    XSIMD_INLINE batch_bool<float, N> bool_cast(const batch_bool<int32_t, N>& x)
    {
        XSIMD_FALLBACK_BOOL_CAST(float, x)
    }

    template <std::size_t N>
    XSIMD_INLINE batch_bool<double, N> bool_cast(const batch_bool<int64_t, N>& x)
    {
