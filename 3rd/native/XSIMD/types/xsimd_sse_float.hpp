...
namespace xsimd
{
    ...
    XSIMD_INLINE batch_bool<float, 4>::batch_bool()
    {
    }

    XSIMD_INLINE batch_bool<float, 4>::batch_bool(bool b)
    {
        m_value = _mm_castsi128_ps(_mm_set1_epi32(-(int)b));
    }

    XSIMD_INLINE batch_bool<float, 4>::batch_bool(bool b0, bool b1, bool b2, bool b3)
    {
        m_value = _mm_castsi128_ps(_mm_setr_epi32(-(int)b0, -(int)b1, -(int)b2, -(int)b3));
    }

    XSIMD_INLINE batch_bool<float, 4>::batch_bool(const __m128& rhs)
    {
        m_value = rhs;
    }

    XSIMD_INLINE batch_bool<float, 4>& batch_bool<float, 4>::operator=(const __m128& rhs)
    {
        m_value = rhs;
        return *this;
    }
    ...
    XSIMD_INLINE batch_bool<float, 4>::operator __m128() const
    {
        return m_value;
    }

    XSIMD_INLINE bool_proxy<float> batch_bool<float, 4>::operator[](std::size_t index)
    {
        return bool_proxy<float>(m_array[index & 3]);
    }

    XSIMD_INLINE bool batch_bool<float, 4>::operator[](std::size_t index) const
    {
        return static_cast<bool>(m_array[index & 3]);
    }

    XSIMD_INLINE __m128 batch_bool<float, 4>::get_value() const
    {
        return m_value;
    }

    XSIMD_INLINE batch_bool<float, 4>& batch_bool<float, 4>::load_values(bool b0, bool b1, bool b2, bool b3)
    {
        m_value = _mm_castsi128_ps(_mm_setr_epi32(-(int)b0, -(int)b1, -(int)b2, -(int)b3));
        return *this;
    }
    ...
    XSIMD_INLINE batch<float, 4>::batch(float f)
        : base_type(_mm_set1_ps(f))
    {
    }

    XSIMD_INLINE batch<float, 4>::batch(float f0, float f1, float f2, float f3)
        : base_type(_mm_setr_ps(f0, f1, f2, f3))
    {
    }

    XSIMD_INLINE batch<float, 4>::batch(const float* src)
        : base_type(_mm_loadu_ps(src))
    {
    }

    XSIMD_INLINE batch<float, 4>::batch(const float* src, aligned_mode)
        : base_type(_mm_load_ps(src))
    {
    }

    XSIMD_INLINE batch<float, 4>::batch(const float* src, unaligned_mode)
        : base_type(_mm_loadu_ps(src))
    {
    }

    XSIMD_INLINE batch<float, 4>::batch(const __m128& rhs)
        : base_type(rhs)
    {
    }

    XSIMD_INLINE batch<float, 4>& batch<float, 4>::operator=(const __m128& rhs)
    {
        this->m_value = rhs;
        return *this;
    }

    XSIMD_INLINE batch<float, 4>::operator __m128() const
    {
        return this->m_value;
    }
    ...
    XSIMD_INLINE batch<float, 4>& batch<float, 4>::load_aligned(const int32_t* src)
    {
        this->m_value = _mm_cvtepi32_ps(_mm_load_si128((__m128i const*)src));
        return *this;
    }

    XSIMD_INLINE batch<float, 4>& batch<float, 4>::load_unaligned(const int32_t* src)
    {
    ...
    XSIMD_INLINE batch<float, 4>& batch<float, 4>::load_aligned(const float* src)
    {
        this->m_value = _mm_load_ps(src);
        return *this;
    }

    XSIMD_INLINE batch<float, 4>& batch<float, 4>::load_unaligned(const float* src)
    {
        this->m_value = _mm_loadu_ps(src);
        return *this;
    }

    XSIMD_INLINE batch<float, 4>& batch<float, 4>::load_aligned(const double* src)
    {
    ...
    XSIMD_INLINE batch<float, 4>& batch<float, 4>::load_unaligned(const double* src)
    {
