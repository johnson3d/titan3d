...
#include "xsimd_neon_float16.hpp"
...
namespace xsimd
{
    ...
    XSIMD_INLINE auto batch<std::complex<float>, 4>::load_aligned(const std::complex<float>* src) -> self_type&
    {
    ...
    XSIMD_INLINE auto batch<std::complex<float>, 4>::load_unaligned(const std::complex<float>* src) -> self_type&
    {
        return load_aligned(src);
    }

    XSIMD_INLINE auto batch<std::complex<float>, 4>::load_aligned(const std::complex<double>* src) -> self_type&
    {
    ...
    XSIMD_INLINE auto batch<std::complex<float>, 4>::load_unaligned(const std::complex<double>* src) -> self_type&
    {
        return load_aligned(src);
    }

    XSIMD_INLINE void batch<std::complex<float>, 4>::store_aligned(std::complex<float>* dst) const
    {
        float32x4x2_t tmp;
    ...
    XSIMD_INLINE void batch<std::complex<float>, 4>::store_unaligned(std::complex<float>* dst) const
    {
        store_aligned(dst);
    }

    XSIMD_INLINE void batch<std::complex<float>, 4>::store_aligned(std::complex<double>* dst) const
    {
#if XSIMD_ARM_INSTR_SET >= XSIMD_ARM8_64_NEON_VERSION
    ...
    XSIMD_INLINE void batch<std::complex<float>, 4>::store_unaligned(std::complex<double>* dst) const
    {
        store_aligned(dst);
    }
    ...
    XSIMD_INLINE auto batch<std::complex<double>, 2>::load_aligned(const std::complex<float>* src) -> self_type&
    {
        const float* buf = reinterpret_cast<const float*>(src);
    ...
    XSIMD_INLINE auto batch<std::complex<double>, 2>::load_unaligned(const std::complex<float>* src) -> self_type&
    {
        return load_aligned(src);
    }

    XSIMD_INLINE auto batch<std::complex<double>, 2>::load_aligned(const std::complex<double>* src) -> self_type&
    {
        const double* buf = reinterpret_cast<const double*>(src);
    ...
    XSIMD_INLINE auto batch<std::complex<double>, 2>::load_unaligned(const std::complex<double>* src) -> self_type&
    {
        return load_aligned(src);
    }

    XSIMD_INLINE void batch<std::complex<double>, 2>::store_aligned(std::complex<float>* dst) const
    {
        float* buf = reinterpret_cast<float*>(dst);
    ...
    XSIMD_INLINE void batch<std::complex<double>, 2>::store_unaligned(std::complex<float>* dst) const
    {
        return store_aligned(dst);
    }

    XSIMD_INLINE void batch<std::complex<double>, 2>::store_aligned(std::complex<double>* dst) const
    {
        float64x2x2_t tmp;
        tmp.val[0] = this->m_real;
        tmp.val[1] = this->m_imag;
    ...
    XSIMD_INLINE void batch<std::complex<double>, 2>::store_unaligned(std::complex<double>* dst) const
    {
        return store_aligned(dst);
    }
    ...
    template <bool i3ec>
    XSIMD_INLINE auto batch<xtl::xcomplex<float, float, i3ec>, 4>::load_aligned(const value_type* src) -> self_type&
    {
        const float* buf = reinterpret_cast<const float*>(src);
        float32x4x2_t tmp = vld2q_f32(buf);
        this->m_real = tmp.val[0];
    ...
    template <bool i3ec>
    XSIMD_INLINE auto batch<xtl::xcomplex<float, float, i3ec>, 4>::load_unaligned(const value_type* src) -> self_type&
    {
        return load_aligned(src);
    }

    template <bool i3ec>
    XSIMD_INLINE auto batch<xtl::xcomplex<float, float, i3ec>, 4>::load_aligned(const xtl::complex<double, double, i3ec>* src) -> self_type&
    {
        const double* buf = reinterpret_cast<const double*>(src);
#if XSIMD_ARM_INSTR_SET >= XSIMD_ARM8_64_NEON_VERSION
        float64x2x2_t tmp0 = vld2q_f64(buf);
    ...
    template <bool i3ec>
    XSIMD_INLINE auto batch<xtl::xcomplex<float, float, i3ec>, 4>::load_unaligned(const xtl::complex<double, double, i3ec>* src) -> self_type&
    {
        return load_unaligned(src);
    }

    template <bool i3ec>
    XSIMD_INLINE void batch<xtl::xcomplex<float, float, i3ec>, 4>::store_aligned(value_type* dst) const
    {
        float32x4x2_t tmp;
        tmp.val[0] = this->m_real;
        tmp.val[1] = this->m_imag;
    ...
    template <bool i3ec>
    XSIMD_INLINE void batch<xtl::xcomplex<float, float, i3ec>, 4>::store_unaligned(value_type* dst) const
    {
        store_aligned(dst);
    }

    template <bool i3ec>
    XSIMD_INLINE void batch<xtl::xcomplex<float, float, i3ec, 4>::store_aligned(xtl::xcomplex<double, double, i3ec>* dst) const
    {
#if XSIMD_ARM_INSTR_SET >= XSIMD_ARM8_64_NEON_VERSION
        double* buf = reinterpret_cast<double*>(dst);
        float64x2x2_t tmp0{ vcvt_f64_f32(vget_low_f32(this->m_real)),
    ...
    template <bool i3ec>
    XSIMD_INLINE void batch<xtl::xcomplex<float, float, i3ec>, 4>::store_unaligned(xtl::xcomplex<double, double, i3ec>* dst) const
    {
        store_aligned(dst);
    }
    ...
    template <bool i3ec>
    XSIMD_INLINE auto batch<xtl::xcomplex<double, double, i3ec>, 2>::load_aligned(const xtl::xcomplex<float, float, i3ec>* src) -> self_type&
    {
        const float* buf = reinterpret_cast<const float*>(src);
        float32x2x2_t tmp = vld2_f32(buf);
        this->m_real = vcvt_f64_f32(tmp.val[0]);
    ...
    template <bool i3ec>
    XSIMD_INLINE auto batch<xtl::xcomplex<double, double, i3ec>, 2>::load_unaligned(const xtl::xcomplex<float, float, i3ec>* src) -> self_type&
    {
        return load_aligned(src);
    }

    template <bool i3ec>
    XSIMD_INLINE auto batch<xtl::xcomplex<double, double, i3ec>, 2>::load_aligned(const value_type* src) -> self_type&
    {
        const double* buf = reinterpret_cast<const double*>(src);
        float64x2x2_t tmp = vld2q_f64(buf);
        this->m_real = tmp.val[0];
    ...
    template <bool i3ec>
    XSIMD_INLINE auto batch<xtl::xcomplex<double, double, i3ec>, 2>::load_unaligned(const value_type* src) -> self_type&
    {
        return load_aligned(src);
    }

    template <bool i3ec>
    XSIMD_INLINE void batch<xtl::xcomplex<double, double, i3ec>, 2>::store_aligned(xtl::xcomplex<float, float, i3ec>* dst) const
    {
        float* buf = reinterpret_cast<float*>(dst);
        float32x2x2_t tmp{ vcvt_f32_f64(this->m_real), vcvt_f32_f64(this->m_imag) };
        vst2_f32(buf, tmp);
    ...
    template <bool i3ec>
    XSIMD_INLINE void batch<xtl::xcomplex<double, double, i3ec>, 2>::store_unaligned(xtl::xcomplex<float, float, i3ec>* dst) const
    {
        return store_aligned(dst);
    }

    template <bool i3ec>
    XSIMD_INLINE void batch<xtl::xcomplex<double, double, i3ec>, 2>::store_aligned(value_type* dst) const
    {
        float64x2x2_t tmp;
        tmp.val[0] = this->m_real;
        tmp.val[1] = this->m_imag;
    ...
    template <bool i3ec>
    XSIMD_INLINE void batch<xtl::xcomplex<double, double, i3ec>, 2>::store_unaligned(value_type* dst) const
    {
        return store_aligned(dst);
    }
