...
namespace xsimd
{
    ...
    XSIMD_INLINE batch<float, 4>::batch()
    {
    }

    XSIMD_INLINE batch<float, 4>::batch(float d)
        : base_type(vdupq_n_f32(d))
    {
    }

    XSIMD_INLINE batch<float, 4>::batch(float d1, float d2, float d3, float d4)
        : base_type(storage_type{d1, d2, d3, d4})
    {
    }

    XSIMD_INLINE batch<float, 4>::batch(const float* d)
        : base_type(vld1q_f32(d))
    {
    }

    XSIMD_INLINE batch<float, 4>::batch(const float* d, aligned_mode)
        : batch(d)
    {
    }

    XSIMD_INLINE batch<float, 4>::batch(const float* d, unaligned_mode)
        : batch(d)
    {
    }

    XSIMD_INLINE batch<float, 4>::batch(const storage_type& rhs)
        : base_type(rhs)
    {
    }

    XSIMD_INLINE batch<float, 4>& batch<float, 4>::operator=(const storage_type& rhs)
    {
        this->m_value = rhs;
        return *this;
    }

    XSIMD_INLINE batch<float, 4>::batch(const batch_bool_type& rhs)
        : base_type(vreinterpretq_f32_u32(vandq_u32(rhs, 
                                                    vreinterpretq_u32_f32(batch(1.f)))))
    {
    }

    XSIMD_INLINE batch<float, 4>& batch<float, 4>::operator=(const batch_bool_type& rhs)
    {
        this->m_value = vreinterpretq_f32_u32(vandq_u32(rhs, vreinterpretq_u32_f32(batch(1.f))));
        return *this;
    }

    XSIMD_INLINE batch<float, 4>& batch<float, 4>::load_aligned(const int8_t* src)
    {
    ...
    XSIMD_INLINE batch<float, 4>& batch<float, 4>::load_unaligned(const int8_t* src)
    {
    ...
    XSIMD_INLINE batch<float, 4>& batch<float, 4>::load_aligned(const uint8_t* src)
    {
    ...
    XSIMD_INLINE batch<float, 4>& batch<float, 4>::load_unaligned(const uint8_t* src)
    {
        return load_aligned(src);
    }

    XSIMD_INLINE batch<float, 4>& batch<float, 4>::load_aligned(const int16_t* src)
    {
    ...
    XSIMD_INLINE batch<float, 4>& batch<float, 4>::load_unaligned(const int16_t* src)
    {
        return load_aligned(src);
    }

    XSIMD_INLINE batch<float, 4>& batch<float, 4>::load_aligned(const uint16_t* src)
    {
    ...
    XSIMD_INLINE batch<float, 4>& batch<float, 4>::load_unaligned(const uint16_t* src)
    {
        return load_aligned(src);
    }

    XSIMD_INLINE batch<float, 4>& batch<float, 4>::load_aligned(const int32_t* d)
    {
        this->m_value = vcvtq_f32_s32(vld1q_s32(d));
        return *this;
    }

    XSIMD_INLINE batch<float, 4>& batch<float, 4>::load_unaligned(const int32_t* d)
    {
        return load_aligned(d);
    }

    XSIMD_INLINE batch<float, 4>& batch<float, 4>::load_aligned(const uint32_t* d)
    {
        this->m_value = vcvtq_f32_u32(vld1q_u32(d));
        return *this;
    }

    XSIMD_INLINE batch<float, 4>& batch<float, 4>::load_unaligned(const uint32_t* d)
    {
        return load_aligned(d);
    }

    XSIMD_INLINE batch<float, 4>& batch<float, 4>::load_aligned(const int64_t* d)
    {
    ...
    XSIMD_INLINE batch<float, 4>& batch<float, 4>::load_unaligned(const int64_t* d)
    {
        return load_aligned(d);
    }

    XSIMD_INLINE batch<float, 4>& batch<float, 4>::load_aligned(const uint64_t* d)
    {
    #if XSIMD_ARM_INSTR_SET >= XSIMD_ARM8_64_NEON_VERSION
    ...
    XSIMD_INLINE batch<float, 4>& batch<float, 4>::load_unaligned(const uint64_t* d)
    {
        return load_aligned(d);
    }
    ...
    XSIMD_INLINE batch<float, 4>& batch<float, 4>::load_aligned(const float* d)
    {
        this->m_value = vld1q_f32(d);
        return *this;
    }

    XSIMD_INLINE batch<float, 4>& batch<float, 4>::load_unaligned(const float* d)
    {
        return load_aligned(d);
    }

    XSIMD_INLINE batch<float, 4>& batch<float, 4>::load_aligned(const double* d)
    {
    #if XSIMD_ARM_INSTR_SET >= XSIMD_ARM8_64_NEON_VERSION
    ...
    XSIMD_INLINE batch<float, 4>& batch<float, 4>::load_unaligned(const double* d)
    {
        return load_aligned(d);
    }

    XSIMD_INLINE void batch<float, 4>::store_aligned(int8_t* dst) const
    {
    ...
    XSIMD_INLINE void batch<float, 4>::store_unaligned(int8_t* dst) const
    {
        store_aligned(dst);
    }

    XSIMD_INLINE void batch<float, 4>::store_aligned(uint8_t* dst) const
    {
    ...
    XSIMD_INLINE void batch<float, 4>::store_unaligned(uint8_t* dst) const
    {
        store_aligned(dst);
    }

    XSIMD_INLINE void batch<float, 4>::store_aligned(int16_t* dst) const
    {
        int32x4_t tmp = vcvtq_s32_f32(this->m_value);
        int16x4_t tmp2 = vmovn_s32(tmp);
    ...
    XSIMD_INLINE void batch<float, 4>::store_unaligned(int16_t* dst) const
    {
        store_aligned(dst);
    }

    XSIMD_INLINE void batch<float, 4>::store_aligned(uint16_t* dst) const
    {
        uint32x4_t tmp = vcvtq_u32_f32(this->m_value);
        uint16x4_t tmp2 = vmovn_u32(tmp);
    ...
    XSIMD_INLINE void batch<float, 4>::store_unaligned(uint16_t* dst) const
    {
        store_aligned(dst);
    }

    XSIMD_INLINE void batch<float, 4>::store_aligned(int32_t* dst) const
    {
        vst1q_s32(dst, vcvtq_s32_f32(this->m_value));
    }

    XSIMD_INLINE void batch<float, 4>::store_unaligned(int32_t* dst) const
    {
        store_aligned(dst);
    }

    XSIMD_INLINE void batch<float, 4>::store_aligned(uint32_t* dst) const
    {
        vst1q_u32(dst, vcvtq_u32_f32(this->m_value));
    }

    XSIMD_INLINE void batch<float, 4>::store_unaligned(uint32_t* dst) const
    {
        store_aligned(dst);
    }

    XSIMD_INLINE void batch<float, 4>::store_aligned(int64_t* dst) const
    {
    #if XSIMD_ARM_INSTR_SET >= XSIMD_ARM8_64_NEON_VERSION
    ...
    XSIMD_INLINE void batch<float, 4>::store_unaligned(int64_t* dst) const
    {
        store_aligned(dst);
    }

    XSIMD_INLINE void batch<float, 4>::store_aligned(uint64_t* dst) const
    {
    #if XSIMD_ARM_INSTR_SET >= XSIMD_ARM8_64_NEON_VERSION
    ...
    XSIMD_INLINE void batch<float, 4>::store_unaligned(uint64_t* dst) const
    {
        store_aligned(dst);
    }

    XSIMD_INLINE void batch<float, 4>::store_aligned(float* dst) const
    {
        vst1q_f32(dst, this->m_value);
    }

    XSIMD_INLINE void batch<float, 4>::store_unaligned(float* dst) const
    {
        store_aligned(dst);
    }

    XSIMD_INLINE void batch<float, 4>::store_aligned(double* dst) const
    {
    #if XSIMD_ARM_INSTR_SET >= XSIMD_ARM8_64_NEON_VERSION
    ...
    XSIMD_INLINE void batch<float, 4>::store_unaligned(double* dst) const
    {
        store_aligned(dst);
    }

    XSIMD_INLINE batch<float, 4>::operator float32x4_t() const
    {
        return this->m_value;
    }
    ...
    namespace detail
    {
        ...
        struct batch_kernel<float, 4>
        {
            ...
            static XSIMD_INLINE batch_type add(const batch_type& lhs, const batch_type& rhs)
            {
                return vaddq_f32(lhs, rhs);
            }

            static XSIMD_INLINE batch_type sub(const batch_type& lhs, const batch_type& rhs)
            {
                return vsubq_f32(lhs, rhs);
            }

            static XSIMD_INLINE batch_type mul(const batch_type& lhs, const batch_type& rhs)
            {
            ...
            static XSIMD_INLINE batch_bool_type lt(const batch_type& lhs, const batch_type& rhs)
            {
                return vcltq_f32(lhs, rhs);
            }

            static XSIMD_INLINE batch_bool_type lte(const batch_type& lhs, const batch_type& rhs)
            {
                return vcleq_f32(lhs, rhs);
            }

            static XSIMD_INLINE batch_type bitwise_and(const batch_type& lhs, const batch_type& rhs)
            {
                return vreinterpretq_f32_u32(vandq_u32(vreinterpretq_u32_f32(lhs),
                                                       vreinterpretq_u32_f32(rhs)));
            }

            static XSIMD_INLINE batch_type bitwise_or(const batch_type& lhs, const batch_type& rhs)
            {
                return vreinterpretq_f32_u32(vorrq_u32(vreinterpretq_u32_f32(lhs),
                                                       vreinterpretq_u32_f32(rhs)));
            }

            static XSIMD_INLINE batch_type bitwise_xor(const batch_type& lhs, const batch_type& rhs)
            {
                return vreinterpretq_f32_u32(veorq_u32(vreinterpretq_u32_f32(lhs),
                                                       vreinterpretq_u32_f32(rhs)));
            }

            static XSIMD_INLINE batch_type bitwise_not(const batch_type& rhs)
            {
                return vreinterpretq_f32_u32(vmvnq_u32(vreinterpretq_u32_f32(rhs)));
            }

            static XSIMD_INLINE batch_type bitwise_andnot(const batch_type& lhs, const batch_type& rhs)
            {
            ...
            static XSIMD_INLINE batch_type fma(const batch_type& x, const batch_type& y, const batch_type& z)
            {
            ...
            static XSIMD_INLINE batch_type fnma(const batch_type& x, const batch_type& y, const batch_type& z)
            {
                return fma(-x, y, z);
            }
            ...
            static XSIMD_INLINE batch_type select(const batch_bool_type& cond, const batch_type& a, const batch_type& b)
            {
				// add by vader, see _mm_blendv_ps
				// todo: optimize
				const uint32x4_t mask31 = vdupq_n_u32(1 << 31);
				uint32x4_t cond_adjusted = vceqq_u32(vandq_u32(cond, mask31), mask31);
                return vbslq_f32(cond_adjusted, a, b);
            }

            static batch_bool_type xsimd_isnan(const batch_type& x)
            {
                return !(x == x);
            }
        };
