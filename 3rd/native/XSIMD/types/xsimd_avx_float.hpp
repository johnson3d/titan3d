...
namespace xsimd
{
    ...
    XSIMD_INLINE batch_bool<float, 8>::batch_bool()
    {
    }

    XSIMD_INLINE batch_bool<float, 8>::batch_bool(bool b)
    {
        m_value = _mm256_castsi256_ps(_mm256_set1_epi32(-(int)b));
    }

    XSIMD_INLINE batch_bool<float, 8>::batch_bool(bool b0, bool b1, bool b2, bool b3,
                                            bool b4, bool b5, bool b6, bool b7)
    {
    ...
    XSIMD_INLINE batch_bool<float, 8>::batch_bool(const __m256& rhs)
    {
        m_value = rhs;
    }

    XSIMD_INLINE batch_bool<float, 8>& batch_bool<float, 8>::operator=(const __m256& rhs)
    {
        m_value = rhs;
        return *this;
    }

    XSIMD_INLINE batch<float, 8>::batch(const batch_bool_type& rhs)
        : base_type(_mm256_and_ps(rhs, batch(1.f)))
    {
    }

    XSIMD_INLINE batch<float, 8>& batch<float, 8>::operator=(const batch_bool_type& rhs)
    {
        this->m_value = _mm256_and_ps(rhs, batch(1.f));
        return *this;
    }

    XSIMD_INLINE batch_bool<float, 8>::operator __m256() const
    {
        return m_value;
    }

    XSIMD_INLINE bool_proxy<float> batch_bool<float, 8>::operator[](std::size_t index)
    {
        return bool_proxy<float>(m_array[index & 7]);
    }

    XSIMD_INLINE bool batch_bool<float, 8>::operator[](std::size_t index) const
    {
        return static_cast<bool>(m_array[index & 7]);
    }

    XSIMD_INLINE __m256 batch_bool<float, 8>::get_value() const
    {
        return m_value;
    }

    XSIMD_INLINE batch_bool<float, 8>& batch_bool<float, 8>::load_values(bool b0, bool b1, bool b2, bool b3,
                                                                   bool b4, bool b5, bool b6, bool b7)
    {
        m_value = _mm256_castsi256_ps(
              _mm256_setr_epi32(-(int)b0, -(int)b1, -(int)b2, -(int)b3,
    ...
    XSIMD_INLINE batch<float, 8>::batch()
    {
    }

    XSIMD_INLINE batch<float, 8>::batch(float f)
        : base_type(_mm256_set1_ps(f))
    {
    }

    XSIMD_INLINE batch<float, 8>::batch(float f0, float f1, float f2, float f3,
                                  float f4, float f5, float f6, float f7)
        : base_type(_mm256_setr_ps(f0, f1, f2, f3, f4, f5, f6, f7))
    {
    }

    XSIMD_INLINE batch<float, 8>::batch(const float* src)
        : base_type(_mm256_loadu_ps(src))
    {
    }

    XSIMD_INLINE batch<float, 8>::batch(const float* src, aligned_mode)
        : base_type(_mm256_load_ps(src))
    {
    }

    XSIMD_INLINE batch<float, 8>::batch(const float* src, unaligned_mode)
        : base_type(_mm256_loadu_ps(src))
    {
    }

    XSIMD_INLINE batch<float, 8>::batch(const __m256& rhs)
        : base_type(rhs)
    {
    }

    XSIMD_INLINE batch<float, 8>& batch<float, 8>::operator=(const __m256& rhs)
    {
        this->m_value = rhs;
        return *this;
    }

    XSIMD_INLINE batch<float, 8>::operator __m256() const
    {
        return this->m_value;
    }
    ...
    XSIMD_INLINE batch<float, 8>& batch<float, 8>::load_aligned(const int8_t* src)
    {
    ...
    XSIMD_INLINE batch<float, 8>& batch<float, 8>::load_unaligned(const int8_t* src)
    {
        return load_aligned(src);
    }

    XSIMD_INLINE batch<float, 8>& batch<float, 8>::load_aligned(const uint8_t* src)
    {
        __m128i tmp = _mm_loadl_epi64((const __m128i*)src);
    ...
    XSIMD_INLINE batch<float, 8>& batch<float, 8>::load_unaligned(const uint8_t* src)
    {
        return load_aligned(src);
    }

    XSIMD_INLINE batch<float, 8>& batch<float, 8>::load_aligned(const int16_t* src)
    {
    ...
    XSIMD_INLINE batch<float, 8>& batch<float, 8>::load_unaligned(const int16_t* src)
    {
    ...
    XSIMD_INLINE batch<float, 8>& batch<float, 8>::load_aligned(const uint16_t* src)
    {
        __m128i tmp = _mm_load_si128((const __m128i*)src);
    ...
    XSIMD_INLINE batch<float, 8>& batch<float, 8>::load_unaligned(const uint16_t* src)
    {
        __m128i tmp = _mm_loadu_si128((const __m128i*)src);
        __m256i tmp2 = detail::xsimd_cvtepu16_epi32(tmp);
        this->m_value = _mm256_cvtepi32_ps(tmp2);
    ...
    XSIMD_INLINE batch<float, 8>& batch<float, 8>::load_aligned(const int32_t* src)
    {
        this->m_value = _mm256_cvtepi32_ps(_mm256_load_si256((__m256i const*)src));
        return *this;
    }

    XSIMD_INLINE batch<float, 8>& batch<float, 8>::load_unaligned(const int32_t* src)
    {
    ...
    XSIMD_INLINE batch<float, 8>& batch<float, 8>::load_aligned(const float* src)
    {
        this->m_value = _mm256_load_ps(src);
        return *this;
    }

    XSIMD_INLINE batch<float, 8>& batch<float, 8>::load_unaligned(const float* src)
    {
        this->m_value = _mm256_loadu_ps(src);
        return *this;
    }

    XSIMD_INLINE batch<float, 8>& batch<float, 8>::load_aligned(const double* src)
    {
    ...
    XSIMD_INLINE batch<float, 8>& batch<float, 8>::load_unaligned(const double* src)
    {
    ...
    XSIMD_INLINE void batch<float, 8>::store_aligned(int8_t* dst) const
    {
    ...
    XSIMD_INLINE void batch<float, 8>::store_unaligned(int8_t* dst) const
    {
        store_aligned(dst);
    }

    XSIMD_INLINE void batch<float, 8>::store_aligned(uint8_t* dst) const
    {
        __m256i tmp = _mm256_cvtps_epi32(this->m_value);
    ...
    XSIMD_INLINE void batch<float, 8>::store_unaligned(uint8_t* dst) const
    {
        store_aligned(dst);
    }

    XSIMD_INLINE void batch<float, 8>::store_aligned(int16_t* dst) const
    {
        __m256i tmp = _mm256_cvtps_epi32(this->m_value);
    ...
    XSIMD_INLINE void batch<float, 8>::store_unaligned(int16_t* dst) const
    {
        __m256i tmp = _mm256_cvtps_epi32(this->m_value);
        __m128i tmp2 = detail::xsimd_cvtepi32_epi16(tmp);
    ...
    XSIMD_INLINE void batch<float, 8>::store_aligned(uint16_t* dst) const
    {
        __m256i tmp = _mm256_cvtps_epi32(this->m_value);
    ...
    XSIMD_INLINE void batch<float, 8>::store_unaligned(uint16_t* dst) const
    {
        __m256i tmp = _mm256_cvtps_epi32(this->m_value);
        __m128i tmp2 = detail::xsimd_cvtepi32_epu16(tmp);
        _mm_storeu_si128((__m128i*)dst, tmp2);
    ...
    XSIMD_INLINE void batch<float, 8>::store_aligned(int32_t* dst) const
    {
        _mm256_store_si256((__m256i*)dst, _mm256_cvtps_epi32(this->m_value));
    }

    XSIMD_INLINE void batch<float, 8>::store_unaligned(int32_t* dst) const
    {
        _mm256_storeu_si256((__m256i*)dst, _mm256_cvtps_epi32(this->m_value));
    }

    XSIMD_INLINE void batch<float, 8>::store_aligned(float* dst) const
    {
        _mm256_store_ps(dst, this->m_value);
    }

    XSIMD_INLINE void batch<float, 8>::store_unaligned(float* dst) const
    {
        _mm256_storeu_ps(dst, this->m_value);
    }

    XSIMD_INLINE void batch<float, 8>::store_aligned(double* dst) const
    {
    ...
    XSIMD_INLINE void batch<float, 8>::store_unaligned(double* dst) const
    {
        store_aligned(dst);
    }
