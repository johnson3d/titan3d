...
namespace xsimd
{
    ...
    template <class MASK>
    XSIMD_INLINE bool_mask_proxy<MASK>::bool_mask_proxy(MASK& ref, std::size_t idx)
        : m_ref(ref), m_idx(idx)
    {
    }

    template <class MASK>
    XSIMD_INLINE bool_mask_proxy<MASK>::operator bool() const
    {
        return ((m_ref >> m_idx) & MASK(1)) != 0;
    }

    template <class MASK>
    XSIMD_INLINE bool_mask_proxy<MASK>& bool_mask_proxy<MASK>::operator=(bool rhs)
    {
    ...
    XSIMD_INLINE batch_bool_avx512<MASK, T>::batch_bool_avx512()
    {
    }

    template <class MASK, class T>
    template <class... Args, class>
    XSIMD_INLINE batch_bool_avx512<MASK, T>::batch_bool_avx512(Args... args)
        : batch_bool_avx512({{static_cast<bool>(args)...}})
    {
    }

    template <class MASK, class T>
    XSIMD_INLINE batch_bool_avx512<MASK, T>::batch_bool_avx512(bool b)
        : m_value(b ? -1 : 0)
    {
    }
    ...
    template <class MASK, class T>
    XSIMD_INLINE batch_bool_avx512<MASK, T>::batch_bool_avx512(const bool (&init)[sizeof(MASK) * 8])
        : m_value(detail::get_init_value<MASK>(init, detail::make_index_sequence<sizeof(MASK) * 8>{}))
    {
    }

    template <class MASK, class T>
    XSIMD_INLINE batch_bool_avx512<MASK, T>::batch_bool_avx512(const MASK& rhs)
        : m_value(rhs)
    {
    }

    template <class MASK, class T>
    XSIMD_INLINE batch_bool_avx512<MASK, T>::operator MASK() const
    {
        return m_value;
    }

    template <class MASK, class T>
    XSIMD_INLINE bool_mask_proxy<MASK> batch_bool_avx512<MASK, T>::operator[](std::size_t idx)
    {
    ...
    template <class MASK, class T>
    XSIMD_INLINE bool batch_bool_avx512<MASK, T>::operator[](std::size_t idx) const
    {
        std::size_t s = simd_batch_traits<T>::size - 1;
    ...
    template <class MASK, class T>
    XSIMD_INLINE T& batch_bool_avx512<MASK, T>::load_array(const std::array<bool, sizeof(MASK) * 8>& src)
    {
        MASK tmp(false);
    ...
    template <class MASK, class T>
    template <class... Args>
    XSIMD_INLINE T& batch_bool_avx512<MASK, T>::load_values(Args... b)
    {
        return load_array({b...});
    }
    ...
    template <class T, std::size_t N>
    XSIMD_INLINE avx512_fallback_batch_bool<T, N>::avx512_fallback_batch_bool()
    {
    }

    template <class T, std::size_t N>
    XSIMD_INLINE avx512_fallback_batch_bool<T, N>::avx512_fallback_batch_bool(bool b)
    ...
    template <class T, std::size_t N>
    template <class... Args, class>
    XSIMD_INLINE avx512_fallback_batch_bool<T, N>::avx512_fallback_batch_bool(Args... args)
    ...
    template <class T, std::size_t N>
    XSIMD_INLINE avx512_fallback_batch_bool<T, N>::avx512_fallback_batch_bool(const __m512i& rhs)
        : m_value(rhs)
    {
    }

    template <class T, std::size_t N>
    XSIMD_INLINE avx512_fallback_batch_bool<T, N>::operator __m512i() const
    {
        return m_value;
    }

    template <class T, std::size_t N>
    XSIMD_INLINE avx512_fallback_batch_bool<T, N>& avx512_fallback_batch_bool<T, N>::operator=(const __m512i& rhs)
    {
        m_value = rhs;
        return *this;
    }
    ...
    template <class T, std::size_t N>
    XSIMD_INLINE bool_proxy<T> avx512_fallback_batch_bool<T, N>::operator[](std::size_t idx)
    {
        return bool_proxy<T>(m_array[idx & (N - 1)]);
    }

    template <class T, std::size_t N>
    XSIMD_INLINE bool avx512_fallback_batch_bool<T, N>::operator[](std::size_t idx) const
    {
    ...
    template <class T, std::size_t N>
    template <class... Args>
    XSIMD_INLINE batch_bool<T, N>& avx512_fallback_batch_bool<T, N>::load_values(Args... b)
    {
        m_value = avx512_detail::int_init(std::integral_constant<std::size_t, sizeof(T)>{},
