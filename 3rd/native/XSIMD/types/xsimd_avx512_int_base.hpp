...
#define XSIMD_APPLY_AVX2_FUNCTION(N, func, avx_lhs, avx_rhs)                                          \
    ...
    namespace avx512_detail
    {
        ...
        XSIMD_INLINE __m512i int_init(std::integral_constant<std::size_t, 4>,
        ...
        XSIMD_INLINE __m512i int_init(std::integral_constant<std::size_t, 8>,
        ...
        template <class T>
        XSIMD_INLINE __m512i int_set(std::integral_constant<std::size_t, 1>, T v)
        {
            return _mm512_set1_epi8(v);
        }

        template <class T>
        XSIMD_INLINE __m512i int_set(std::integral_constant<std::size_t, 2>, T v)
        {
            return _mm512_set1_epi16(v);
        }

        template <class T>
        XSIMD_INLINE __m512i int_set(std::integral_constant<std::size_t, 4>, T v)
        {
            return _mm512_set1_epi32(v);
        }

        template <class T>
        XSIMD_INLINE __m512i int_set(std::integral_constant<std::size_t, 8>, T v)
        {
    ...
    template <class T, std::size_t N>
    XSIMD_INLINE avx512_int_batch<T, N>::avx512_int_batch()
    {
    }

    template <class T, std::size_t N>
    XSIMD_INLINE avx512_int_batch<T, N>::avx512_int_batch(T i)
    ...
    XSIMD_INLINE avx512_int_batch<T, N>::avx512_int_batch(Args... args)
        : base_type(avx512_detail::int_init(std::integral_constant<std::size_t, sizeof(T)>{}, args...))
    {
    }

    template <class T, std::size_t N>
    XSIMD_INLINE avx512_int_batch<T, N>::avx512_int_batch(const T* src)
        : base_type(_mm512_loadu_si512((__m512i const*) src))
    {
    }

    template <class T, std::size_t N>
    XSIMD_INLINE avx512_int_batch<T, N>::avx512_int_batch(const T* src, aligned_mode)
        : base_type(_mm512_load_si512((__m512i const*) src))
    {
    }

    template <class T, std::size_t N>
    XSIMD_INLINE avx512_int_batch<T, N>::avx512_int_batch(const T* src, unaligned_mode)
        : base_type(_mm512_loadu_si512((__m512i const*) src))
    {
    }

    template <class T, std::size_t N>
    XSIMD_INLINE avx512_int_batch<T, N>::avx512_int_batch(const __m512i& rhs)
        : base_type(rhs)
    {
    }

    template <class T, std::size_t N>
    XSIMD_INLINE avx512_int_batch<T, N>& avx512_int_batch<T, N>::operator=(const __m512i& rhs)
    {
        this->m_value = rhs;
        return *this;
    }
    ...
    template <class T, std::size_t N>
    XSIMD_INLINE avx512_int_batch<T, N>::avx512_int_batch(const batch_bool<T, N>& rhs)
    ...
    template <class T, std::size_t N>
    XSIMD_INLINE avx512_int_batch<T, N>::operator __m512i() const
    {
        return this->m_value;
    }

    template <class T, std::size_t N>
    XSIMD_INLINE batch<T, N>& avx512_int_batch<T, N>::load_aligned(const T* src)
    {
    ...
    template <class T, std::size_t N>
    XSIMD_INLINE batch<T, N>& avx512_int_batch<T, N>::load_unaligned(const T* src)
    {
    ...
    template <class T, std::size_t N>
    XSIMD_INLINE batch<T, N>& avx512_int_batch<T, N>::load_aligned(const flipped_sign_type_t<T>* src)
    {
        this->m_value = _mm512_load_si512((__m512i const*) src);
        return (*this)();
    }
    ...
    template <class T, std::size_t N>
    XSIMD_INLINE batch<T, N>& avx512_int_batch<T, N>::load_unaligned(const flipped_sign_type_t<T>* src)
    {
        this->m_value = _mm512_loadu_si512((__m512i const*) src);
        return (*this)();
    }
    ...
    template <class T, std::size_t N>
    XSIMD_INLINE void avx512_int_batch<T, N>::store_aligned(T* dst) const
    {
        _mm512_store_si512(dst, this->m_value);
    }

    template <class T, std::size_t N>
    XSIMD_INLINE void avx512_int_batch<T, N>::store_unaligned(T* dst) const
    {
        _mm512_storeu_si512(dst, this->m_value);
    }

    template <class T, std::size_t N>
    XSIMD_INLINE void avx512_int_batch<T, N>::store_aligned(flipped_sign_type_t<T>* dst) const
    {
        _mm512_store_si512(dst, this->m_value);
    }

    template <class T, std::size_t N>
    XSIMD_INLINE void avx512_int_batch<T, N>::store_unaligned(flipped_sign_type_t<T>* dst) const
    {
        _mm512_storeu_si512(dst, this->m_value);
    }
