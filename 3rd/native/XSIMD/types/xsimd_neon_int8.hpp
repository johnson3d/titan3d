...
namespace xsimd
{
    ...
    XSIMD_INLINE batch<int8_t, 16>::batch()
    {
    }

    XSIMD_INLINE batch<int8_t, 16>::batch(int8_t d)
        : base_type(vdupq_n_s8(d))
    {
    }

    template <class... Args, class>
    XSIMD_INLINE batch<int8_t, 16>::batch(Args... args)
        : base_type(storage_type{static_cast<int8_t>(args)...})
    {
    }

    XSIMD_INLINE batch<int8_t, 16>::batch(const int8_t* d)
        : base_type(vld1q_s8(d))
    {
    }

    XSIMD_INLINE batch<int8_t, 16>::batch(const int8_t* d, aligned_mode)
        : batch(d)
    {
    }

    XSIMD_INLINE batch<int8_t, 16>::batch(const int8_t* d, unaligned_mode)
        : batch(d)
    {
    }

    XSIMD_INLINE batch<int8_t, 16>::batch(const char* d)
        : batch(reinterpret_cast<const int8_t*>(d))
    {
    }

    XSIMD_INLINE batch<int8_t, 16>::batch(const char* d, aligned_mode)
        : batch(reinterpret_cast<const int8_t*>(d))
    {
    }

    XSIMD_INLINE batch<int8_t, 16>::batch(const char* d, unaligned_mode)
        : batch(reinterpret_cast<const int8_t*>(d))
    {
    }

    XSIMD_INLINE batch<int8_t, 16>::batch(const storage_type& rhs)
        : base_type(rhs)
    {
    }

    XSIMD_INLINE batch<int8_t, 16>& batch<int8_t, 16>::operator=(const storage_type& rhs)
    {
        this->m_value = rhs;
        return *this;
    }
    ...
    namespace detail
    {
        XSIMD_INLINE int8x16_t init_from_bool(uint8x16_t a)
        {
            return vandq_s8(reinterpret_cast<int8x16_t>(a), vdupq_n_s8(1));
        }
    }

    XSIMD_INLINE batch<int8_t, 16>::batch(const batch_bool_type& rhs)
        : base_type(detail::init_from_bool(rhs))
    {
    }

    XSIMD_INLINE batch<int8_t, 16>& batch<int8_t, 16>::operator=(const batch_bool_type& rhs)
    {
        this->m_value = detail::init_from_bool(rhs);
        return *this;
    }

    XSIMD_INLINE batch<int8_t, 16>& batch<int8_t, 16>::load_aligned(const int8_t* src)
    {
        this->m_value = vld1q_s8(src);
        return *this;
    }

    XSIMD_INLINE batch<int8_t, 16>& batch<int8_t, 16>::load_unaligned(const int8_t* src)
    {
        return load_aligned(src);
    }

    XSIMD_INLINE batch<int8_t, 16>& batch<int8_t, 16>::load_aligned(const uint8_t* src)
    {
        this->m_value = vreinterpretq_s8_u8(vld1q_u8(src));
        return *this;
    }

    XSIMD_INLINE batch<int8_t, 16>& batch<int8_t, 16>::load_unaligned(const uint8_t* src)
    {
        return load_aligned(src);
    }

    XSIMD_INLINE void batch<int8_t, 16>::store_aligned(int8_t* dst) const
    {
        vst1q_s8(dst, this->m_value);
    }

    XSIMD_INLINE void batch<int8_t, 16>::store_unaligned(int8_t* dst) const
    {
        store_aligned(dst);
    }

    XSIMD_INLINE void batch<int8_t, 16>::store_aligned(uint8_t* dst) const
    {
        vst1q_u8(dst, vreinterpretq_u8_s8(this->m_value));
    }

    XSIMD_INLINE void batch<int8_t, 16>::store_unaligned(uint8_t* dst) const
    {
        store_aligned(dst);
    }
    ...
    XSIMD_INLINE batch<int8_t, 16>::operator int8x16_t() const
    {
        return this->m_value;
    }
    ...
    namespace detail
    {
        XSIMD_INLINE batch<int8_t, 16> shift_left(const batch<int8_t, 16>& lhs, int32_t n)
        {
            switch(n)
            {
                case 0: return lhs;
        ...
        XSIMD_INLINE batch<int8_t, 16> shift_right(const batch<int8_t, 16>& lhs, int32_t n)
        {
            switch(n)
            {
                case 0: return lhs;
    ...
    XSIMD_INLINE batch<int8_t, 16> operator<<(const batch<int8_t, 16>& lhs, int32_t rhs)
    {
    ...
    XSIMD_INLINE batch<int8_t, 16> operator<<(const batch<int8_t, 16>& lhs, const batch<int8_t, 16>& rhs)
    {
