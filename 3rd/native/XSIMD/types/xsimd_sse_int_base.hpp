...
namespace xsimd
{
   ...
   /********************
    ...
    {
        template <class... Args>
        XSIMD_INLINE __m128i int_init(std::integral_constant<std::size_t, 1>, Args... args)
        {
            return _mm_setr_epi8(args...);
        }

        template <class... Args>
        XSIMD_INLINE __m128i int_init(std::integral_constant<std::size_t, 2>, Args... args)
        {
            return _mm_setr_epi16(args...);
        }

        template <class... Args>
        XSIMD_INLINE __m128i int_init(std::integral_constant<std::size_t, 4>, Args... args)
        {
            return _mm_setr_epi32(args...);
        }

        template <class... Args, class I0, class I1>
        XSIMD_INLINE __m128i int_init(std::integral_constant<std::size_t, 8>, I0 i0, I1 i1)
        {
            return _mm_set_epi64x(i1, i0);
        }

        template <class T>
        XSIMD_INLINE __m128i int_set(std::integral_constant<std::size_t, 1>, T v)
        {
            return _mm_set1_epi8(v);
        }

        template <class T>
        XSIMD_INLINE __m128i int_set(std::integral_constant<std::size_t, 2>, T v)
        {
            return _mm_set1_epi16(v);
        }

        template <class T>
        XSIMD_INLINE __m128i int_set(std::integral_constant<std::size_t, 4>, T v)
        {
            return _mm_set1_epi32(v);
        }

        template <class T>
        XSIMD_INLINE __m128i int_set(std::integral_constant<std::size_t, 8>, T v)
        {
            return _mm_set1_epi64x(v);
        }

        XSIMD_INLINE __m128i cmpeq_epi64_sse2(__m128i lhs, __m128i rhs)
        {
    ...
    template <class T, std::size_t N>
    XSIMD_INLINE sse_int_batch_bool<T, N>::sse_int_batch_bool()
    {
    }

    template <class T, std::size_t N>
    XSIMD_INLINE sse_int_batch_bool<T, N>::sse_int_batch_bool(bool b)
    ...
    XSIMD_INLINE sse_int_batch_bool<T, N>::sse_int_batch_bool(Args... args)
    ...
    template <class T, std::size_t N>
    XSIMD_INLINE sse_int_batch_bool<T, N>::sse_int_batch_bool(const __m128i& rhs)
        : m_value(rhs)
    {
    }

    template <class T, std::size_t N>
    XSIMD_INLINE sse_int_batch_bool<T, N>& sse_int_batch_bool<T, N>::operator=(const __m128i& rhs)
    {
        m_value = rhs;
        return *this;
    }
    ...
    template <class T, std::size_t N>
    XSIMD_INLINE sse_int_batch_bool<T, N>::operator __m128i() const
    {
        return m_value;
    }

    template <class T, std::size_t N>
    XSIMD_INLINE bool_proxy<T> sse_int_batch_bool<T, N>::operator[](std::size_t index)
    {
        return bool_proxy<T>(m_array[index & (N - 1)]);
    }

    template <class T, std::size_t N>
    XSIMD_INLINE bool sse_int_batch_bool<T, N>::operator[](std::size_t index) const
    {
        return static_cast<bool>(m_array[index & (N - 1)]);
    }

    template <class T, std::size_t N>
    XSIMD_INLINE __m128i sse_int_batch_bool<T, N>::get_value() const
    {
        return m_value;
    }
    ...
    template <class T, std::size_t N>
    template <class... Args>
    XSIMD_INLINE batch_bool<T, N>& sse_int_batch_bool<T, N>::load_values(Args... args)
    {
        m_value = sse_detail::int_init(std::integral_constant<std::size_t, sizeof(T)>{},
                                       static_cast<T>(args ? typename std::make_signed<T>::type{-1} : 0)...);
        return (*this)();
    ...
    template <class T, std::size_t N>
    XSIMD_INLINE sse_int_batch<T, N>::sse_int_batch()
    {
    }

    template <class T, std::size_t N>
    XSIMD_INLINE sse_int_batch<T, N>::sse_int_batch(T i)
    ...
    template <class T, std::size_t N>
    template <class... Args, class>
    XSIMD_INLINE sse_int_batch<T, N>::sse_int_batch(Args... args)
        : base_type(sse_detail::int_init(std::integral_constant<std::size_t, sizeof(T)>{}, args...))
    {
    }

    template <class T, std::size_t N>
    XSIMD_INLINE sse_int_batch<T, N>::sse_int_batch(const T* src)
        : base_type(_mm_loadu_si128((__m128i const*)src))
    {
    }

    template <class T, std::size_t N>
    XSIMD_INLINE sse_int_batch<T, N>::sse_int_batch(const T* src, aligned_mode)
        : base_type(_mm_load_si128((__m128i const*)src))
    {
    }

    template <class T, std::size_t N>
    XSIMD_INLINE sse_int_batch<T, N>::sse_int_batch(const T* src, unaligned_mode)
        : base_type(_mm_loadu_si128((__m128i const*)src))
    {
    }

    template <class T, std::size_t N>
    XSIMD_INLINE sse_int_batch<T, N>::sse_int_batch(const __m128i& rhs)
        : base_type(rhs)
    {
    }

    template <class T, std::size_t N>
    XSIMD_INLINE sse_int_batch<T, N>::sse_int_batch(const batch_bool_type& rhs)
        : base_type(_mm_and_si128(rhs, batch<T, N>(1)))
    {
    }

    template <class T, std::size_t N>
    XSIMD_INLINE batch<T, N>& sse_int_batch<T, N>::operator=(const __m128i& rhs)
    {
        this->m_value = rhs;
        return (*this)();
    }
    ...
    template <class T, std::size_t N>
    XSIMD_INLINE batch<T, N>& sse_int_batch<T, N>::operator=(const batch_bool_type& rhs)
    {
    ...
    template <class T, std::size_t N>
    XSIMD_INLINE sse_int_batch<T, N>::operator __m128i() const
    {
        return this->m_value;
    }

    template <class T, std::size_t N>
    XSIMD_INLINE batch<T, N>& sse_int_batch<T, N>::load_aligned(const T* src)
    {
    ...
    template <class T, std::size_t N>
    XSIMD_INLINE batch<T, N>& sse_int_batch<T, N>::load_unaligned(const T* src)
    {
    ...
    template <class T, std::size_t N>
    XSIMD_INLINE batch<T, N>& sse_int_batch<T, N>::load_aligned(const flipped_sign_type_t<T>* src)
    {
        this->m_value = _mm_load_si128((__m128i const*)src);
        return (*this)();
    }
    ...
    template <class T, std::size_t N>
    XSIMD_INLINE batch<T, N>& sse_int_batch<T, N>::load_unaligned(const flipped_sign_type_t<T>* src)
    {
        this->m_value = _mm_loadu_si128((__m128i const*)src);
        return (*this)();
    }
    ...
    template <class T, std::size_t N>
    XSIMD_INLINE void sse_int_batch<T, N>::store_aligned(T* dst) const
    {
        _mm_store_si128((__m128i*)dst, this->m_value);
    }

    template <class T, std::size_t N>
    XSIMD_INLINE void sse_int_batch<T, N>::store_unaligned(T* dst) const
    {
        _mm_storeu_si128((__m128i*)dst, this->m_value);
    }

    template <class T, std::size_t N>
    XSIMD_INLINE void sse_int_batch<T, N>::store_aligned(flipped_sign_type_t<T>* dst) const
    {
        _mm_store_si128((__m128i*)dst, this->m_value);
    }

    template <class T, std::size_t N>
    XSIMD_INLINE void sse_int_batch<T, N>::store_unaligned(flipped_sign_type_t<T>* dst) const
    {
        _mm_storeu_si128((__m128i*)dst, this->m_value);
    }
    ...
    namespace sse_detail
    {
        ...
        XSIMD_INLINE batch<T, N> shift_impl(F&& f, const batch<T, N>& lhs, int32_t rhs)
        {
        ...
        XSIMD_INLINE batch<T, N> shift_impl(F&& f, const batch<T, N>& lhs, const batch<S, N>& rhs)
        {
            alignas(16) T tmp_lhs[N], tmp_res[N];
