...
namespace xsimd
{
    ...
    XSIMD_INLINE batch<int16_t, 8>::batch()
    {
    }

    XSIMD_INLINE batch<int16_t, 8>::batch(int16_t d)
        : base_type(vdupq_n_s16(d))
    {
    }

    template <class... Args, class>
    XSIMD_INLINE batch<int16_t, 8>::batch(Args... args)
        : base_type(storage_type{static_cast<int16_t>(args)...})
    {
    }

    XSIMD_INLINE batch<int16_t, 8>::batch(const int16_t* d)
        : base_type(vld1q_s16(d))
    {
    }

    XSIMD_INLINE batch<int16_t, 8>::batch(const int16_t* d, aligned_mode)
        : batch(d)
    {
    }

    XSIMD_INLINE batch<int16_t, 8>::batch(const int16_t* d, unaligned_mode)
        : batch(d)
    {
    }

    XSIMD_INLINE batch<int16_t, 8>::batch(const char* d)
        : batch(reinterpret_cast<const int16_t*>(d))
    {
    }

    XSIMD_INLINE batch<int16_t, 8>::batch(const char* d, aligned_mode)
        : batch(reinterpret_cast<const int16_t*>(d))
    {
    }

    XSIMD_INLINE batch<int16_t, 8>::batch(const char* d, unaligned_mode)
        : batch(reinterpret_cast<const int16_t*>(d))
    {
    }

    XSIMD_INLINE batch<int16_t, 8>::batch(const storage_type& rhs)
        : base_type(rhs)
    {
    }

    XSIMD_INLINE batch<int16_t, 8>& batch<int16_t, 8>::operator=(const storage_type& rhs)
    {
        this->m_value = rhs;
        return *this;
    }
    ...
    namespace detail
    {
        XSIMD_INLINE int16x8_t init_from_bool(uint16x8_t a)
        {
            return vandq_s16(reinterpret_cast<int16x8_t>(a), vdupq_n_s16(1));
        }
    }

    XSIMD_INLINE batch<int16_t, 8>::batch(const batch_bool_type& rhs)
        : base_type(detail::init_from_bool(rhs))
    {
    }

    XSIMD_INLINE batch<int16_t, 8>& batch<int16_t, 8>::operator=(const batch_bool_type& rhs)
    {
        this->m_value = detail::init_from_bool(rhs);
        return *this;
    }

    XSIMD_INLINE batch<int16_t, 8>& batch<int16_t, 8>::load_aligned(const int16_t* src)
    {
        this->m_value = vld1q_s16(src);
        return *this;
    }

    XSIMD_INLINE batch<int16_t, 8>& batch<int16_t, 8>::load_unaligned(const int16_t* src)
    {
        return load_aligned(src);
    }

    XSIMD_INLINE batch<int16_t, 8>& batch<int16_t, 8>::load_aligned(const uint16_t* src)
    {
        this->m_value = vreinterpretq_s16_u16(vld1q_u16(src));
        return *this;
    }

    XSIMD_INLINE batch<int16_t, 8>& batch<int16_t, 8>::load_unaligned(const uint16_t* src)
    {
        return load_aligned(src);
    }

    XSIMD_INLINE void batch<int16_t, 8>::store_aligned(int16_t* dst) const
    {
        vst1q_s16(dst, this->m_value);
    }

    XSIMD_INLINE void batch<int16_t, 8>::store_unaligned(int16_t* dst) const
    {
        store_aligned(dst);
    }

    XSIMD_INLINE void batch<int16_t, 8>::store_aligned(uint16_t* dst) const
    {
        vst1q_u16(dst, vreinterpretq_u16_s16(this->m_value));
    }

    XSIMD_INLINE void batch<int16_t, 8>::store_unaligned(uint16_t* dst) const
    {
        store_aligned(dst);
    }
    ...
    XSIMD_INLINE batch<int16_t, 8>::operator int16x8_t() const
    {
        return this->m_value;
    }
    ...
    namespace detail
    {
        XSIMD_INLINE batch<int16_t, 8> shift_left(const batch<int16_t, 8>& lhs, int32_t n)
        {
            switch(n)
            {
                case 0: return lhs;
        ...
        XSIMD_INLINE batch<int16_t, 8> shift_right(const batch<int16_t, 8>& lhs, int32_t n)
        {
            switch(n)
            {
                case 0: return lhs;
    ...
    XSIMD_INLINE batch<int16_t, 8> operator<<(const batch<int16_t, 8>& lhs, int32_t rhs)
    {
    ...
    XSIMD_INLINE batch<int16_t, 8> operator<<(const batch<int16_t, 8>& lhs, const batch<int16_t, 8>& rhs)
    {
