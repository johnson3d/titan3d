...
namespace xsimd
{
    ...
    template <class X>
    typename simd_batch_traits<X>::batch_bool_type
    xsimd_isnan(const simd_base<X>& x);
    ...
    struct batch_cast_impl
    {
        ...
        static XSIMD_INLINE batch<T_out, N> run_impl(const batch<T_in, N>& x, detail::index_sequence<I...>)
        {
            return batch<T_out, N>(static_cast<T_out>(x[I])...);
        }

    public:
        static XSIMD_INLINE batch<T_out, N> run(const batch<T_in, N>& x)
        {
    ...
    {
        static XSIMD_INLINE batch<T, N> run(const batch<T, N>& x)
        {
            return x;
        }
    };
    ...
    #define XSIMD_BITWISE_CAST_INTRINSIC(T_IN, N_IN, T_OUT, N_OUT, INTRINSIC)  \
        ...
        {                                                                      \
            static XSIMD_INLINE batch<T_OUT, N_OUT> run(const batch<T_IN, N_IN>& x)  \
            {                                                                  \
                return INTRINSIC(x);                                           \
            }                                                                  \
        };
...
#define XSIMD_DEFINE_LOAD_STORE(TYPE, N, CVT_TYPE, ALIGNMENT)                  \
    XSIMD_INLINE batch<TYPE, N>& batch<TYPE, N>::load_aligned(const CVT_TYPE* src)   \
    {                                                                          \
    ...
    XSIMD_INLINE batch<TYPE, N>& batch<TYPE, N>::load_unaligned(const CVT_TYPE* src) \
    {                                                                          \
        return load_aligned(src);                                              \
    }                                                                          \
    XSIMD_INLINE void batch<TYPE, N>::store_aligned(CVT_TYPE* dst) const             \
    {                                                                          \
        alignas(ALIGNMENT) TYPE tmp[N];                                        \
        store_aligned(tmp);                                                    \
        unroller<N>([&](std::size_t i) {                                       \
    ...
    XSIMD_INLINE void batch<TYPE, N>::store_unaligned(CVT_TYPE* dst) const           \
    {                                                                          \
...
#define XSIMD_DEFINE_BITWISE_CAST(TYPE, N)                                     \
    XSIMD_INLINE batch<TYPE, N> bitwise_cast(const batch_bool<TYPE, N>& src)         \
    {                                                                          \
        TYPE z(0);                                                             \
...
#define XSIMD_DEFINE_BITWISE_CAST_ALL(NMIN)                                    \
     ...
     */
    template <class X>
    XSIMD_INLINE auto simd_base<X>::operator()() -> batch_reference
    {
     ...
     */
    template <class X>
    XSIMD_INLINE auto simd_base<X>::operator()() const -> const_batch_reference
    {
        return derived_cast().get();
    }
     ...
     */
    template <class X>
    XSIMD_INLINE X& simd_base<X>::derived_cast()
    {
     ...
     */
    template <class X>
    XSIMD_INLINE const X& simd_base<X>::derived_cast() const
    {
    ...
    template <class X>
    XSIMD_INLINE simd_batch<X>::simd_batch(storage_type value)
        : m_value(value)
    {
    }
     ...
     */
    template <class X>
    XSIMD_INLINE X& simd_batch<X>::operator+=(const X& rhs)
    {
        (*this)() = (*this)() + rhs;
        return (*this)();
    }
     ...
     */
    template <class X>
    XSIMD_INLINE X& simd_batch<X>::operator+=(const value_type& rhs)
    {
        (*this)() = (*this)() + X(rhs);
        return (*this)();
    }
     ...
     */
    template <class X>
    XSIMD_INLINE X& simd_batch<X>::operator++()
    {
        (*this)() += value_type(1);
        return (*this)();
    }
     ...
     */
    template <class X>
    XSIMD_INLINE X& simd_batch<X>::operator++(int)
    {
        X tmp = (*this)();
        (*this)() += value_type(1);
        return tmp;
    ...
    template <class X>
    XSIMD_INLINE X& simd_batch<X>::load_aligned(const char* src)
    {
        return (*this)().load_aligned(reinterpret_cast<const char_itype*>(src));
    }

    template <class X>
    XSIMD_INLINE X& simd_batch<X>::load_unaligned(const char* src)
    {
    ...
    template <class X>
    XSIMD_INLINE auto simd_batch<X>::get() -> batch_reference
    {
        return this->derived_cast();
    }

    template <class X>
    XSIMD_INLINE auto simd_batch<X>::get() const -> const_batch_reference
    {
        return this->derived_cast();
    }
    
    template <class X>
    XSIMD_INLINE auto simd_batch<X>::operator[](std::size_t index) -> value_type&
    {
        return m_array[index & (size - 1)];
    }

    template <class X>
    XSIMD_INLINE auto simd_batch<X>::operator[](std::size_t index) const -> const value_type&
    {
        return m_array[index & (size - 1)];
    }

    template <class X>
    XSIMD_INLINE auto simd_batch<X>::begin() -> iterator
    {
        return m_array;
    }

    template <class X>
    XSIMD_INLINE auto simd_batch<X>::end() -> iterator
    {
        return m_array + size;
    }

    template <class X>
    XSIMD_INLINE auto simd_batch<X>::begin() const -> const_iterator
    {
        return cbegin();
    }

    template <class X>
    XSIMD_INLINE auto simd_batch<X>::end() const -> const_iterator
    {
        return cend();
    }

    template <class X>
    XSIMD_INLINE auto simd_batch<X>::cbegin() const -> const_iterator
    {
        return m_array;
    }

    template <class X>
    XSIMD_INLINE auto simd_batch<X>::cend() const -> const_iterator
    {
        return m_array + size;
    }

    template <class X>
    XSIMD_INLINE auto simd_batch<X>::rbegin() -> reverse_iterator
    {
        return reverse_iterator(end());
    }

    template <class X>
    XSIMD_INLINE auto simd_batch<X>::rend() -> reverse_iterator
    {
        return reverse_iterator(begin());
    }

    template <class X>
    XSIMD_INLINE auto simd_batch<X>::rbegin() const -> const_reverse_iterator
    {
        return crbegin();
    }

    template <class X>
    XSIMD_INLINE auto simd_batch<X>::rend() const -> const_reverse_iterator
    {
        return crend();
    }

    template <class X>
    XSIMD_INLINE auto simd_batch<X>::crbegin() const -> const_reverse_iterator
    {
        return const_reverse_iterator(end());
    }

    template <class X>
    XSIMD_INLINE auto simd_batch<X>::crend() const -> const_reverse_iterator
    {
...
#define XSIMD_UNARY_OP(OP, FUNC)                                                                   \
    template <class X>                                                                             \
    XSIMD_INLINE batch_type_t<X> operator OP(const simd_base<X>& rhs)                                    \
    {                                                                                              \
        using value_type = typename simd_batch_traits<X>::value_type;                              \
...
#define XSIMD_BINARY_OP(OP, FUNC)                                                                  \
    ...
    XSIMD_INLINE batch_type_t<X> operator OP(const simd_base<X>& lhs, const simd_base<Y>& rhs)           \
    {                                                                                              \
        using value_type = typename simd_batch_traits<X>::value_type;                              \
        using kernel = detail::batch_kernel<value_type, simd_batch_traits<X>::size>;               \
        return kernel::FUNC(lhs(), rhs());                                                         \
                                                                                                   ...
                                                                                                   \
    template <class X>                                                                             \
    XSIMD_INLINE batch_type_t<X> operator OP(const typename simd_batch_traits<X>::value_type& lhs,       \
    ...
    {                                                                                              \
        return batch_type_t<X>(lhs) OP rhs();                                                      \
                                                                                                   ...
                                                                                                   \
    template <class X>                                                                             \
    XSIMD_INLINE batch_type_t<X> operator OP(const simd_base<X>& lhs,                                    \
...
#define XSIMD_BINARY_BOOL_OP(OP, FUNC)                                                             \
    template <class X>                                                                             \
    XSIMD_INLINE typename simd_batch_traits<X>::batch_bool_type operator OP(const simd_base<X>& lhs,     \
                                                                      const simd_base<X>& rhs)     \
    {                                                                                              \
        using value_type = typename simd_batch_traits<X>::value_type;                              \
        using kernel = detail::batch_kernel<value_type, simd_batch_traits<X>::size>;               \
        return kernel::FUNC(lhs(), rhs());                                                         \
                                                                                                   ...
                                                                                                   \
    template <class X>                                                                             \
    XSIMD_INLINE typename simd_batch_traits<X>::batch_bool_type operator OP(                             \
...
#define XSIMD_BINARY_BOOL_OP_DERIVED(OP, BASE_OP)                                                  \
     ...
     */
    template <class X>
    XSIMD_INLINE batch_type_t<X> operator-(const simd_base<X>& rhs);
     ...
     */
    template <class X>
    XSIMD_INLINE X operator+(const simd_batch<X>& rhs)
    {
        return rhs();
    }
     ...
     */
    template <class X, class Y>
    XSIMD_INLINE batch_type_t<X> operator&(const simd_base<X>& lhs, const simd_base<Y>& rhs);
     ...
     */
    template <class X>
    XSIMD_INLINE batch_type_t<X> bitwise_andnot(const simd_batch<X>& lhs, const simd_batch<X>& rhs)
    {
        using value_type = typename simd_batch_traits<X>::value_type;                              \
        using kernel = detail::batch_kernel<value_type, simd_batch_traits<X>::size>;               \
     ...
     */
    template <class X>
    XSIMD_INLINE typename simd_batch_traits<X>::batch_bool_type
    operator!(const simd_base<X>& rhs)
    {
     ...
     */
    template <class X>
    XSIMD_INLINE batch_type_t<X> min(const simd_base<X>& lhs, const simd_base<X>& rhs)
    {
        using value_type = typename simd_batch_traits<X>::value_type;
        using kernel = detail::batch_kernel<value_type, simd_batch_traits<X>::size>;
     ...
     */
    template <class X>
    XSIMD_INLINE batch_type_t<X> max(const simd_base<X>& lhs, const simd_base<X>& rhs)
    {
        using value_type = typename simd_batch_traits<X>::value_type;
        using kernel = detail::batch_kernel<value_type, simd_batch_traits<X>::size>;
     ...
     */
    template <class X>
    XSIMD_INLINE batch_type_t<X> fmin(const simd_batch<X>& lhs, const simd_batch<X>& rhs)
    {
        using value_type = typename simd_batch_traits<X>::value_type;
        using kernel = detail::batch_kernel<value_type, simd_batch_traits<X>::size>;
     ...
     */
    template <class X>
    XSIMD_INLINE batch_type_t<X> fmax(const simd_batch<X>& lhs, const simd_batch<X>& rhs)
    {
        using value_type = typename simd_batch_traits<X>::value_type;
        using kernel = detail::batch_kernel<value_type, simd_batch_traits<X>::size>;
     ...
     */
    template <class X>
    XSIMD_INLINE real_batch_type_t<X> abs(const simd_base<X>& rhs)
    {
        using value_type = typename simd_batch_traits<X>::value_type;
        using kernel = detail::batch_kernel<value_type, simd_batch_traits<X>::size>;
     ...
     */
    template <class X>
    XSIMD_INLINE batch_type_t<X> fabs(const simd_base<X>& rhs)
    {
        using value_type = typename simd_batch_traits<X>::value_type;
        using kernel = detail::batch_kernel<value_type, simd_batch_traits<X>::size>;
     ...
     */
    template <class X>
    XSIMD_INLINE batch_type_t<X> sqrt(const simd_base<X>& rhs)
    {
        using value_type = typename simd_batch_traits<X>::value_type;
        using kernel = detail::batch_kernel<value_type, simd_batch_traits<X>::size>;
     ...
     */
    template <class X>
    XSIMD_INLINE batch_type_t<X> fma(const simd_base<X>& x, const simd_base<X>& y, const simd_base<X>& z)
    {
        using value_type = typename simd_batch_traits<X>::value_type;
        using kernel = detail::batch_kernel<value_type, simd_batch_traits<X>::size>;
     ...
     */
    template <class X>
    XSIMD_INLINE batch_type_t<X> fms(const simd_base<X>& x, const simd_base<X>& y, const simd_base<X>& z)
    {
        using value_type = typename simd_batch_traits<X>::value_type;
        using kernel = detail::batch_kernel<value_type, simd_batch_traits<X>::size>;
     ...
     */
    template <class X>
    XSIMD_INLINE batch_type_t<X> fnma(const simd_base<X>& x, const simd_base<X>& y, const simd_base<X>& z)
    {
        using value_type = typename simd_batch_traits<X>::value_type;
        using kernel = detail::batch_kernel<value_type, simd_batch_traits<X>::size>;
     ...
     */
    template <class X>
    XSIMD_INLINE batch_type_t<X> fnms(const simd_base<X>& x, const simd_base<X>& y, const simd_base<X>& z)
    {
        using value_type = typename simd_batch_traits<X>::value_type;
        using kernel = detail::batch_kernel<value_type, simd_batch_traits<X>::size>;
     ...
     */
    template <class X>
    XSIMD_INLINE typename simd_batch_traits<X>::value_type
    hadd(const simd_base<X>& rhs)
    {
        using value_type = typename simd_batch_traits<X>::value_type;
        using kernel = detail::batch_kernel<value_type, simd_batch_traits<X>::size>;
     ...
     */
    template <class X>
    XSIMD_INLINE batch_type_t<X> select(const typename simd_batch_traits<X>::batch_bool_type& cond, const simd_base<X>& a, const simd_base<X>& b)
    {
        using value_type = typename simd_batch_traits<X>::value_type;
        using kernel = detail::batch_kernel<value_type, simd_batch_traits<X>::size>;
     ...
     */
    template <class X>
    XSIMD_INLINE std::ostream& operator<<(std::ostream& out, const simd_batch<X>& rhs)
    {
        out << '(';
...
#define GENERIC_OPERATOR_IMPLEMENTATION(OP)        \
    ...
    template <class T, std::size_t N>
    XSIMD_INLINE batch<T, N> operator&&(const batch<T, N>& lhs, const batch<T, N>& rhs)
    {
    ...
    XSIMD_INLINE batch<T_out, N> batch_cast(const batch<T_in, N>& x)
    {
    ...
    XSIMD_INLINE B bitwise_cast(const batch<float, N>& x)
    {
        return bitwise_cast_impl<batch<float, N>, B>::run(x);
    }

    template <class B, std::size_t N>
    XSIMD_INLINE B bitwise_cast(const batch<double, N>& x)
    {
        return bitwise_cast_impl<batch<double, N>, B>::run(x);
    }

    template <class B, std::size_t N>
    XSIMD_INLINE B bitwise_cast(const batch<int32_t, N>& x)
    {
        return bitwise_cast_impl<batch<int32_t, N>, B>::run(x);
    }

    template <class B, std::size_t N>
    XSIMD_INLINE B bitwise_cast(const batch<int64_t, N>& x)
    {
        return bitwise_cast_impl<batch<int64_t, N>, B>::run(x);
    }

    template <class T, std::size_t N>
    XSIMD_INLINE batch<T, N> bitwise_cast(const batch_bool<T, N>& src)
    {
    ...
    template <class T, std::size_t N>
    XSIMD_INLINE batch<T, N> fma(const batch<T, N>& a, const batch<T, N>& b, const batch<T, N>& c)
    {
