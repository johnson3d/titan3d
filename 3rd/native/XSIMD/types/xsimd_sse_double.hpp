...
namespace xsimd
{
    ...
    XSIMD_INLINE batch_bool<double, 2>::batch_bool()
    {
    }

    XSIMD_INLINE batch_bool<double, 2>::batch_bool(bool b)
    {
        m_value = _mm_castsi128_pd(_mm_set1_epi32(-(int)b));
    }

    XSIMD_INLINE batch_bool<double, 2>::batch_bool(bool b0, bool b1)
    {
        m_value = _mm_castsi128_pd(_mm_setr_epi32(-(int)b0, -(int)b0, -(int)b1, -(int)b1));
    }

    XSIMD_INLINE batch_bool<double, 2>::batch_bool(const __m128d& rhs)
    {
        m_value = rhs;
    }

    XSIMD_INLINE batch_bool<double, 2>& batch_bool<double, 2>::operator=(const __m128d& rhs)
    {
        m_value = rhs;
        return *this;
    }
    ...
    XSIMD_INLINE batch_bool<double, 2>::operator __m128d() const
    {
        return m_value;
    }

    XSIMD_INLINE bool_proxy<double> batch_bool<double, 2>::operator[](std::size_t index)
    {
        return bool_proxy<double>(m_array[index & 1]);
    }

    XSIMD_INLINE bool batch_bool<double, 2>::operator[](std::size_t index) const
    {
        return static_cast<bool>(m_array[index & 1]);
    }

    XSIMD_INLINE __m128d batch_bool<double, 2>::get_value() const
    {
        return m_value;
    }

    XSIMD_INLINE batch_bool<double, 2>& batch_bool<double, 2>::load_values(bool b0, bool b1)
    {
        m_value = _mm_castsi128_pd(_mm_setr_epi32(-(int)b0, -(int)b0, -(int)b1, -(int)b1));
        return *this;
    }
    ...
    XSIMD_INLINE batch<double, 2>::batch(double d0, double d1)
        : base_type(_mm_setr_pd(d0, d1))
    {
    }

    XSIMD_INLINE batch<double, 2>::batch(const double* src)
        : base_type(_mm_loadu_pd(src))
    {
    }

    XSIMD_INLINE batch<double, 2>::batch(const double* src, aligned_mode)
        : base_type(_mm_load_pd(src))
    {
    }

    XSIMD_INLINE batch<double, 2>::batch(const double* src, unaligned_mode)
        : base_type(_mm_loadu_pd(src))
    {
    }

    XSIMD_INLINE batch<double, 2>::batch(const __m128d& rhs)
        : base_type(rhs)
    {
    }

    XSIMD_INLINE batch<double, 2>& batch<double, 2>::operator=(const __m128d& rhs)
    {
        this->m_value = rhs;
        return *this;
    }

    XSIMD_INLINE batch<double, 2>::operator __m128d() const
    {
        return this->m_value;
    ...
    XSIMD_INLINE batch<double, 2>& batch<double, 2>::load_aligned(const double* src)
    {
        this->m_value = _mm_load_pd(src);
        return *this;
    }

    XSIMD_INLINE batch<double, 2>& batch<double, 2>::load_unaligned(const double* src)
    {
