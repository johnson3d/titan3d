...
namespace xsimd
{
    ...
    XSIMD_INLINE batch<uint16_t, 8>::batch()
    {
    }

    XSIMD_INLINE batch<uint16_t, 8>::batch(uint16_t d)
        : base_type(vdupq_n_u16(d))
    {
    }

    template <class... Args, class>
    XSIMD_INLINE batch<uint16_t, 8>::batch(Args... args)
        : base_type(storage_type{static_cast<uint16_t>(args)...})
    {
    }

    XSIMD_INLINE batch<uint16_t, 8>::batch(const uint16_t* d)
        : base_type(vld1q_u16(d))
    {
    }

    XSIMD_INLINE batch<uint16_t, 8>::batch(const uint16_t* d, aligned_mode)
        : batch(d)
    {
    }

    XSIMD_INLINE batch<uint16_t, 8>::batch(const uint16_t* d, unaligned_mode)
        : batch(d)
    {
    }

    XSIMD_INLINE batch<uint16_t, 8>::batch(const storage_type& rhs)
        : base_type(rhs)
    {
    }

    XSIMD_INLINE batch<uint16_t, 8>& batch<uint16_t, 8>::operator=(const storage_type& rhs)
    {
        this->m_value = rhs;
        return *this;
    }

    XSIMD_INLINE batch<uint16_t, 8>::batch(const batch_bool_type& rhs)
        : base_type(vandq_u16(rhs, batch(1)))
    {
    }

    XSIMD_INLINE batch<uint16_t, 8>& batch<uint16_t, 8>::operator=(const batch_bool_type& rhs)
    {
        this->m_value = vandq_u16(rhs, batch(1));
        return *this;
    }

    XSIMD_INLINE batch<uint16_t, 8>& batch<uint16_t, 8>::load_aligned(const int16_t* src)
    {
        this->m_value = vreinterpretq_u16_s16(vld1q_s16(src));
        return *this;
    }

    XSIMD_INLINE batch<uint16_t, 8>& batch<uint16_t, 8>::load_unaligned(const int16_t* src)
    {
        return load_aligned(src);
    }


    XSIMD_INLINE batch<uint16_t, 8>& batch<uint16_t, 8>::load_aligned(const uint16_t* src)
    {
        this->m_value = vld1q_u16(src);
        return *this;
    }

    XSIMD_INLINE batch<uint16_t, 8>& batch<uint16_t, 8>::load_unaligned(const uint16_t* src)
    {
        return load_aligned(src);
    }

    XSIMD_INLINE void batch<uint16_t, 8>::store_aligned(int16_t* dst) const
    {
        vst1q_s16(dst, vreinterpretq_s16_u16(this->m_value));
    }

    XSIMD_INLINE void batch<uint16_t, 8>::store_unaligned(int16_t* dst) const
    {
        store_aligned(dst);
    }

    XSIMD_INLINE void batch<uint16_t, 8>::store_aligned(uint16_t* dst) const
    {
        vst1q_u16(dst, this->m_value);
    }

    XSIMD_INLINE void batch<uint16_t, 8>::store_unaligned(uint16_t* dst) const
    {
        store_aligned(dst);
    }
    ...
    XSIMD_INLINE batch<uint16_t, 8>::operator uint16x8_t() const
    {
        return this->m_value;
    }
    ...
    namespace detail
    {
        XSIMD_INLINE batch<uint16_t, 8> shift_left(const batch<uint16_t, 8>& lhs, int32_t n)
        {
            switch(n)
            {
                case 0: return lhs;
        ...
        XSIMD_INLINE batch<uint16_t, 8> shift_right(const batch<uint16_t, 8>& lhs, int32_t n)
        {
            switch(n)
            {
                case 0: return lhs;
    ...
    XSIMD_INLINE batch<uint16_t, 8> operator<<(const batch<uint16_t, 8>& lhs, int32_t rhs)
    {
    ...
    XSIMD_INLINE batch<uint16_t, 8> operator<<(const batch<uint16_t, 8>& lhs, const batch<int16_t, 8>& rhs)
    {
