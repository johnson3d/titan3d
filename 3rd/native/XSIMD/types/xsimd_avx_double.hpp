...
namespace xsimd
{
    ...
    XSIMD_INLINE batch_bool<double, 4>::batch_bool()
    {
    }

    XSIMD_INLINE batch_bool<double, 4>::batch_bool(bool b)
    {
        m_value = _mm256_castsi256_pd(_mm256_set1_epi32(-(int)b));
    }

    XSIMD_INLINE batch_bool<double, 4>::batch_bool(bool b0, bool b1, bool b2, bool b3)
    {
    ...
    XSIMD_INLINE batch_bool<double, 4>::batch_bool(const __m256d& rhs)
    {
        m_value = rhs;
    }

    XSIMD_INLINE batch_bool<double, 4>& batch_bool<double, 4>::operator=(const __m256d& rhs)
    {
        m_value = rhs;
        return *this;
    }

    XSIMD_INLINE batch_bool<double, 4>::operator __m256d() const
    {
        return m_value;
    }

    XSIMD_INLINE bool_proxy<double> batch_bool<double, 4>::operator[](std::size_t index)
    {
        return bool_proxy<double>(m_array[index & 3]);
    }

    XSIMD_INLINE bool batch_bool<double, 4>::operator[](std::size_t index) const
    {
        return static_cast<bool>(m_array[index & 3]);
    }

    XSIMD_INLINE __m256d batch_bool<double, 4>::get_value() const
    {
        return m_value;
    }

    XSIMD_INLINE batch_bool<double, 4>& batch_bool<double, 4>::load_values(bool b0, bool b1, bool b2, bool b3)
    {
        m_value = _mm256_castsi256_pd(
                    _mm256_setr_epi32(-(int)b0, -(int)b0, -(int)b1, -(int)b1,
                                      -(int)b2, -(int)b2, -(int)b3, -(int)b3));
    ...
    XSIMD_INLINE batch<double, 4>::batch()
    {
    }

    XSIMD_INLINE batch<double, 4>::batch(double d)
        : base_type(_mm256_set1_pd(d))
    {
    }

    XSIMD_INLINE batch<double, 4>::batch(double d0, double d1, double d2, double d3)
        : base_type(_mm256_setr_pd(d0, d1, d2, d3))
    {
    }

    XSIMD_INLINE batch<double, 4>::batch(const double* src)
        : base_type(_mm256_loadu_pd(src))
    {
    }

    XSIMD_INLINE batch<double, 4>::batch(const double* src, aligned_mode)
        : base_type(_mm256_load_pd(src))
    {
    }

    XSIMD_INLINE batch<double, 4>::batch(const double* src, unaligned_mode)
        : base_type(_mm256_loadu_pd(src))
    {
    }

    XSIMD_INLINE batch<double, 4>::batch(const __m256d& rhs)
        : base_type(rhs)
    {
    }

    XSIMD_INLINE batch<double, 4>& batch<double, 4>::operator=(const __m256d& rhs)
    {
        this->m_value = rhs;
        return *this;
    }

    XSIMD_INLINE batch<double, 4>::batch(const batch_bool_type& rhs)
        : base_type(_mm256_and_pd(rhs, batch(1.)))
    {
    }

    XSIMD_INLINE batch<double, 4>& batch<double, 4>::operator=(const batch_bool_type& rhs)
    {
        this->m_value = _mm256_and_pd(rhs, batch(1.));
        return *this;
    }

    XSIMD_INLINE batch<double, 4>::operator __m256d() const
    {
        return this->m_value;
    }
    ...
    XSIMD_INLINE batch<double, 4>& batch<double, 4>::load_aligned(const int8_t* src)
    {
    ...
    XSIMD_INLINE batch<double, 4>& batch<double, 4>::load_unaligned(const int8_t* src)
    {
        return load_aligned(src);
    }

    XSIMD_INLINE batch<double, 4>& batch<double, 4>::load_aligned(const uint8_t* src)
    {
        __m128i tmp = _mm_loadl_epi64((const __m128i*)src);
    ...
    XSIMD_INLINE batch<double, 4>& batch<double, 4>::load_unaligned(const uint8_t* src)
    {
        return load_aligned(src);
    }

    XSIMD_INLINE batch<double, 4>& batch<double, 4>::load_aligned(const int16_t* src)
    {
        __m128i tmp = _mm_loadl_epi64((const __m128i*)src);
    ...
    XSIMD_INLINE batch<double, 4>& batch<double, 4>::load_unaligned(const int16_t* src)
    {
        return load_aligned(src);
    }

    XSIMD_INLINE batch<double, 4>& batch<double, 4>::load_aligned(const uint16_t* src)
    {
        __m128i tmp = _mm_loadl_epi64((const __m128i*)src);
    ...
    XSIMD_INLINE batch<double, 4>& batch<double, 4>::load_unaligned(const uint16_t* src)
    {
        return load_aligned(src);
    }

    XSIMD_INLINE batch<double, 4>& batch<double, 4>::load_aligned(const int32_t* src)
    {
        this->m_value = _mm256_cvtepi32_pd(_mm_load_si128((__m128i const*)src));
        return *this;
    }

    XSIMD_INLINE batch<double, 4>& batch<double, 4>::load_unaligned(const int32_t* src)
    {
    ...
    XSIMD_INLINE batch<double, 4>& batch<double, 4>::load_aligned(const float* src)
    {
        this->m_value = _mm256_cvtps_pd(_mm_load_ps(src));
        return *this;
    }

    XSIMD_INLINE batch<double, 4>& batch<double, 4>::load_unaligned(const float* src)
    {
        this->m_value = _mm256_cvtps_pd(_mm_loadu_ps(src));
        return *this;
    }

    XSIMD_INLINE batch<double, 4>& batch<double, 4>::load_aligned(const double* src)
    {
        this->m_value = _mm256_load_pd(src);
        return *this;
    }

    XSIMD_INLINE batch<double, 4>& batch<double, 4>::load_unaligned(const double* src)
    {
        this->m_value = _mm256_loadu_pd(src);
        return *this;
    }

    XSIMD_INLINE void batch<double, 4>::store_aligned(int8_t* dst) const
    {
    ...
    XSIMD_INLINE void batch<double, 4>::store_unaligned(int8_t* dst) const
    {
        store_aligned(dst);
    }

    XSIMD_INLINE void batch<double, 4>::store_aligned(uint8_t* dst) const
    {
        __m128i tmp = _mm256_cvtpd_epi32(this->m_value);
        __m128i tmp1 = _mm_packs_epi32(tmp, _mm_set1_epi32(0));
    ...
    XSIMD_INLINE void batch<double, 4>::store_unaligned(uint8_t* dst) const
    {
        store_aligned(dst);
    }

    XSIMD_INLINE void batch<double, 4>::store_aligned(int16_t* dst) const
    {
        __m128i tmp = _mm256_cvtpd_epi32(this->m_value);
        __m128i tmp1 = _mm_packs_epi32(tmp, _mm_set1_epi32(0));
    ...
    XSIMD_INLINE void batch<double, 4>::store_unaligned(int16_t* dst) const
    {
        store_aligned(dst);
    }

    XSIMD_INLINE void batch<double, 4>::store_aligned(uint16_t* dst) const
    {
        __m128i tmp = _mm256_cvtpd_epi32(this->m_value);
        __m128i tmp1 = _mm_packs_epi32(tmp, _mm_set1_epi32(0));
        _mm_storel_epi64((__m128i*)dst, tmp1);
    ...
    XSIMD_INLINE void batch<double, 4>::store_unaligned(uint16_t* dst) const
    {
        store_aligned(dst);
    }

    XSIMD_INLINE void batch<double, 4>::store_aligned(int32_t* dst) const
    {
        _mm_store_si128((__m128i*)dst, _mm256_cvtpd_epi32(this->m_value));
    }

    XSIMD_INLINE void batch<double, 4>::store_unaligned(int32_t* dst) const
    {
        _mm_storeu_si128((__m128i*)dst, _mm256_cvtpd_epi32(this->m_value));
    }

    XSIMD_INLINE void batch<double, 4>::store_aligned(float* dst) const
    {
        _mm_store_ps(dst, _mm256_cvtpd_ps(this->m_value));
    }

    XSIMD_INLINE void batch<double, 4>::store_unaligned(float* dst) const
    {
        _mm_storeu_ps(dst, _mm256_cvtpd_ps(this->m_value));
    }

    XSIMD_INLINE void batch<double, 4>::store_aligned(double* dst) const
    {
        _mm256_store_pd(dst, this->m_value);
    }

    XSIMD_INLINE void batch<double, 4>::store_unaligned(double* dst) const
    {
