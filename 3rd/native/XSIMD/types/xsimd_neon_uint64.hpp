...
namespace xsimd
{
    ...
    *************************************/
    ...
    XSIMD_INLINE batch<uint64_t, 2>::batch()
    {
    }

    XSIMD_INLINE batch<uint64_t, 2>::batch(uint64_t src)
        : base_type(vdupq_n_u64(src))
    {
    }

    template <class... Args, class>
    XSIMD_INLINE batch<uint64_t, 2>::batch(Args... args)
        : base_type(storage_type{static_cast<uint64_t>(args)...})
    {
    }

    XSIMD_INLINE batch<uint64_t, 2>::batch(const uint64_t* src)
        : base_type(vld1q_u64(src))
    {
    }

    XSIMD_INLINE batch<uint64_t, 2>::batch(const uint64_t* src, aligned_mode)
        : batch(src)
    {
    }

    XSIMD_INLINE batch<uint64_t, 2>::batch(const uint64_t* src, unaligned_mode)
        : batch(src)
    {
    }

    XSIMD_INLINE batch<uint64_t, 2>::batch(const storage_type& rhs)
        : base_type(rhs)
    {
    }

    XSIMD_INLINE batch<uint64_t, 2>& batch<uint64_t, 2>::operator=(const storage_type& rhs)
    {
        this->m_value = rhs;
        return *this;
    }

    XSIMD_INLINE batch<uint64_t, 2>::batch(const batch_bool_type& rhs)
        : base_type(vandq_u64(rhs, batch(1)))
    {
    }

    XSIMD_INLINE batch<uint64_t, 2>& batch<uint64_t, 2>::operator=(const batch_bool_type& rhs)
    {
    ...
    XSIMD_INLINE batch<uint64_t, 2>& batch<uint64_t, 2>::load_aligned(const int32_t* src)
    {
    ...
    XSIMD_INLINE batch<uint64_t, 2>& batch<uint64_t, 2>::load_unaligned(const int32_t* src)
    {
        return load_aligned(src);
    }

    XSIMD_INLINE batch<uint64_t, 2>& batch<uint64_t, 2>::load_aligned(const uint32_t* src)
    {
    ...
    XSIMD_INLINE batch<uint64_t, 2>& batch<uint64_t, 2>::load_unaligned(const uint32_t* src)
    {
        return load_aligned(src);
    }

    XSIMD_INLINE batch<uint64_t, 2>& batch<uint64_t, 2>::load_aligned(const int64_t* src)
    {
        this->m_value = vreinterpretq_u64_s64(vld1q_s64(src));
        return *this;
    }

    XSIMD_INLINE batch<uint64_t, 2>& batch<uint64_t, 2>::load_unaligned(const int64_t* src)
    {
        return load_aligned(src);
    }

    XSIMD_INLINE batch<uint64_t, 2>& batch<uint64_t, 2>::load_aligned(const uint64_t* src)
    {
        this->m_value = vld1q_u64(src);
        return *this;
    }

    XSIMD_INLINE batch<uint64_t, 2>& batch<uint64_t, 2>::load_unaligned(const uint64_t* src)
    {
        return load_aligned(src);
    }
    ...
    XSIMD_INLINE batch<uint64_t, 2>& batch<uint64_t, 2>::load_aligned(const float* src)
    {
    ...
    XSIMD_INLINE batch<uint64_t, 2>& batch<uint64_t, 2>::load_unaligned(const float* src)
    {
        return load_aligned(src);
    }

    XSIMD_INLINE batch<uint64_t, 2>& batch<uint64_t, 2>::load_aligned(const double* src)
    {
    #if XSIMD_ARM_INSTR_SET >= XSIMD_ARM8_64_NEON_VERSION
    ...
    XSIMD_INLINE batch<uint64_t, 2>& batch<uint64_t, 2>::load_unaligned(const double* src)
    {
        return load_aligned(src);
    }

    XSIMD_INLINE void batch<uint64_t, 2>::store_aligned(int32_t* dst) const
    {
        int32x2_t tmp = vmovn_s64(vreinterpretq_s64_u64(this->m_value));
        vst1_s32((int32_t*)dst, tmp);
    }

    XSIMD_INLINE void batch<uint64_t, 2>::store_unaligned(int32_t* dst) const
    {
        store_aligned(dst);
    }

    XSIMD_INLINE void batch<uint64_t, 2>::store_aligned(uint32_t* dst) const
    {
        uint32x2_t tmp = vmovn_u64(this->m_value);
        vst1_u32((uint32_t*)dst, tmp);
    }

    XSIMD_INLINE void batch<uint64_t, 2>::store_unaligned(uint32_t* dst) const
    {
        store_aligned(dst);
    }

    XSIMD_INLINE void batch<uint64_t, 2>::store_aligned(int64_t* dst) const
    {
        vst1q_s64(dst, vreinterpretq_s64_u64(this->m_value));
    }

    XSIMD_INLINE void batch<uint64_t, 2>::store_unaligned(int64_t* dst) const
    {
        store_aligned(dst);
    }

    XSIMD_INLINE void batch<uint64_t, 2>::store_aligned(uint64_t* dst) const
    {
        vst1q_u64(dst, this->m_value);
    }

    XSIMD_INLINE void batch<uint64_t, 2>::store_unaligned(uint64_t* dst) const
    {
        store_aligned(dst);
    }

    XSIMD_INLINE void batch<uint64_t, 2>::store_aligned(float* dst) const
    {
    #if XSIMD_ARM_INSTR_SET >= XSIMD_ARM8_64_NEON_VERSION
    ...
    XSIMD_INLINE void batch<uint64_t, 2>::store_unaligned(float* dst) const
    {
        store_aligned(dst);
    }

    XSIMD_INLINE void batch<uint64_t, 2>::store_aligned(double* dst) const
    {
    #if XSIMD_ARM_INSTR_SET >= XSIMD_ARM8_64_NEON_VERSION
    ...
    XSIMD_INLINE void batch<uint64_t, 2>::store_unaligned(double* dst) const
    {
        store_aligned(dst);
    }

    XSIMD_INLINE batch<uint64_t, 2>::operator storage_type() const
    {
        return this->m_value;
    }
    ...
    namespace detail
    {
        ...
        XSIMD_INLINE batch<uint64_t, 2> shift_left(const batch<uint64_t, 2>& lhs, int32_t n)
        {
            switch(n)
            {
                case 0: return lhs;
        ...
        XSIMD_INLINE batch<uint64_t, 2> shift_right(const batch<uint64_t, 2>& lhs, int32_t n)
        {
            switch(n)
            {
                case 0: return lhs;
    ...
    XSIMD_INLINE batch<uint64_t, 2> operator<<(const batch<uint64_t, 2>& lhs, int32_t rhs)
    {
    ...
    XSIMD_INLINE batch<uint64_t, 2> operator<<(const batch<uint64_t, 2>& lhs, const batch<int64_t, 2>& rhs)
    {
