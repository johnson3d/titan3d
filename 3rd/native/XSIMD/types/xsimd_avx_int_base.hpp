...
#define XSIMD_APPLY_SSE_FUNCTION(func, avx_lhs, avx_rhs)     \
    ...
    {
        template <class... Args>
        XSIMD_INLINE __m256i int_init(std::integral_constant<std::size_t, 1>, Args... args)
        {
            return _mm256_setr_epi8(args...);
        }

        template <class... Args>
        XSIMD_INLINE __m256i int_init(std::integral_constant<std::size_t, 2>, Args... args)
        {
            return _mm256_setr_epi16(args...);
        }

        template <class... Args>
        XSIMD_INLINE __m256i int_init(std::integral_constant<std::size_t, 4>, Args... args)
        {
            return _mm256_setr_epi32(args...);
        }

        template <class... Args>
        XSIMD_INLINE __m256i int_init(std::integral_constant<std::size_t, 8>, Args... args)
        {
            return _mm256_setr_epi64x(args...);
        }

        template <class T>
        XSIMD_INLINE __m256i int_set(std::integral_constant<std::size_t, 1>, T v)
        {
            return _mm256_set1_epi8(v);
        }

        template <class T>
        XSIMD_INLINE __m256i int_set(std::integral_constant<std::size_t, 2>, T v)
        {
            return _mm256_set1_epi16(v);
        }

        template <class T>
        XSIMD_INLINE __m256i int_set(std::integral_constant<std::size_t, 4>, T v)
        {
            return _mm256_set1_epi32(v);
        }

        template <class T>
        XSIMD_INLINE __m256i int_set(std::integral_constant<std::size_t, 8>, T v)
        {
    ...
    template <class T, std::size_t N>
    XSIMD_INLINE avx_int_batch_bool<T, N>::avx_int_batch_bool()
    {
    }

    template <class T, std::size_t N>
    XSIMD_INLINE avx_int_batch_bool<T, N>::avx_int_batch_bool(bool b)
    ...
    XSIMD_INLINE avx_int_batch_bool<T, N>::avx_int_batch_bool(Args... args)
    ...
    template <class T, std::size_t N>
    XSIMD_INLINE avx_int_batch_bool<T, N>::avx_int_batch_bool(const __m256i& rhs)
        : m_value(rhs)
    {
    }

    template <class T, std::size_t N>
    XSIMD_INLINE avx_int_batch_bool<T, N>& avx_int_batch_bool<T, N>::operator=(const __m256i& rhs)
    {
        m_value = rhs;
        return *this;
    }
    ...
    template <class T, std::size_t N>
    XSIMD_INLINE avx_int_batch_bool<T, N>::operator __m256i() const
    {
        return m_value;
    }

    template <class T, std::size_t N>
    XSIMD_INLINE bool_proxy<T> avx_int_batch_bool<T, N>::operator[](std::size_t index)
    {
        return bool_proxy<T>(m_array[index & (N - 1)]);
    }

    template <class T, std::size_t N>
    XSIMD_INLINE bool avx_int_batch_bool<T, N>::operator[](std::size_t index) const
    {
        return static_cast<bool>(m_array[index & (N - 1)]);
    }

    template <class T, std::size_t N>
    XSIMD_INLINE __m256i avx_int_batch_bool<T, N>::get_value() const
    {
        return m_value;
    }
    ...
    template <class T, std::size_t N>
    template <class... Args>
    XSIMD_INLINE batch_bool<T, N>& avx_int_batch_bool<T, N>::load_values(Args... args)
    {
        m_value = avx_detail::int_init(std::integral_constant<std::size_t, sizeof(T)>{},
                                       static_cast<T>(args ? typename std::make_signed<T>::type{-1} : 0)...);
        return (*this)();
    ...
    template <class T, std::size_t N>
    XSIMD_INLINE avx_int_batch<T, N>::avx_int_batch()
    {
    }

    template <class T, std::size_t N>
    XSIMD_INLINE avx_int_batch<T, N>::avx_int_batch(T i)
    ...
    template <class T, std::size_t N>
    template <class... Args, class>
    XSIMD_INLINE avx_int_batch<T, N>::avx_int_batch(Args... args)
        : base_type(avx_detail::int_init(std::integral_constant<std::size_t, sizeof(T)>{}, args...))
    {
    }

    template <class T, std::size_t N>
    XSIMD_INLINE avx_int_batch<T, N>::avx_int_batch(const T* src)
        : base_type(_mm256_loadu_si256((__m256i const*)src))
    {
    }

    template <class T, std::size_t N>
    XSIMD_INLINE avx_int_batch<T, N>::avx_int_batch(const T* src, aligned_mode)
        : base_type(_mm256_load_si256((__m256i const*)src))
    {
    }

    template <class T, std::size_t N>
    XSIMD_INLINE avx_int_batch<T, N>::avx_int_batch(const T* src, unaligned_mode)
        : base_type(_mm256_loadu_si256((__m256i const*)src))
    {
    }

    template <class T, std::size_t N>
    XSIMD_INLINE avx_int_batch<T, N>::avx_int_batch(const __m256i& rhs)
        : base_type(rhs)
    {
    }
    ...
    namespace detail
    {
        XSIMD_INLINE __m256i bitwise_and_impl(__m256i lhs, __m256i rhs)
        {
#if XSIMD_X86_INSTR_SET >= XSIMD_X86_AVX2_VERSION
            return _mm256_and_si256(lhs, rhs);
#else
    ...
    template <class T, std::size_t N>
    XSIMD_INLINE avx_int_batch<T, N>::avx_int_batch(const batch_bool_type& rhs)
        : base_type(detail::bitwise_and_impl(rhs, batch<T, N>(1)))
    {
    }

    template <class T, std::size_t N>
    XSIMD_INLINE batch<T, N>& avx_int_batch<T, N>::operator=(const __m256i& rhs)
    {
        this->m_value = rhs;
        return (*this)();
    }
    ...
    template <class T, std::size_t N>
    XSIMD_INLINE batch<T, N>& avx_int_batch<T, N>::operator=(const batch_bool_type& rhs)
    {
    ...
    template <class T, std::size_t N>
    XSIMD_INLINE avx_int_batch<T, N>::operator __m256i() const
    {
        return this->m_value;
    }

    template <class T, std::size_t N>
    XSIMD_INLINE batch<T, N>& avx_int_batch<T, N>::load_aligned(const T* src)
    {
    ...
    template <class T, std::size_t N>
    XSIMD_INLINE batch<T, N>& avx_int_batch<T, N>::load_unaligned(const T* src)
    {
    ...
    template <class T, std::size_t N>
    XSIMD_INLINE batch<T, N>& avx_int_batch<T, N>::load_aligned(const flipped_sign_type_t<T>* src)
    {
        this->m_value = _mm256_load_si256((__m256i const*) src);
        return (*this)();
    }
    ...
    template <class T, std::size_t N>
    XSIMD_INLINE batch<T, N>& avx_int_batch<T, N>::load_unaligned(const flipped_sign_type_t<T>* src)
    {
        this->m_value = _mm256_loadu_si256((__m256i const*) src);
        return (*this)();
    }
    ...
    template <class T, std::size_t N>
    XSIMD_INLINE void avx_int_batch<T, N>::store_aligned(T* dst) const
    {
        _mm256_store_si256((__m256i*) dst, this->m_value);
    }

    template <class T, std::size_t N>
    XSIMD_INLINE void avx_int_batch<T, N>::store_unaligned(T* dst) const
    {
        _mm256_storeu_si256((__m256i*) dst, this->m_value);
    }

    template <class T, std::size_t N>
    XSIMD_INLINE void avx_int_batch<T, N>::store_aligned(flipped_sign_type_t<T>* dst) const
    {
        _mm256_store_si256((__m256i*) dst, this->m_value);
    }

    template <class T, std::size_t N>
    XSIMD_INLINE void avx_int_batch<T, N>::store_unaligned(flipped_sign_type_t<T>* dst) const
    {
        _mm256_storeu_si256((__m256i*) dst, this->m_value);
    }
