...
namespace xsimd
{
    ...
    {
        XSIMD_INLINE __m128i load_aligned_int64(const int8_t* src)
        {
        ...
        XSIMD_INLINE __m128i load_aligned_int64(const uint8_t* src)
        {
            __m128i tmp = _mm_loadl_epi64((const __m128i*)src);
#if XSIMD_X86_INSTR_SET >= XSIMD_X86_SSE4_1_VERSION
        ...
        XSIMD_INLINE __m128i load_aligned_int64(const int16_t* src)
        {
            __m128i tmp = _mm_loadl_epi64((const __m128i*)src);
#if XSIMD_X86_INSTR_SET >= XSIMD_X86_SSE4_1_VERSION
        ...
        XSIMD_INLINE __m128i load_aligned_int64(const uint16_t* src)
        {
            __m128i tmp = _mm_loadl_epi64((const __m128i*)src);
#if XSIMD_X86_INSTR_SET >= XSIMD_X86_SSE4_1_VERSION
        ...
        XSIMD_INLINE void store_aligned_int64(__m128i src, int8_t* dst)
        {
        ...
        XSIMD_INLINE void store_aligned_int64(__m128i src, uint8_t* dst)
        {
            alignas(16) int64_t tmp[2];
            _mm_store_si128((__m128i*)tmp, src);
        ...
        XSIMD_INLINE void store_aligned_int64(__m128i src, int16_t* dst)
        {
            alignas(16) int64_t tmp[2];
            _mm_store_si128((__m128i*)tmp, src);
        ...
        XSIMD_INLINE void store_aligned_int64(__m128i src, uint16_t* dst)
        {
            alignas(16) int64_t tmp[2];
            _mm_store_si128((__m128i*)tmp, src);
...
#define SSE_DEFINE_LOAD_STORE_INT64(TYPE, CVT_TYPE)                            \
    XSIMD_INLINE batch<TYPE, 2>& batch<TYPE, 2>::load_aligned(const CVT_TYPE* src)   \
    {                                                                          \
        this->m_value = sse_detail::load_aligned_int64(src);                   \
        return *this;                                                          \
    }                                                                          \
    XSIMD_INLINE batch<TYPE, 2>& batch<TYPE, 2>::load_unaligned(const CVT_TYPE* src) \
    {                                                                          \
        return load_aligned(src);                                              \
    }                                                                          \
    XSIMD_INLINE void batch<TYPE, 2>::store_aligned(CVT_TYPE* dst) const             \
    {                                                                          \
        sse_detail::store_aligned_int64(this->m_value, dst);                   \
    }                                                                          \
    XSIMD_INLINE void batch<TYPE, 2>::store_unaligned(CVT_TYPE* dst) const           \
    {                                                                          \
        store_aligned(dst);                                                    \
    }
