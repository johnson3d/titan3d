...
namespace xsimd
{
    ...
    namespace detail
    {
        ...
        struct identity { using type = T; };
            ...
            using value_type = T;
            static constexpr std::size_t size() XSIMD_NOEXCEPT { return sizeof...(Is); }
            };
...
#define XSIMD_MACRO_UNROLL_BINARY(FUNC)                                                                   \
    ...
    XSIMD_INLINE void unroller_impl(F&& f, detail::index_sequence<I...>)
    {
        static_cast<void>(std::initializer_list<int>{(f(I), 0)...});
    }

    template <std::size_t N, class F>
    XSIMD_INLINE void unroller(F&& f)
    {
