...
namespace EngineNS.GamePlay.Component
{
    ...
    [GamePlay.Component.CustomConstructionParamsAttribute(typeof(GLogicMacrossComponentInitializer), "逻辑动画状态机组件", "LogicMacross Component")]
    public class GLogicMacrossComponent : GComponentMacross
    {
        ...
        public override void OnBeginPlay(GPlacementComponent placement)
        {
            ...
            pose = CEngine.Instance.SkeletonAssetManager.GetSkeleton(CEngine.Instance.RenderContext, SkeletonAssetName).BoneTab.Clone();
            ...
            skinModifier.AnimationPoseProxy.Pose = pose;
        ...
        public override void Update(GPlacementComponent placement)
        {
            ...
            var skinModifier = mesh.SceneMesh.MdfQueue.FindModifier<EngineNS.Graphics.Mesh.CGfxSkinModifier>();
            if (curPose == null)
                curPose = CEngine.Instance.SkeletonAssetManager.GetSkeleton(CEngine.Instance.RenderContext, RName.GetRName(skinModifier.SkeletonAsset)).BoneTab.Clone();
            ...
            if (currentBestMotionPose != null && currentBestMotionPose.HashName == bestMotionPose.HashName)
            {
                if (currentBestMotionPose.PlayTime - bestMotionPose.PlayTime < 5)
                {
                    ...
                    var time = currentAnimTime % (motionField.Motions[currentAnimaHash].DurationInMillionSecond + 1);
            ...
            curPose.BlendWithTargetPose(skinModifier.AnimationPoseProxy.Pose, 0.5f);
            skinModifier.AnimationPoseProxy.Pose = curPose;
            if (!mIsDebug)
                return;
            DebugTick(placement);
        }
        ...
        {
            var seq = AnimationClip.CreateAnimationClipSync(name);
            seq.Bind(pose);
            return seq;
        }
