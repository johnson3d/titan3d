...
namespace EngineNS.GamePlay.Component
{
    ...
    public class GMacrossAnimationComponent : GComponent
    {
        ...
        {
            var cachedPose = new CGfxCachedAnimationPose(mPose.Clone());
        ...
        {
            mPose = EngineNS.CEngine.Instance.SkeletonAssetManager.GetSkeleton(EngineNS.CEngine.Instance.RenderContext, mSkeletonAssetName).CreateSkeletonPose();
        }

        protected Dictionary<RName, LogicAnimationStateMachine> mStateMachineList = new Dictionary<RName, LogicAnimationStateMachine>(new RName.EqualityComparer());
        public Dictionary<RName, LogicAnimationStateMachine> StateMachineList
        {
            get => mStateMachineList;
            private set => mStateMachineList = value;
        }
        public LogicAnimationStateMachine CreateStateMachine(string name)
        {
            LogicAnimationStateMachine stateMachine = new LogicAnimationStateMachine(name);
        ...
        {
            var machine = sender as LogicAnimationStateMachine;
            //machine.Pose.Shadowed();
        }
        ...
        public override void Tick(GPlacementComponent placement)
        {
            ...
            if (phy != null)
            {
            //    var displayment = mPose.ExtractRootMotion();
        ...
        public float GetAnimationSequenceRemainingTime(string stateMachineName, string stateName, string animName)
        {
            ...
            //    if (tickCom is LogicAnimationStateMachine)
            //    {
            //        var machine = tickCom as LogicAnimationStateMachine;
            //        if (machine.Name == stateMachineName.ToLower())
            //        {
            //            if (machine.CurrentState.Name == stateName.ToLower())
            //            {
