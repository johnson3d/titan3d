...
namespace EngineNS.GamePlay.Component
{
    ...
    [GamePlay.Component.CustomConstructionParamsAttribute(typeof(GSubSkinComponentInitializer), "模型组件", "Mesh", "SubSkinComponent")]
    [Editor.Editor_ComponentClassIconAttribute("icon/staticmesh_64x.txpic", RName.enRNameType.Editor)]
    public class GSubSkinComponent : GVisualComponent, IComponentHostSelectOperation,IPlaceable
    {
        [Rtti.MetaClass]
        public class GSubSkinComponentInitializer : GComponentInitializer
        {
            public GSubSkinComponent HostMeshComp;
            ...
            internal async System.Threading.Tasks.Task UpdateMtlInst()
            {
                ...
                if (mesh == null)
                    return;
                HostMeshComp.SetSceneMesh(CEngine.Instance.RenderContext, mesh);
            ...
            [Rtti.MetaClass]
            public class MtlInst : IO.Serializer.Serializer
            {
                ...
                public GSubSkinComponent MeshComp;
                private RName mName;
            ...
            private List<GSubSkinComponentInitializer.MtlInst> mMtlList = null;
            [Rtti.MetaData]
            public List<GSubSkinComponentInitializer.MtlInst> MtlList
            {
                get
                {
                    return mMtlList;
        ...
        public override bool Visible
        {
            get
            {
                var initializer = this.Initializer as GSubSkinComponentInitializer;
        ...
        public GSubSkinComponent()
        {
            this.Initializer = new GSubSkinComponentInitializer();
        ...
        public RName MeshName
        {
            get
            {
                var init = this.Initializer as GSubSkinComponentInitializer;
                if (init == null)
        ...
        public List<GSubSkinComponentInitializer.MtlInst> MtlList
        {
            get
            {
                var init = Initializer as GSubSkinComponentInitializer;
                if (init == null)
                    return null;
                return init.MtlList;
            }
        ...
        public void SetSceneMesh(CRenderContext RHICtx, Graphics.Mesh.CGfxMesh value)
        {
            mSceneMesh = value;

            if (mSceneMesh != null)
                RefreshSceneMeshData(RHICtx);

            if (value != null)
            {
                ((GSubSkinComponentInitializer)Initializer).MeshName = value.Name;
            }
            var hostMesh = mHostContainer as GMeshComponent;
            if (hostMesh != null)
            {
                var hostSkinModifier = hostMesh.SceneMesh.MdfQueue.FindModifier<EngineNS.Graphics.Mesh.CGfxSkinModifier>();
                var skinModifier = mSceneMesh.MdfQueue.FindModifier<EngineNS.Graphics.Mesh.CGfxSkinModifier>();
                if (hostSkinModifier != null && skinModifier != null && hostSkinModifier.AnimationPose.HashSkeleton== skinModifier.AnimationPose.HashSkeleton)
                {
                    skinModifier.CorrectAnimationPose(hostSkinModifier.AnimationPose, skinModifier.SkinSkeleton.CreateSkeletonPose());
                    skinModifier.CorrectAnimationPose(hostSkinModifier.MeshSpaceAnimPose, skinModifier.SkinSkeleton.CreateSkeletonPose());
                    skinModifier.AnimationPose = hostSkinModifier.AnimationPose;
                    skinModifier.MeshSpaceAnimPose = hostSkinModifier.MeshSpaceAnimPose;
                }
            }
        }
        ...
        private void UpdateSceneMeshForEditerMode(CCommandList cmd)
        {
            ...
            {
                EditerModeTask = mSceneMesh.UpdatedOrigionMesh(rc);
            }
            else
            {
                for (UInt32 i = 0; i < SceneMesh.MtlMeshArray.Length; i++)
        ...
        public static Profiler.TimeScope ScopeCommitMesh = Profiler.TimeScopeManager.GetTimeScope(typeof(GSubSkinComponent), nameof(GatherVisibleSceneMesh));
