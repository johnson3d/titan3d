...
namespace CSharpCode.UIEditor.Elements
{
    public class Columns : IContainer
    {
        ...
        {
            ImGuiAPI.Columns(NumOfCols, Name, CppBool.FromBoolean(Border));
            string b = Border ? "true" : "false";
            form.AddLine($"ImGuiAPI.Columns({NumOfCols}, \"{Name}\", CppBool.FromBoolean({b}));");
            form.PushBrackets();
        }
        public override void EndCode(Form form)
        {
            form.PopBrackets();
            form.AddLine($"ImGuiAPI.Columns(1, null, CppBool.FromBoolean(true));");
