...
namespace EngineNS.Bricks.ExcelTable
{
    ...
    public class ExcelImporter : ExcelBase
    {
        ...
        public List<T> Table2Objects<T>() where T : class, new()

        {
            try
            {
                ...
                for (int i = 1; i <= sheet.LastRowNum; i++)
                {
                    ...
                    var obj = Rtti.RttiHelper.CreateInstance(typeof(T)) as T; //new T();
        ...
        public List<object> Table2Objects2(Type objType)

        {
            try
            {
                ...
                for (int i = 1; i <= sheet.LastRowNum; i++)
                {
                    ...
                    var obj = Rtti.RttiHelper.CreateInstance(type);
                    foreach (var f in Binder)
                    {
                        if (f.Value.FieldInfo == null)
                            continue;
        ...
        public object Cell2Object(NPOI.SS.UserModel.ICell cell, System.Type type, out bool doSet)
        {
            ...
            else
            {
                ...
                {
                    return Vector2.TryParse(cell.ToString());
                }
                else if (type == typeof(Vector3))
                {
                    return Vector3.TryParse(cell.ToString());
                }
                else if (type == typeof(Vector4))
                {
                    return Vector4.TryParse(cell.ToString());
                }
                doSet = false;
            }
            return null;
