...
class InputGeom : public VIUnknown
{
public:
	enum AreaType
	{
...
public:
	RTTI_DEF(InputGeom, 0x068f74cd5b9a0b6d, true);
	InputGeom();
	~InputGeom();
	vBOOL LoadMesh(IRenderContext* rc, IGeometryMesh* mesh, float scale);

	bool loadMesh(rcContext* ctx, IRenderContext* rc, IGeometryMesh* mesh, float scale);
	bool IsInWalkArea(v3dVector3_t min, v3dVector3_t max);
	void CreateConvexVolumes(AreaType areatype, IBlobObject* blob, v3dxVector3 min, v3dxVector3 max);
	void DeleteConvexVolumesByArea(AreaType areatype);
