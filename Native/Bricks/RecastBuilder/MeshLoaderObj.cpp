...
bool rcMeshLoaderObj::load(IRenderContext* rc, IGeometryMesh* mesh, float scale)
{
	m_scale = scale;
	auto vb = mesh->GetVertexBuffer(VST_Position);
	IBlobObject posData;
	vb->GetBufferData(rc, &posData);
	v3dxVector3* pPos = (v3dxVector3*)posData.GetData();

	auto ib = mesh->GetIndexBuffer();
	IBlobObject indexData;
	ib->GetBufferData(rc, &indexData);
	
	int vcap = 0;
	auto vertCount = vb->mDesc.ByteWidth / sizeof(v3dxVector3);
	...
	if (ib->mDesc.Type == IBT_Int16)
	{
		...
		triCount = ib->mDesc.ByteWidth / sizeof(USHORT) / 3;
	...
	{
		triCount = ib->mDesc.ByteWidth / sizeof(int) / 3;
