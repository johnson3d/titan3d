...
RTTI_IMPL(EngineNS::InputGeom, EngineNS::VIUnknown);
...
vBOOL InputGeom::LoadMesh(IRenderContext* rc, IGeometryMesh* mesh, float scale)
{
	return (vBOOL)loadMesh(&GRCContext, rc, mesh, scale);
}
		
bool InputGeom::loadMesh(rcContext* ctx, IRenderContext* rc, IGeometryMesh* mesh, float scale)
{
	if (m_mesh)
	{
		delete m_chunkyMesh;
...
bool InputGeom::IsInWalkArea(v3dVector3_t min, v3dVector3_t max)
{
	if (WalkDatas.size() == 0)
		return false;
...
void InputGeom::CreateConvexVolumes(AreaType areatype, IBlobObject* blob, v3dxVector3 min, v3dxVector3 max)
{
	// Create
...
void InputGeom::DeleteConvexVolumesByArea(AreaType areatype)
{
