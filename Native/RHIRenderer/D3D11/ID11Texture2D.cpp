...
bool ID11Texture2D::Init(ID11RenderContext* rc, const ITexture2DDesc* desc)
{
	...
	if (desc->InitData.pSysMem != nullptr)
	{
		D3D11_SUBRESOURCE_DATA d11Desc;
		d11Desc.SysMemPitch = desc->InitData.SysMemPitch;
		d11Desc.SysMemSlicePitch = desc->InitData.SysMemSlicePitch;
		d11Desc.pSysMem = desc->InitData.pSysMem->GetData();
		rc->mDevice->CreateTexture2D(&Tex2dDesc, &d11Desc, &m_pDX11Texture2D);
	}
	else
	{
		rc->mDevice->CreateTexture2D(&Tex2dDesc, NULL, &m_pDX11Texture2D);
	}
	return true;
}
...
bool ID11Texture2D::InitD11Texture2D(ID3D11Texture2D* pTex2d)
{
	...
	pTex2d->GetDesc(&desc);
	memcpy(&mDesc, &desc, sizeof(D3D11_TEXTURE2D_DESC));
