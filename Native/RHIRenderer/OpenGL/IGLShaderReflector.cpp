...
void IGLShaderReflector::ReflectProgram(IGLRenderContext* rc, const std::shared_ptr<GLSdk::GLBufferId>& mProgram, EShaderType shaderType, ShaderReflector* pReflector)
{
	...
	for (GLuint cbIndex = 0; cbIndex < uboCount; cbIndex++)
	{
		...
		if (desc.Name.find("type_") == 0)
		{
			desc.Name = desc.Name.substr(5);
		}
		...
		//fuck opengl!
		//Ϊ�˴���struct����ȽϺõķ�ʽ�����Ƿ���ͬ��.xxx��ȫ���ϳ�һ��struct������ͳ�Ƴ���С��offset���ܹ���size
		//gmmͬѧ�����ң�2006���ѧ�ҷ��֣�gpu��ȡ���ݵ�ʱ��ṹ����ΪҪ�ֱ�ȡ�������ݣ���ȷǽṹ�����ܵ���ൽ30%
		//����shader����ԭ���ϲ�Ҫ��struct
		std::map<std::string, ConstantVarDesc*> structVars;
		for (auto iter = desc.Vars.begin(); iter != desc.Vars.end(); )
		{
			auto strPos = iter->Name.find_first_of(".");
			if (strPos != std::string::npos)
			{
				iter->Name = iter->Name.substr(0, strPos);
				...
				else
				{
					...
					stVar->Elements = 1;//��֧�ֽṹ�����飬gmm�����飡
					structVars.insert(std::make_pair(iter->Name, stVar));
				}
				iter = desc.Vars.erase(iter);
				continue;
			}
			strPos = iter->Name.find_first_of("[");
			if (strPos != std::string::npos)
			{
				iter->Name = iter->Name.substr(0, strPos);
			}
	...
	for (GLint uIdx = 0; uIdx < uniformCount; uIdx++)
	{
		...
		{//RWBuffer<uint>
			...
			else
			{
				...
				sampBindInfo.Name = "CS_" + sampBindInfo.Name;
