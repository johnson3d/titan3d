...
bool IGLTexture2D::Init(IGLRenderContext* pCtx, const ITexture2DDesc* pDesc)
{
	...
	IBlobObject* pixelData = mDesc.InitData.pSysMem;
	...
	if (pDesc->Format == PXF_D24_UNORM_S8_UINT || pDesc->Format == PXF_D32_FLOAT || pDesc->Format == PXF_D16_UNORM)
	{
		...
		// ��������base level max level��������shader�в���depth texture���Ƿ���0
	...
	else
	{
		...
		for (UINT i = 0; i < mDesc.MipLevels; i++)
		{
			...
			if (pixelData != nullptr)
				sdk->TexImage2D(GL_TEXTURE_2D, (GLint)i, internalFormat, width, height, 0, format, type, pixelData->GetData());
			else
