...
IGLShaderProgram::IGLShaderProgram()
{
	...
	mProgramReflector.UnsafeRef(new ShaderReflector());
}
...
vBOOL IGLShaderProgram::LinkShaders(IRenderContext* rc)
{
	...
	auto glvs = (IGLVertexShader*)mVertexShader;
	sdk->AttachShader(mProgram, glvs->mShader);

	auto glps = (IGLVertexShader*)mPixelShader;
	...
	sdk->DetachShader(mProgram, ((IGLVertexShader*)mVertexShader)->mShader);
	sdk->DetachShader(mProgram, ((IGLPixelShader*)mPixelShader)->mShader);
	...
	for (size_t i = 0; i < mInputLayout->mDesc.Layouts.size(); i++)
	{
		...
		std::string attribName = "in_var_" + elem.SemanticName;
...
bool IGLShaderProgram::GetShaderResourceBindInfo(UINT Index, TextureBindInfo* info) const
{
...
bool IGLShaderProgram::GetSamplerBindInfo(UINT Index, SamplerBindInfo* info) const
{
