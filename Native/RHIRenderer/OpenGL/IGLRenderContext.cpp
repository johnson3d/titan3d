...
IPass* IGLRenderContext::CreatePass()
{
	auto pass = new IGLPass();
	IPassDesc desc;
...
{
	auto mesh = new IGLGeometryMesh();
	if (mesh->Init(this) == false)
	{
		mesh->Release();
		return nullptr;
	}
...
{
	auto texture = new IGLShaderResourceView();
	if (false == texture->InitGLPointer(this, file, false))
	{
		texture->Release();
		return nullptr;
	}
