...
vBOOL GfxTextureStreaming::Init(IRenderContext* rc, IShaderResourceView* srv, const ITxPicDesc* txdesc, XNDNode* node)
{
	Cleanup();
	...
	{
		auto mipNode = node->GetChild("PngMips");
		if (mipNode == nullptr)
		{
		...
		mSrcNode.StrongRef(node);
		mCompressMode = PCM_None;
		SetStreamMipLevel((int)mipNode->GetAttribVector().size());
	}
	else
	{
		...
		auto mipNode = node->GetChild(tryCompressFormat);
		if (mipNode == nullptr)
		{
			mipNode = node->GetChild("PngMips");
			if (mipNode == nullptr)
			{
				if (node->Resource != nullptr)
				{
...
vBOOL GfxTextureStreaming::LoadAllMips(IRenderContext* rc, IShaderResourceView* srv)
{
	...
	for (int i = mLoadMaxMip; i < (int)mMipBuffers.size(); i++)
	{
		...
		auto attr = mMipNode->GetAttrib(name.c_str());
		if (attr == nullptr)
			return FALSE;

		attr->BeginRead(__FILE__, __LINE__);

		auto memLength = attr->GetLength();
...
vBOOL GfxTextureStreaming::LoadNextMip2(IRenderContext* rc, IShaderResourceView* srv)
{
	...
	auto attr = mSrcNode->GetAttrib(name.c_str());
	if (attr == nullptr)
		return FALSE;
...
vBOOL GfxTextureStreaming::LoadNextMip(IRenderContext* rc, IShaderResourceView* srv)
{
	...
	else if (mCompressMode == PCM_ETC2)
	{
		...
		attr->Read(layer);
		mMipBuffers[mLoadMaxMip].Width = layer.Width;
		mMipBuffers[mLoadMaxMip].Height = layer.Height;
...
extern "C"
{
	VFX_API void SDK_GfxTextureStreaming_TryCompressFormat(EPixelCompressMode compressMode)
	{
		GfxTextureStreaming::mTryCompressMode = compressMode;
	}
	CSharpReturnAPI0(int, EngineNS, GfxTextureStreaming, GetFullMipCount);
	CSharpReturnAPI0(int, EngineNS, GfxTextureStreaming, GetLoadedMipCount);
	CSharpReturnAPI2(vBOOL, EngineNS, GfxTextureStreaming, LoadAllMips, IRenderContext*, IShaderResourceView*);
	CSharpReturnAPI2(vBOOL, EngineNS, GfxTextureStreaming, LoadNextMip, IRenderContext*, IShaderResourceView*);
}