...
#include "vfxMemPoolObject.h"
	...
	extern vBOOL IsBadReadPtr(CONST VOID *lp, UINT_PTR ucb);
#endif
...
extern "C"
{
	VFX_API size_t SDK_vfxMemory_MemoryUsed()
	{
		return g_memoey_used;
	}
 
	VFX_API size_t SDK_vfxMemory_MemoryMax()
	{
		return g_memory_max; 
	} 

	VFX_API size_t SDK_vfxMemory_MemoryAllocTimes()
	{
		return g_alloc_times;
	}

	VFX_API void SDK_vfxMemory_SetMemAllocCallBack(FOnMemAlloc cb)
	{
		GOnMemAllocCallBack = cb;
	}
	
	VFX_API void SDK_vfxMemory_SetMemFreeCallBack(FOnMemFree cb)
	{
		GOnMemFreeCallBack = cb;
	}

	VFX_API void SDK_vfxMemory_SetMemLeakCallBack(FOnMemLeak cb)
	{
