...
public:
	...
	void Free(void * memory)
	{
		...
		try
		{
			...
			if (tmp->crch != 0xCCCCCCCC)	//前效验位有问题，很可能越界或错误的指针
			{
			...
			if (*(DWORD*)(tmp->obj + orisize) != 0xCCCCCCCC)//后效验位有问题，很可能越界或错误的指针
			{
				if (IsBadReadPtr(p->file, 4) == 0)
					_vfxTraceA("%s(%Id) : Verify pointer(0x%p) failed! Alloc ID : %Id\n"
						, p->file, p->line, p->data, p->id);
...
//注意，这个类的派生类不能被再次派生。否则，new 和 delete 将被混乱
