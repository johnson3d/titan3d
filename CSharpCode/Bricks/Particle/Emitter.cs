...
namespace EngineNS.Bricks.Particle
{
    ...
    public partial class TtEmitter : AuxPtrType<IEmitter> 
    {
        ...
        public unsafe void Update(TtNebulaParticle nebula, Graphics.Pipeline.TtRenderPolicy policy, TtParticleGraphNode particleSystem, float elapsed, Vector3 Location)
        {
            if (Mesh == null)
                return;
            ...
            else
            {
                ...
                {
                    (arg as TtEmitter).Flush2GPU();
                }, this);
                //Flush2GPU(particleSystem.CmdList.mCoreObject);
            }
        }
        public unsafe void UpdateCPU(TtNebulaParticle nebula, TtParticleGraphNode particleSystem, float elapsed)
        {
        ...
        public unsafe void UpdateGPU(NxRHI.TtCommandList cmdlist, TtParticleGraphNode particleSystem, float elapsed)
        {
        ...
        public uint LiveNumber { get; private set; } = 0;
        public unsafe void Flush2GPU()
        {
            LiveNumber = mCoreObject.GetLiveNumber();
            if (LiveNumber == 0)
                return;
            if (mGpuResources.ParticlesBuffer != null)
            {
                mGpuResources.ParticlesBuffer.GpuBuffer.UpdateGpuData(0, mCoreObject.GetParticleAddress(), MaxParticle * (uint)sizeof(FParticle));
            }
            if (mGpuResources.CurAlivesBuffer != null)
            {
                mGpuResources.CurAlivesBuffer.GpuBuffer.UpdateGpuData(4, mCoreObject.GetCurrentAliveAddress(), mCoreObject.GetLiveNumber() * (uint)sizeof(uint));//mCoreObject.GetLiveNumber()
            }
        }
