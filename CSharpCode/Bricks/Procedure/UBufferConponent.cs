...
namespace EngineNS.Bricks.Procedure
{
    ...
    public partial class UBufferComponent : IDisposable
    {
        ...
        public unsafe TtGpuBufferBase GetGpuBuffer<T>() where T : unmanaged
        {
            if (GpuBuffer == null)
            {
                GpuBuffer = new TtGpuBuffer<T>();
                GpuBuffer.SetSize((uint)(Width * Height * Depth), SuperPixels.DataPointer, NxRHI.EBufferType.BFT_SRV | NxRHI.EBufferType.BFT_UAV);
            }
            return GpuBuffer;
        }
        ...
        public unsafe TtGpuBufferBase GetGpuTexture2D<T>() where T : unmanaged
        {
            if (GpuBuffer == null)
            {
                GpuBuffer = new TtGpuBuffer<T>();
                GpuBuffer.SetTexture2D((uint)Width, (uint)Height, SuperPixels.DataPointer, NxRHI.EBufferType.BFT_SRV | NxRHI.EBufferType.BFT_UAV);
            }
            return GpuBuffer;
        }
        public unsafe void Upload2GpuTexture2D(NxRHI.TtCommandList cmd, EPixelFormat format)
        {
            ...
            footprint.Width = (uint)Width;
            footprint.Height = (uint)Height;
            footprint.Depth = 1;
            footprint.Format = format;
            footprint.RowPitch = (uint)(CoreSDK.GetPixelFormatByteWidth(format) * Width);
            footprint.TotalSize = footprint.RowPitch * footprint.Height;
	...
	partial class UBufferComponent
	{
		...
		public unsafe void macross_DispatchPixels (string nodeName, FOnPerPixel onPerPiexel, bool bMultThread) 
		{
			using(var stackframe = EngineNS.Macross.TtMacrossStackTracer.CurrentFrame)
			{
				if(stackframe != null)
				{
					stackframe.SetWatchVariable(nodeName + ":onPerPiexel", onPerPiexel);
					stackframe.SetWatchVariable(nodeName + ":bMultThread", bMultThread);
				}
			}
		...
		public unsafe void* macross_GetSuperPixelAddress (string nodeName, int x, int y, int z, System.Type retType) 
		{
			using(var stackframe = EngineNS.Macross.TtMacrossStackTracer.CurrentFrame)
			{
				if(stackframe != null)
				{
					...
					stackframe.SetWatchVariable(nodeName + ":y", y);
					stackframe.SetWatchVariable(nodeName + ":z", z);
					stackframe.SetWatchVariable(nodeName + ":retType", retType);
				}
			}
		...
		public unsafe float macross_Sampler2DFloat1 (string nodeName, in Vector3 uvw, EBufferSamplerType type, EPixelAddressMode address) 
		{
			using(var stackframe = EngineNS.Macross.TtMacrossStackTracer.CurrentFrame)
			{
				if(stackframe != null)
				{
					stackframe.SetWatchVariable(nodeName + ":uvw", uvw);
					stackframe.SetWatchVariable(nodeName + ":type", type);
					stackframe.SetWatchVariable(nodeName + ":address", address);
				}
			}
		...
		public unsafe float macross_Sampler2DFloat1 (string nodeName, float u, float v, float w, EBufferSamplerType type, EPixelAddressMode address) 
		{
			using(var stackframe = EngineNS.Macross.TtMacrossStackTracer.CurrentFrame)
			{
				if(stackframe != null)
				{
					stackframe.SetWatchVariable(nodeName + ":u", u);
