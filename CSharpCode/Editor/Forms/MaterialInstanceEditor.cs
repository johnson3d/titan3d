...
namespace EngineNS.Editor.Forms
{
    public class UMaterialInstanceEditor : TtLightEnvironemnt, Editor.IAssetEditor, IRootForm
    {
        ...
        protected async Thread.Async.TtTask<bool> Initialize_PreviewMaterialInstance(Graphics.Pipeline.TtViewportSlate viewport, TtSlateApplication application, Graphics.Pipeline.TtRenderPolicy policy, float zMin, float zMax)
        {
            ...
            if (ok)
            {
                var meshNode = await GamePlay.Scene.TtMeshNode.AddMeshNode(viewport.World, viewport.World.Root, new GamePlay.Scene.TtMeshNode.TtMeshNodeData(), typeof(GamePlay.TtPlacement), mesh,
            ...
            var studioContext = await PreviewViewport.CreateStudioEnvironment(aabb);
