...
namespace EngineNS
{
    ...
    public struct DRay : System.IEquatable<DRay>
    {
        /// <summary>
        /// 对象位置
        /// </summary>
        ...
        /// <param name="position">对象位置</param>
        /// <param name="direction">对象的方向</param>
        public DRay(in DVector3 position, in Vector3 direction)
        {
            Position = position;
            Direction = direction;
        }
        ...
        {
            ray.Direction.Normalize();
            double dotDirection = (plane.Normal.X * ray.Direction.X) + (plane.Normal.Y * ray.Direction.Y) + (plane.Normal.Z * ray.Direction.Z);
            ...
            var dotPosition = (plane.Normal.X * ray.Position.X) + (plane.Normal.Y * ray.Position.Y) + (plane.Normal.Z * ray.Position.Z);
            var num = (-plane.D - dotPosition) / dotDirection;
        ...
        public static bool Intersects(in DRay ray, in DVector3 vertex1, in DVector3 vertex2, in DVector3 vertex3, out double distance, out double barycentricU, out double barycentricV)
        {
            unsafe
            {
                fixed (double* pinnedDist = &distance)
                {
                    fixed (double* pinnedU = &barycentricU)
                    {
                        fixed (double* pinnedV = &barycentricV)
                        {
                            fixed (DRay* pinnedRay = &ray)
                            fixed (DVector3* pvertex1 = &vertex1)
                            fixed (DVector3* pvertex2 = &vertex2)
                            fixed (DVector3* pvertex3 = &vertex3)
                            {
                                var dir = pinnedRay->Direction.AsDVector();
                                if (IDllImportApi.v3dxIntersectDTri(pvertex1,
                                    pvertex2, pvertex3,
                                    &pinnedRay->Position, &dir,
                                    pinnedU, pinnedV, pinnedDist) != 0)
                                    return true;
                                else
                                    return false;
        ...
        public static bool Intersects(in DRay ray, in DBoundingBox box, out double distance)
        {
            ...
            {
                double inv = 1.0f / ray.Direction.X;
                double min = (box.Minimum.X - ray.Position.X) * inv;
                double max = (box.Maximum.X - ray.Position.X) * inv;
            ...
            {
                double inv = 1.0f / ray.Direction.Y;
                double min = (box.Minimum.Y - ray.Position.Y) * inv;
                double max = (box.Maximum.Y - ray.Position.Y) * inv;
            ...
            {
                double inv = 1.0f / ray.Direction.Z;
                double min = (box.Minimum.Z - ray.Position.Z) * inv;
                double max = (box.Maximum.Z - ray.Position.Z) * inv;
        ...
        public static bool Intersects(in DRay ray, in Aabb box, out double distance)
        {
            double d = 0.0f;
            double maxValue = double.MaxValue;
            ...
            {
                var inv = 1.0f / ray.Direction.X;
                var min = (box.Minimum.X - ray.Position.X) * inv;
                var max = (box.Maximum.X - ray.Position.X) * inv;
            ...
            {
                var inv = 1.0f / ray.Direction.Y;
                var min = (box.Minimum.Y - ray.Position.Y) * inv;
                var max = (box.Maximum.Y - ray.Position.Y) * inv;
            ...
            {
                var inv = 1.0f / ray.Direction.Z;
                var min = (box.Minimum.Z - ray.Position.Z) * inv;
                var max = (box.Maximum.Z - ray.Position.Z) * inv;
        ...
        public static bool Intersects(in Ray ray, in DBoundingSphere sphere, out double distance)
        {
            ...
            var y = sphere.Center.Y - ray.Position.Y;
            var z = sphere.Center.Z - ray.Position.Z;
            ...
            ray.Direction.Normalize();
            var dot = (x * ray.Direction.X) + (y * ray.Direction.Y) + (z * ray.Direction.Z);
            if (dot < 0.0f)
            {
                distance = 0.0f;
                return false;
            ...
            distance = dot - (double)(Math.Sqrt((double)(rr - temp)));
            return true;
        }
        /// <summary>
        /// 重载"=="操作符
        ...
        /// <param name="right">射线对象</param>
        /// <returns>如果两条射线相同返回true，否则返回false</returns>
        public static bool operator ==(in DRay left, in DRay right)
        {
            return left.Equals(right);
            //return Equals( left, right );
        }
        ...
        /// <param name="value2">射线对象</param>
        /// <returns>如果两条射线相同返回true，否则返回false</returns>
        public static bool Equals(in DRay value1, in DRay value2)
        {
            return (value1.Position == value2.Position && value1.Direction == value2.Direction);
        }
    }
