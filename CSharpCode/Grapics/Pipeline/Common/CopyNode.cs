...
namespace EngineNS.Graphics.Pipeline.Common
{
    ...
    {
        public TtRenderGraphPin SrcPinIn = TtRenderGraphPin.CreateInputOutput("Src", true, EPixelFormat.PXF_R16G16B16A16_FLOAT, NxRHI.EBufferType.BFT_SRV);
        ...
        public override void BeforeTick(TtRenderPolicy policy)
        {
            var srcBuffer = GetAttachBuffer(SrcPinIn);
            if (Current == null || srcBuffer.BufferDesc.Format != Current.BufferDesc.Format ||
                srcBuffer.BufferDesc.Width != Current.BufferDesc.Width ||
                srcBuffer.BufferDesc.Height != Current.BufferDesc.Height)
            {
                CoreSDK.DisposeObject(ref ResultBuffer[0]);
                CoreSDK.DisposeObject(ref ResultBuffer[1]);
                ...
                ResultBuffer[0].BufferDesc = srcBuffer.BufferDesc;
                ResultBuffer[0].CreateBufferViews(in ResultBuffer[0].BufferDesc);

                ResultBuffer[1].BufferDesc = srcBuffer.BufferDesc;
                ResultBuffer[1].CreateBufferViews(in ResultBuffer[0].BufferDesc);

                SrcPinIn.Attachement.BufferDesc.Format = Current.BufferDesc.Format;
                SrcPinIn.Attachement.BufferDesc.Width = Current.BufferDesc.Width;
                SrcPinIn.Attachement.BufferDesc.Height = Current.BufferDesc.Height;
            }
            base.BeforeTick(policy);
        }
        public override unsafe void Tick(GamePlay.TtWorld world, TtRenderPolicy policy, NxRHI.TtCommandList frameCmdList, bool bClear)
    ...
    [Bricks.CodeBuilder.ContextMenu("Copy", "Copy\\GetPrevFrame", Bricks.RenderPolicyEditor.TtPolicyGraph.RGDEditorKeyword)]
    [Rtti.Meta("")]
    public class TtGetPrevFrameNode : TAuxRenderGraphNode<TtGetPrevFrameNode>
    {
        ...
        public string PrevNodeName
        {
            ...
            {
                mPrevNodeName = value;
                mCopy2NextFrameNode = RenderGraph?.FindNode<TtCopy2NextFrameNode>(mPrevNodeName);
            }
        }
        TtAttachBuffer PrevOutAttachement = new TtAttachBuffer();
        public TtGetPrevFrameNode()
        {
            Name = "GetPrevFrameNode";
        }
        public override void InitNodePins()
        {
            PrevPinOut.LifeMode = TtAttachBuffer.ELifeMode.Imported;
        ...
        public override void FrameBuild(TtRenderPolicy policy)
        {
            ...
            if (mCopy2NextFrameNode == null)
            {
                PrevNodeName = PrevNodeName;
            }
            if (mCopy2NextFrameNode == null)
            {
                Profiler.Log.WriteLine<Profiler.TtGraphicsGategory>(Profiler.ELogTag.Error, $"TtGetPrevFrameNode: Copy2NextFrameNode '{PrevNodeName}' not found");
                return;
            }
            if (mCopy2NextFrameNode.Previos != null)
            {
                PrevOutAttachement.GpuResource = mCopy2NextFrameNode.Previos.GpuResource;
                PrevOutAttachement.Srv = mCopy2NextFrameNode.Previos.Srv;
                PrevOutAttachement.Rtv = mCopy2NextFrameNode.Previos.Rtv;
                PrevOutAttachement.Dsv = mCopy2NextFrameNode.Previos.Dsv;
                PrevOutAttachement.Uav = mCopy2NextFrameNode.Previos.Uav;
            }
            this.ImportAttachment(PrevPinOut, PrevOutAttachement);
            base.FrameBuild(policy);
        }
        public override void BeforeTick(TtRenderPolicy policy)
        {
            if (mCopy2NextFrameNode != null && mCopy2NextFrameNode.Previos != null)
            {
                PrevOutAttachement.GpuResource = mCopy2NextFrameNode.Previos.GpuResource;
                PrevOutAttachement.Srv = mCopy2NextFrameNode.Previos.Srv;
                PrevOutAttachement.Rtv = mCopy2NextFrameNode.Previos.Rtv;
            ...
            //var previos = mCopy2NextFrameNode.Previos;
            //if (previos == null || previos.GpuResource == null)
            //    return;

            //PrevPinOut.Attachement.Format = mCopy2NextFrameNode.SrcPinIn.Attachement.Format;
            //PrevPinOut.Attachement.Width = mCopy2NextFrameNode.SrcPinIn.Attachement.Width;
            //PrevPinOut.Attachement.Height = mCopy2NextFrameNode.SrcPinIn.Attachement.Height;

            //var attachement = RenderGraph.AttachmentCache.ImportAttachment(PrevPinOut, ResultAttachement);
