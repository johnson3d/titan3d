...
namespace EngineNS.Graphics.Mesh
{
    ...
    public partial class TtRenderMesh : IDisposable
    {
        ...
        private void SetWorldMatrixToCBuffer(in Matrix tm)
        {
            if (mWorldMatrixSetFrame != TtEngine.Instance.CurrentTickFrame)
            {
                var preWorldMatrix = PerMeshCBuffer.GetMatrix(TtCoreShaderBinder.TtPerMeshCBufferVarIndexer.Instance.WorldMatrix);
                PerMeshCBuffer.SetMatrix(TtCoreShaderBinder.TtPerMeshCBufferVarIndexer.Instance.PreWorldMatrix, preWorldMatrix);
                mWorldMatrixSetFrame = TtEngine.Instance.CurrentTickFrame;
            }
            //else
            //{
            //    System.Diagnostics.Debug.Assert(false, "同一帧内多次调用DirectSetWorldMatrix，可能会导致PreWorldMatrix不正确");
            //}
