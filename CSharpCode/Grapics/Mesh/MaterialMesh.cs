...
namespace EngineNS.Graphics.Mesh
{
    ...
    public class TtMaterialMeshAMeta : IO.IAssetMeta
    {
        ...
        public override async Thread.Async.TtTask<List<GamePlay.Scene.TtNode>> GetSnapshotNodes(Graphics.Pipeline.TtOffscreenRenderer renderer)
        {
            var nodes = new List<GamePlay.Scene.TtNode>();
            Graphics.Mesh.TtMaterialMesh materialMesh = await GetAssetName().CreateAsset<TtMaterialMesh>();
            var renderMesh = new Graphics.Mesh.TtRenderMesh();
            var ok = renderMesh.Initialize(materialMesh);
            if (ok)
            {
                var meshNode = await GamePlay.Scene.TtMeshNode.AddMeshNode(renderer.World, renderer.World.Root, new GamePlay.Scene.TtMeshNode.TtMeshNodeData(), typeof(GamePlay.TtPlacement), renderMesh, DVector3.Zero, Vector3.One, Quaternion.Identity);
