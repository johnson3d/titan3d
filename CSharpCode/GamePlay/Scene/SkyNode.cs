...
namespace EngineNS.GamePlay.Scene
{
    ...
    public class TtSkyNode : TtMeshNode
    {
        ...
        protected override async Thread.Async.TtTask<bool> InitializeNode(TtWorld world, TtNodeData data, EBoundVolumeType bvType, Type placementType)
        {
            ...
            SunMesh.UserShading = await Graphics.Pipeline.Shader.TtShadingEnv.CreateShadingEnv<Graphics.Pipeline.Shader.CommanShading.UDrawViewportShading>();
            return true;
        }
        ...
        public override bool OnTickLogic(TtNodeTickParameters args)
        {
            ...
            // 用纯净的 ViewProjection 逆矩阵把天空 mesh 的 NDC 立方体反变换到世界空间.
            // 投影矩阵已不再含 jitter (jitter 由 GBuffer VS 在 SV_Position 阶段注入), 这里
            // 不需要也不应该用 jitter 版本, 否则天空相对场景会偏一帧抖动.
            var matrix = args.Policy.DefaultCamera.GetViewProjectionInverse();
