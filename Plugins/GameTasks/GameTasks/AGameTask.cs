...
namespace EngineNS.Rtti
{
    public class AssemblyEntry
    {
        public class AGameTaskAssemblyDesc : UAssemblyDesc
        {
            ...
            {
                Profiler.Log.WriteLine(Profiler.ELogTag.Info, "Core", "Plugins:GameTasks AssemblyDesc Created");
            }
            ~AGameTaskAssemblyDesc()
            {
                Profiler.Log.WriteLine(Profiler.ELogTag.Info, "Core", "Plugins:GameTasks AssemblyDesc Destroyed");
            }
...
namespace EngineNS.Plugins.GameTasks
{
    ...
    public class UGameTaskPlugin : Bricks.AssemblyLoader.UPlugin
    {
    ...
    public class AGameTaskDescriptorManager
    {
        ...
        {
            var files = IO.FileManager.GetFiles(location.Address, "*.task", false);
            foreach (var i in files)
            {
                var descriptor = IO.FileManager.LoadXmlToObject(i) as AGameTaskDescriptor;
                if (descriptor == null)
                    continue;
