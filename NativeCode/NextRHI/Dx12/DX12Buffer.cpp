...
namespace NxRHI
{
	...
	DX12Buffer::~DX12Buffer()
	{
		...
		if (device == nullptr)
			return;*/
		//auto pdxResource = (ID3D12Resource*)mGpuMemory->GetHWBuffer();
		//wchar_t name[128] = {};
		//UINT size = sizeof(name);
		//pdxResource->GetPrivateData(WKPDID_D3DDebugObjectNameW, &size, name);
		//if (wcscmp(name, L"GrassInst") == 0)
		//{
		//	//ASSERT(false);
	...
	bool DX12Buffer::Init(DX12GpuDevice* device, const FBufferDesc& desc)
	{
		...
		if (desc.Type & EBufferType::BFT_CBuffer)
		{
			...
			mGpuMemory = MakeWeakRef(device->mCBufferMemAllocator->AllocGpuMem(device, desc.Size, "CBuffer"));
		}
		...
		{
			resState = D3D12_RESOURCE_STATES::D3D12_RESOURCE_STATE_RAYTRACING_ACCELERATION_STRUCTURE;
			GpuState = EGpuResourceState::GRS_GenericRead;
		}
	...
	void DX12Buffer::UpdateGpuData(ICommandList* cmd, UINT subRes, void* pData, const FSubResourceFootPrint* pFootPrint)
	{
		if (cmd != nullptr && Desc.Usage == EGpuUsage::USAGE_DEFAULT)
		{
...

			{
				...
				cpDraw->BindBufferSrc(bf);
				cpDraw->Mode = ECopyDrawMode::CDM_Buffer2Buffer;
				cpDraw->FootPrint.Format = pFootPrint->Format;
				cpDraw->FootPrint.X = 0;
				cpDraw->FootPrint.Y = 0;
				cpDraw->FootPrint.Z = 0;
	...
	bool DX12Buffer::Map(UINT index, FMappedSubResource* res, bool forRead)
	{
		...
		{
			D3D12_RANGE range{};
			range.Begin = mGpuMemory->GpuMem->Offset;
			range.End = range.Begin + Desc.Size;
			auto pTarGpuResource = ((DX12GpuHeap*)mGpuMemory->GpuMem->GpuHeap)->mGpuResource;
			void* pData = nullptr;
	...
	bool DX12Texture::Init(DX12GpuDevice* device, const FTextureDesc& desc)
	{
		...
		D3D12_RESOURCE_STATES resState = D3D12_RESOURCE_STATES::D3D12_RESOURCE_STATE_GENERIC_READ;
		GpuState = EGpuResourceState::GRS_GenericRead;
		D3D12_HEAP_PROPERTIES properties{};
		properties.Type = D3D12_HEAP_TYPE_DEFAULT;
		...
		if (desc.BindFlags & EBufferType::BFT_DSV)
		{
			/*switch (desc.Format)
			{
				case EPixelFormat::PXF_D32_FLOAT_S8X24_UINT:
					data.Format = DXGI_FORMAT_R32_FLOAT_X8X24_TYPELESS;
					break;
	...
	void DX12Texture::UpdateGpuData(ICommandList* cmd, UINT subRes, void* pData, const FSubResourceFootPrint* pFootPrint)
	{
		//UINT subRes = mipIndex + Desc.MipLevels * arrayIndex;
		if (Desc.Usage == EGpuUsage::USAGE_DEFAULT)
		{
		...
		else //if (Desc.Usage == EGpuUsage::USAGE_DYNAMIC || Desc.Usage == EGpuUsage::USAGE_STAGING)
		{
			...
			{
				for(UINT i = 0; i < pFootPrint->Height; i++)
				{
					//copy row by row
				}
	...
	bool DX12CbView::Init(DX12GpuDevice* device, IBuffer* pBuffer, const FCbvDesc& desc)
	{
		...
		mView = MakeWeakRef(device->mCbvSrvUavAllocator->AllocDX12Heap());
	...
	void SrvDesc2DX(D3D12_SHADER_RESOURCE_VIEW_DESC* tar, const FSrvDesc* src)
	{
		...
		switch (tar->Format)
		{
			case DXGI_FORMAT_X24_TYPELESS_G8_UINT:
				...
				if (src->Type == ST_Texture2D)
					tar->Texture2D.PlaneSlice = 1;
				break;
	...
	bool DX12RenderTargetView::Init(DX12GpuDevice* device, ITexture* pBuffer, const FRtvDesc* desc)
	{
		...
		if (desc != nullptr)
			Desc = *desc;
		GpuResource = pBuffer;
		mView = MakeWeakRef(device->mRtvAllocator->AllocDX12Heap());
	...
	bool DX12DepthStencilView::Init(DX12GpuDevice* device, ITexture* pBuffer, const FDsvDesc& desc)
	{
		...
		GpuResource = pBuffer;
		mView = MakeWeakRef(device->mDsvAllocator->AllocDX12Heap());
	...
	bool DX12TopAccelerationStructure::IsBuild(DX12GpuDevice* device)
	{
		...
		else
		{
			...
			if (result)
			{
				UINT size = (UINT)(sizeof(D3D12_RAYTRACING_INSTANCE_DESC) * mBottomASInstances.size());
	...
	{	
		...
		mBuildDesc.Inputs.InstanceDescs = ((DX12Buffer*)mInstanceGpuBuffer->GetBuffer())->GetGPUVirtualAddress();
