...
public:
	...
	void		BuildLocalityLinks( FDisjointSet& DisjointSet, const v3dxBox3& Bounds, const std::vector<INT32>& GroupIndexes, FGetCenter& GetCenter );
...
template< typename FGetCenter >
void FGraphPartitioner::BuildLocalityLinks( FDisjointSet& DisjointSet, const v3dxBox3& Bounds, const std::vector<INT32>& GroupIndexes, FGetCenter& GetCenter )
{
...
// 		[&]( UINT32 Index )
	...
	{
		v3dxVector3 Center = GetCenter( Index );
		...
		if( RunLength < 128 )
		{
			...
			for( int Direction = 0; Direction < 2; Direction++ )
			{
				...
				for( INT32 Iterations = 0; Iterations < 16; Iterations++ )
				{
					...
					{
						// Add to sorted list
						float AdjDist2 = ( Center - GetCenter( AdjIndex ) ).getLengthSq();
