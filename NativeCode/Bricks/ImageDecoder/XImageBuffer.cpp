...
XImageBuffer* XImageBuffer::ConvertToFloatImage()
{
	...
	image->m_nBitCount = sizeof(float) * 4;
...
bool XImageBuffer::Create(int nWidth, int nHeight, XImagePixelFormat pixelFormat)
{
	Cleanup();
	...
	case XIPF_R8G8B8A8:
		...
		m_nStride = (m_nWidth * m_nBitCount + 7) / 8;
		break;
	case XIPF_R16G16B16A16F:
		m_nBitCount = 64;
		m_nStride = (m_nWidth * m_nBitCount + 15) / 16;
		break;
	case XIPF_R32G32B32A32F:
		m_nBitCount = 128;
		m_nStride = (m_nWidth * m_nBitCount + 31) / 32;
		break;
	}
	m_pImage = _AllocSharedImageData(m_nStride * m_nHeight);
	if (m_pImage == NULL)
