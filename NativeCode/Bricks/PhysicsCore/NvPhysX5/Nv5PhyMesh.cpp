...
Nv5PhyTriMesh::~Nv5PhyTriMesh()
{
	Cleanup();
}

void Nv5PhyTriMesh::Cleanup()
{
	if (mMesh != nullptr)
	{
		mMesh->release();
...
bool Nv5PhyTriMesh::CreateFromCookedData(PhyContext* ctx, void* cookedData, UINT size)
{
	Cleanup();
	...
	mMesh = ((Nv5PhyContext*)ctx)->mContext->createTriangleMesh(readBuffer);
	if (mMesh == nullptr)
		return false;
...
FMeshDataProvider* Nv5PhyTriMesh::CreateMeshProvider()
{
...
Nv5PhyConvexMesh::~Nv5PhyConvexMesh()
{
	if (mMesh != nullptr)
	{
		mMesh->release();
