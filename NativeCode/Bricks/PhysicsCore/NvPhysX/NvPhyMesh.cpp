...
NvPhyTriMesh::~NvPhyTriMesh()
{
	Cleanup();
}

void NvPhyTriMesh::Cleanup()
{
	if (mMesh != nullptr)
	{
		mMesh->release();
...
bool NvPhyTriMesh::CreateFromCookedData(PhyContext* ctx, void* cookedData, UINT size)
{
	Cleanup();
	...
	mMesh = ((NvPhyContext*)ctx)->mContext->createTriangleMesh(readBuffer);
	if (mMesh == nullptr)
		return false;
...
FMeshDataProvider* NvPhyTriMesh::CreateMeshProvider()
{
...
NvPhyConvexMesh::~NvPhyConvexMesh()
{
	if (mMesh != nullptr)
	{
		mMesh->release();
