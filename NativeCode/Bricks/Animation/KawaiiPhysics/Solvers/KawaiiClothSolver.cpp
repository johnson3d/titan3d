...
namespace KawaiiPhysics
{
	...
	void KawaiiClothSolver::SimulateCloth(FKawaiiClothData& Cloth, const FKawaiiPhysicsContext& Context, FTopLevelBVH* ColliderBVH)
	{
		...
		// same semantics as the chain solver.
		for (auto& chain : Cloth.ChainTable)
			XPBDSolver::ApplyWorldMoveLag(chain, Context);
		...
		// Pull back toward the animated pose (Stiffness), per vertical chain (root -> tip)
		for (auto& chain : Cloth.ChainTable)
			XPBDSolver::ApplyPoseStiffness(chain, dt, Context.SimulationFPS);
