...
namespace KawaiiPhysics
{
		...
		const std::vector<int32_t>& ParentIndices)
	{
		...
		for (size_t i = 0; i < Setups.size(); ++i)
		{
			...
			SimJointHelpers::UpdatePhysicsSettings(
				chain.Particles,
				setup,
				(uint32_t)(i + 1));
	...
	void KawaiiChainSolver::SimulateChain(FKawaiiChainData& Chain, const FKawaiiPhysicsContext& Context, FTopLevelBVH* ColliderBVH)
	{
		...
		// Teleport guard: a huge actor jump must not be turned into a whip.
		const v3dxVector3 componentDeltaWorld = Context.ComponentLocation - Context.PrevComponentLocation;
		if (Context.TeleportDistanceThreshold > 0.0f &&
			Vec3Length(componentDeltaWorld) > Context.TeleportDistanceThreshold)
		{
			for (auto& P : Chain.Particles)
				P.SnapToPose();
			return;
		...
		// Actor movement / rotation -> per-particle lag (WorldDampingLocation / Rotation)
		XPBDSolver::ApplyWorldMoveLag(Chain.Particles, Context);
		...
		// Pull back toward the animated pose (Stiffness). Must run after gravity and before the
		// constraint sweep, so the constraints resolve the blended target instead of fighting it.
		XPBDSolver::ApplyPoseStiffness(Chain.Particles, dt, Context.SimulationFPS);
