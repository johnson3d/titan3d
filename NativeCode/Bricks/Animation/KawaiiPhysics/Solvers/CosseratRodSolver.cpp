...
namespace CosseratRodSolver
{
		...
		float dt)
	{
		...
		float compliance = (1.0f - Rod.StretchAndShearStiffness) / (dt * dt + KAWAII_SMALL_NUMBER);
		...
		float dt)
	{
		...
		float compliance = (1.0f - Rod.BendAndTwistStiffness) / (dt * dt + KAWAII_SMALL_NUMBER);
	...
	void SolvePointAttachmentConstraint(FCosseratRodData& Rod, float dt)
	{
		...
		float compliance = (1.0f - Rod.PointAttachmentStiffness) / (dt * dt + KAWAII_SMALL_NUMBER);

		for (size_t i = 0; i < Rod.Particles.size(); ++i)
		{
	...
	void SolveOrientationAttachmentConstraint(FCosseratRodData& Rod, float dt)
	{
		...
		float compliance = (1.0f - Rod.OrientationAttachmentStiffness) / (dt * dt + KAWAII_SMALL_NUMBER);
