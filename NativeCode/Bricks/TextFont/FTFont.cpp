...
ENGINE_RTTI_IMPL(EngineNS::FTFontManager)

Canvas::IImage* FTFont::GetWord(WCHAR c, Canvas::FCanvasVertex vert[4], v3dxVector2* size) const
{
	auto pThis = (FTFont*)this;
	auto pWorld = pThis->GetWord(c);
	size->x = (float)pWorld->FontSize;
	size->y = (float)(pWorld->PixelY + pWorld->PixelHeight);

	//todo: fill vert[4] ...
	
	return pWorld->Texture;
}
...
void FTFont::Init(NxRHI::IGpuDevice* rc, FTFontManager* ftMgr, const char* name, int fontSize, int cacheTexSize, int texSize)
{
