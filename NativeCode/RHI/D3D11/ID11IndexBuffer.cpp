...
ID11IndexBuffer::ID11IndexBuffer()
{
	mBuffer = nullptr;
}
...
void ID11IndexBuffer::GetBufferData(IRenderContext* rc, IBlobObject* data)
{
...
void ID11IndexBuffer::UpdateGPUBuffData(ICommandList* cmd, void* ptr, UINT size)
{
	...
	if (size < mDesc.ByteWidth && mDesc.CPUAccess & CAS_WRITE)
	{
		...
		if (S_OK == ((ID11CommandList*)cmd)->mDeferredContext->Map(mBuffer, 0, D3D11_MAP_WRITE_DISCARD, 0, &mapData))
		{
			memcpy(mapData.pData, ptr, size);
			((ID11CommandList*)cmd)->mDeferredContext->Unmap(mBuffer, 0);
		}
	}
	else
	{
...
bool ID11IndexBuffer::Init(ID11RenderContext* rc, const IIndexBufferDesc* desc)
{
	D3D11_BUFFER_DESC bd;
	memset(&bd, 0, sizeof(bd));
	...
	auto debuginfo = VStringA_FormatV("IB_%u", UniqueId++);
	mBuffer->SetPrivateData(WKPDID_D3DDebugObjectName, (UINT)debuginfo.length(), debuginfo.c_str());
#endif
	return true;
}

bool ID11IndexBuffer::Init(ID11RenderContext* rc, const IIndexBufferDesc* desc, const IGpuBuffer* pBuffer)
{
	mDesc = *desc;
...
//? still not used...
void ID11IndexBuffer::SetDebugInfo(const char* info)
{
	mDebugInfo = info;
	mDebugInfo += std::to_string(GIBNameId++);
	if (mBuffer != nullptr)
	{
		mBuffer->SetPrivateData(WKPDID_D3DDebugObjectName, 0, NULL);
		mBuffer->SetPrivateData(WKPDID_D3DDebugObjectName, (UINT)mDebugInfo.length(), mDebugInfo.c_str());
	}
}
