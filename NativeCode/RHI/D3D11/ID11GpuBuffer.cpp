...
vBOOL ID11GpuBuffer::UpdateBufferData(ICommandList* cmd, UINT offset, void* data, UINT size)
{
	...
	{
		/*auto d11Ctx = (ID11RenderContext*)cmd->GetContext();
		auto tmpDesc = mBufferDesc;
		tmpDesc.CPUAccessFlags = CAS_WRITE;
		tmpDesc.Usage = USAGE_DYNAMIC;
		tmpDesc.BindFlags = 0;
		auto memBuffer = (ID11GpuBuffer*)d11Ctx->CreateGpuBuffer(&tmpDesc, nullptr);
		D3D11_MAPPED_SUBRESOURCE mapRes;
		auto hr = pContext->Map(memBuffer->mBuffer, 0, D3D11_MAP_WRITE_NO_OVERWRITE, 0, &mapRes);
		if (hr != S_OK)
		{
			return FALSE;
		}

		memcpy((BYTE*)mapRes.pData + offset, data, size);

		pContext->Unmap(memBuffer->mBuffer, 0);

		pContext->CopyResource(mBuffer, memBuffer->mBuffer);
		memBuffer->Release();*/
		/*if (offset > 0)
		{
			BYTE* pTmp = new BYTE[offset + size];
			memset(pTmp, offset, 0);
			memcpy(pTmp + offset, data, size);
			pContext->UpdateSubresource(mBuffer, 0, nullptr, pTmp, 0, 0);
			delete[] pTmp;
		}
		else*/
		{
