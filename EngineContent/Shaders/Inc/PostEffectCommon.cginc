...
float GaussWeight2D(float x, float y, float sigma)
{
	...
	float a = -(x * x + y * y) / (2.0f * sigma_2);
	return pow(E, a) / (2.0f * PI * sigma_2);
}
...
float3 GaussNxN(Texture2D Tex, SamplerState Sampler, float2 uv, int n, float2 stride, float sigma)
{
	...
	for (int i = -r; i <= r; i++)
	{
		for (int j = -r; j <= r; j++)
		{
			...
			color += Tex.SampleLevel(Sampler, coord, 0).rgb * w;
			weight += w;
		}
	}
...
float3 GaussNxN_KarisAverage(Texture2D Tex, SamplerState Sampler, float2 uv, int n, float2 stride, float sigma)
{
	...
	for (int i = -r; i <= r; i++)
	{
		for (int j = -r; j <= r; j++)
		{
			float w = GaussWeight2D(i, j, sigma);
			float2 coord = uv + float2(i, j) * stride;
			float3 c = Tex.SampleLevel(Sampler, coord, 0).rgb;
			float lum = CalcLuminance(c);
			float w2 = 1.0f / (1.0f + lum);
			w = w * w2;
			color += c * w;
			weight += w;
		}
