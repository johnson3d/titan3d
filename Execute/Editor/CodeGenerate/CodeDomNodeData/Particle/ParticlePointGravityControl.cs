...
namespace CodeDomNode.Particle
{
    ...
    public class ParticlePointGravityControlConstructionParams : StructNodeControlConstructionParams
    {

        [EngineNS.Rtti.MetaData]
        public Type CreateType { get; set; } = typeof(EngineNS.Bricks.Particle.PointGravityNode.Data);//测试Box

        [EngineNS.Rtti.MetaData]
        public override string BaseClassName { get; set; } = "EngineNS.Bricks.Particle.PointGravityNode";
        public override EditorCommon.CodeGenerateSystem.INodeConstructionParams Duplicate()
        {
            var retVal = base.Duplicate() as ParticlePointGravityControlConstructionParams;
            retVal.CreateType = CreateType;
            return retVal;
        }

    }
    [CodeGenerateSystem.CustomConstructionParamsAttribute(typeof(ParticlePointGravityControlConstructionParams))]
    public partial class ParticlePointGravityControl : CodeGenerateSystem.Base.BaseNodeControl, IParticleNode, IParticleGradient
    {
        ...
        public ParticlePointGravityControl(ParticlePointGravityControlConstructionParams csParam)
            : base(csParam)
        {
            InitConstruction();
            ...
            {
                NodeName = "ParticlePointGravity";
            }

            IsOnlyReturnValue = true;
            AddLinkPinInfo("ParticlePointGravityControlDown", mCtrlValueLinkHandleDown, null);
            AddLinkPinInfo("ParticlePointGravityControlUp", mCtrlValueLinkHandleUp, null);

            mCtrlValueLinkHandleDown.ResetDefaultFilterBySystem("ParticlePointGravityControlUp");
        ...
        public void SyncValues(EngineNS.Bricks.Particle.CGfxParticleSystem sys)
        {
            ...
            if (sys.SubParticleSystems != null && sys.SubParticleSystems.Count != 0)
            {
                for (int i = 0; i < sys.SubParticleSystems.Count; i++)
                {
                    ...
                    var node = subsys.PointGravityNode;
                    if (node != null)
                    {
                        if (node.GetType().Name.Equals(GetClassName()))
                        {
                            node._Data = ParticleNode as EngineNS.Bricks.Particle.PointGravityNode.Data;
                            return;
                        }
                    }
                }
        ...
        {
            return "PointGravity_" + Id.ToString().Replace("-", "_");
        }
        ...
        protected override void EndLink(LinkPinControl linkObj)
        {
            ...
            var pinInfo = GetLinkPinInfo("ParticlePointGravityControlDown");
        ...
        {
            ParticleDataSaveLoad.SaveData("ParticlePointGravityNode", xndNode, newGuid, CSParam as ParticlePointGravityControlConstructionParams, this, mLinkedNodesContainer);
        ...
        {
            //await AwaitLoad();
            NeedResetLoadValue = await ParticleDataSaveLoad.LoadData("ParticlePointGravityNode", xndNode, CSParam as ParticlePointGravityControlConstructionParams, this, mLinkedNodesContainer);
        ...
        {
            CollectLinkPinInfo(smParam, "ParticlePointGravityControlDown", CodeGenerateSystem.Base.enLinkType.Module, CodeGenerateSystem.Base.enBezierType.Left, CodeGenerateSystem.Base.enLinkOpType.Start, false);
            CollectLinkPinInfo(smParam, "ParticlePointGravityControlUp", CodeGenerateSystem.Base.enLinkType.Module, CodeGenerateSystem.Base.enBezierType.Right, CodeGenerateSystem.Base.enLinkOpType.End, true);
        }
        ...
        {
            return typeof(ParticlePointGravityControl);
        }
