...
namespace CodeDomNode.Particle
{
    ...
    public class ParticleTrailControlConstructionParams : StructNodeControlConstructionParams
    {

        [EngineNS.Rtti.MetaData]
        public Type CreateType { get; set; } = typeof(EngineNS.Bricks.Particle.TrailDataControl);//测试Box

        [EngineNS.Rtti.MetaData]
        public override string BaseClassName { get; set; } = "EngineNS.Bricks.Particle.TrailNode";
        public override EditorCommon.CodeGenerateSystem.INodeConstructionParams Duplicate()
        {
            var retVal = base.Duplicate() as ParticleTrailControlConstructionParams;
            retVal.CreateType = CreateType;
            return retVal;
        }

    }
    [CodeGenerateSystem.CustomConstructionParamsAttribute(typeof(ParticleTrailControlConstructionParams))]
    public partial class ParticleTrailControl : CodeGenerateSystem.Base.BaseNodeControl, IParticleNode, IParticleSaveData, IParticleGradient
    {
        ...
        public ParticleTrailControl(ParticleTrailControlConstructionParams csParam)
            : base(csParam)
        {
            InitConstruction();
            ...
            {
                NodeName = "ParticleTrail";
            }

            IsOnlyReturnValue = true;
            AddLinkPinInfo("ParticleTrailControlDown", mCtrlValueLinkHandleDown, null);
            AddLinkPinInfo("ParticleTrailControlUp", mCtrlValueLinkHandleUp, null);

            mCtrlValueLinkHandleDown.ResetDefaultFilterBySystem("ParticleTrailControlUp");
        ...
        public void SyncValues(EngineNS.Bricks.Particle.CGfxParticleSystem sys)
        {
            ...
            if (sys.SubParticleSystems != null && sys.SubParticleSystems.Count != 0)
            {
                for (int i = 0; i < sys.SubParticleSystems.Count; i++)
                {
                    ...
                    var node = subsys.TrailNode;
                    if (node != null)
                    {
                        if (node.GetType().Name.Equals(GetClassName()))
                        {
                            node.TrailDataControlData = ParticleNode as EngineNS.Bricks.Particle.TrailDataControl;
                            return;
                        }
                    }
                }
            ...
            if (sys.SubParticleSystems != null && sys.SubParticleSystems.Count != 0)
            {
                for (int i = 0; i < sys.SubParticleSystems.Count; i++)
                {
                    ...
                    if (node != null)
                    {
                        if (node.GetType().Name.Equals(GetClassName()))
                        {
                            ParticleNode = node.TrailDataControlData;
                            break;
                        }
                    }
                }
        ...
        {
            return "ParticleTrail" + Id.ToString().Replace("-", "_");
        }
        ...
        string[] ParticleTrailTemplate =
        {
           "CallSetDealTrailDataColorAndWidth",
        };
        ...
        protected override void EndLink(LinkPinControl linkObj)
        {
            ...
            var pinInfo = GetLinkPinInfo("ParticleTrailControlDown");
        ...
        {
            var param = CSParam as ParticleTrailControlConstructionParams;
            var assist = this.HostNodesContainer.HostControl;
            var TitleString = HostNodesContainer.TitleString;
        ...
        async System.Threading.Tasks.Task InitializeLinkedNodesContainer()
        {
            ...
            {
                for (int i = 0; i < ParticleTrailTemplate.Length; i++)
                {
                    CreateParticleMethodCategory(ParticleTrailTemplate[i], 50, 100 * i + 50);
                }
            }
            mLinkedNodesContainer.HostNode = this;
        }
        ...
        {
            ParticleDataSaveLoad.SaveData("ParticleTrailNode", xndNode, newGuid, CSParam as ParticleTrailControlConstructionParams, this, mLinkedNodesContainer);
        ...
        {
            //await AwaitLoad();
            NeedResetLoadValue = await ParticleDataSaveLoad.LoadData2("ParticleTrailNode", xndNode, CSParam as ParticleTrailControlConstructionParams, this, mLinkedNodesContainer);
        ...
        {
            CollectLinkPinInfo(smParam, "ParticleTrailControlDown", CodeGenerateSystem.Base.enLinkType.Module, CodeGenerateSystem.Base.enBezierType.Left, CodeGenerateSystem.Base.enLinkOpType.Start, false);
            CollectLinkPinInfo(smParam, "ParticleTrailControlUp", CodeGenerateSystem.Base.enLinkType.Module, CodeGenerateSystem.Base.enBezierType.Right, CodeGenerateSystem.Base.enLinkOpType.End, true);
        }
        ...
        {
            return typeof(ParticleTrailControl);
        }
