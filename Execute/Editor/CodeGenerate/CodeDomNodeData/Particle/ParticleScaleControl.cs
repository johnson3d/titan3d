...
{
    public class ParticleScaleControlConstructionParams : StructNodeControlConstructionParams
    {
        [EngineNS.Rtti.MetaData]
        public override string BaseClassName { get; set; } = "EngineNS.Bricks.Particle.ParticleScaleNode";

        public override EditorCommon.CodeGenerateSystem.INodeConstructionParams Duplicate()
        {
            var retVal = base.Duplicate() as ParticleScaleControlConstructionParams;
            return retVal;
        }

    }
    [CodeGenerateSystem.CustomConstructionParamsAttribute(typeof(ParticleScaleControlConstructionParams))]
    public partial class ParticleScaleControl : CodeGenerateSystem.Base.BaseNodeControl, IParticleNode, IParticleSaveData, IParticleGradient
    {
            ...
            if (sys.SubParticleSystems != null && sys.SubParticleSystems.Count != 0)
            {
                for (int i = 0; i < sys.SubParticleSystems.Count; i++)
                {
                    ...
                    if (subsys.SubStates != null)
                    {
                        ...
                        {
                            var scalenode = subsys.SubStates[j].ParticleScaleNode;
                            if (scalenode != null)
                            {
                                if (scalenode.GetType().Name.Equals(GetClassName()))
                                {
                                    DataGradient.PDataGradient = scalenode;
                                    break;
                                }
                            }
                        }
                    ...
                    if (subsys.SubStates != null)
                    {
                        for (int j = 0; j < subsys.SubStates.Length; j++)
                        {
                            ...
                            if (scalenode != null)
                            {
                                if (scalenode.GetType().Name.Equals(GetClassName()))
                                {
                                    subsys.SubStates[j].ParticleScaleNode = DataGradient.PDataGradient as EngineNS.Bricks.Particle.ParticleScaleNode;
                                    return;
                                }
                            }
                        }
        ...
        {
            return "ScaleNode_" + Id.ToString().Replace("-", "_");
        }
        ...
        {
            var param = CSParam as ParticleScaleControlConstructionParams;
        ...
        private void CreateParticleMethodCategory(string methodname, float x, float y)
        {
            ...
            Type type = typeof(EngineNS.Bricks.Particle.ParticleScaleNode);
        ...
        protected override void EndLink(LinkPinControl linkObj)
        {
            ...
            var pinInfo = GetLinkPinInfo("ParticleScaleControlDown");
        ...
        {
            ParticleDataSaveLoad.SaveData("ParticleScaleNode", xndNode, newGuid, CSParam as ParticleScaleControlConstructionParams, this, mLinkedNodesContainer);

            var csParam = CSParam as ParticleScaleControlConstructionParams;
        ...
        public override async System.Threading.Tasks.Task Load(XndNode xndNode)
        {
            ...
            //}
            //await AwaitLoad();
            await ParticleDataSaveLoad.LoadData2("ParticleScaleNode", xndNode, CSParam as ParticleScaleControlConstructionParams, this, mLinkedNodesContainer);
        ...
        {
            CollectLinkPinInfo(smParam, "ParticleScaleControlDown", CodeGenerateSystem.Base.enLinkType.Module, CodeGenerateSystem.Base.enBezierType.Left, CodeGenerateSystem.Base.enLinkOpType.Start, false);
            CollectLinkPinInfo(smParam, "ParticleScaleControlUp", CodeGenerateSystem.Base.enLinkType.Module, CodeGenerateSystem.Base.enBezierType.Right, CodeGenerateSystem.Base.enLinkOpType.End, true);
        }
        ...
        {
            return typeof(ParticleScaleControl);
        }
        ...
        public override async System.Threading.Tasks.Task GCode_CodeDom_GenerateCode(CodeTypeDeclaration codeClass, CodeStatementCollection codeStatementCollection, LinkPinControl element, GenerateCodeContext_Method context)
        {
            ...
            DataGradient.GCode_CodeDom_GenerateCode(codeClass, GetClassName(), "StartValue", true);
