    ...
    internal class CSharpCodeGenerator : ICodeCompiler, ICodeGenerator
    {
        ...
        private string CmdArgsFromParameters(CompilerParameters options)
        {
            ...
            if (String.IsNullOrWhiteSpace(options.CoreAssemblyFileName))
            {
                ...
                if (Program.TryGetProbableCoreAssemblyFilePath(options, out probableCoreAssemblyFilePath))
                {
        ...
        private void GenerateArrayCreateExpression(CodeArrayCreateExpression e)
        {
            ...
            else
            {
                ...
                int nestedArrayDepth = Program.NestedArrayDepth(e.CreateType);
        ...
        {
            throw new InvalidOperationException("System.CodeDom.CodeCastExpression已不再使用, 请使用CodeGenerateSystem.CodeDom.CodeCastExpression");
            //Output.Write("((");
        ...
        private void GenerateCastExpression(CodeGenerateSystem.CodeDom.CodeCastExpression e)
        {
            if(e.IsValueType)
            {
                Output.Write("((");
                Output.Write(e.TargetTypeStr);
                Output.Write(")(");
                GenerateExpression(e.Expression);
                Output.Write("))");
            }
        ...
        {
            throw new InvalidOperationException("不再使用System.CodeDom.CodeMethodInvokeExpression，请使用CodeGenerateSystem.CodeDom.CodeMethodInvokeExpression");
            //GenerateMethodReferenceExpression(e.Method);
            //Output.Write("(");
            //OutputExpressionList(e.Parameters);
            //Output.Write(")");
        }
        private void GenerateMethodInvokeExpression(CodeGenerateSystem.CodeDom.CodeMethodInvokeExpression e)
        {
            if (e.UseAwait)
                Output.Write("await ");
            GenerateMethodReferenceExpression(e.Method);
        ...
        private void GenerateLambdaExpression(CodeGenerateSystem.CodeDom.CodeLambdaExpression e)
        {
            if (e.MethodInvoke.Parameters.Count == e.LambdaParams.Count)
            {
                if (string.IsNullOrEmpty(e.LambdaFieldName))
                    throw new InvalidOperationException("CodeLambdaExpression没有设置LambdaFieldName");
                // 没有在lambda中添加非delegate定义的参数
                Output.Write($"({e.LambdaFieldName})");
            }
            else
            {
                Output.Write("(");
                string paramStr = "";
                for(int i=0; i<e.LambdaParams.Count; i++)
                {
                    var param = e.LambdaParams[i];
                    switch(param.Dir)
                    {
                        ...
                        case FieldDirection.Ref:
                            paramStr += "ref " + param.Name + ",";
                            break;
                        case FieldDirection.Out:
                            paramStr += "out " + param.Name + ",";
                            break;
                    }
                }
                paramStr = paramStr.TrimEnd(',');
                Output.Write($"( {paramStr} )=> ");
                Output.Write("{ ");
                if (e.NeedReturn)
                    Output.Write("return ");
                GenerateMethodInvokeExpression(e.MethodInvoke);
                Output.Write("; }");
                Output.Write(")");
            }
        }
        ...
        private void GenerateStatement(CodeStatement e)
        {
            ...
            else if(e is CodeGenerateSystem.CodeDom.CodeSwitchCaseStatement)
            {
                GenerateSwitchStatement((CodeGenerateSystem.CodeDom.CodeSwitchCaseStatement)e);
            }
        ...
        private void GeneratePrimitiveExpressionBase(CodePrimitiveExpression e)
        {
            ...
            else if(e.Value.GetType().IsEnum)
            {
                var enumType = e.Value.GetType();
                var name = System.Enum.GetName(enumType, e.Value);
                if(name == null)
                {
                    string tempStr = "";
                    var vals = System.Enum.GetValues(enumType);
                    var eIntVal = System.Convert.ToInt64(e.Value);
                    foreach(var val in vals)
                    {
                        var intVal = System.Convert.ToInt64(val);
                        if (intVal == 0)
                            continue;
                        if((eIntVal & intVal) == intVal)
                        {
                            tempStr += EngineNS.Rtti.RttiHelper.GetAppTypeString(enumType) + "." + System.Enum.GetName(enumType, val) + "|";
                        }
                    }
                    ...
                    Output.Write(EngineNS.Rtti.RttiHelper.GetAppTypeString(enumType) + "." + System.Enum.GetName(enumType, e.Value));
            }
            else if (e.Value.GetType().Equals(typeof(EngineNS.Vector3)))
            {
                var v = (EngineNS.Vector3)e.Value;
                Output.Write("new " + e.Value.GetType().FullName + "("+ v.X +","+v.Y +","+v.Z+")");
            }
            else
            {
        ...
        private void GenerateSwitchStatement(CodeGenerateSystem.CodeDom.CodeSwitchCaseStatement e)
        {
            if((e.SwitchItemType.IsPrimitive) || (e.SwitchItemType == typeof(string)) || (e.SwitchItemType.IsEnum))
            {
                Output.Write("switch(");
                GenerateExpression(e.Expression);
                Output.Write(")");
                OutputStartingBrace();
                Indent++;
                foreach (var caseStatements in e.CaseStatements)
                {
                    Output.Write("case ");
                    GenerateExpression(caseStatements.Key);
                    output.Write(":");
                    Indent++;
                    OutputStartingBrace();
                    Indent++;
                    GenerateStatements(caseStatements.Value);
                    Indent--;
                    Output.WriteLine("}");
                ...
                var defaultStatements = e.DefaultStatements;
        ...
        private void GenerateExpression(CodeExpression e)
        {
            ...
            else if(e is CodeGenerateSystem.CodeDom.CodeCastExpression)
            {
                GenerateCastExpression((CodeGenerateSystem.CodeDom.CodeCastExpression)e);
            }
            ...
            else if(e is CodeGenerateSystem.CodeDom.CodeMethodInvokeExpression)
            {
                GenerateMethodInvokeExpression((CodeGenerateSystem.CodeDom.CodeMethodInvokeExpression)e);
            }
            ...
            else if(e is CodeGenerateSystem.CodeDom.CodeLambdaExpression)
            {
                GenerateLambdaExpression((CodeGenerateSystem.CodeDom.CodeLambdaExpression)e);
            }
            else
            {
                if (e == null)
        ...
        private void GenerateMethod(CodeMemberMethod e, CodeTypeDeclaration c)
        {
            ...
            var customMethod = e as CodeGenerateSystem.CodeDom.CodeMemberMethod;
            if(customMethod != null)
            {
                if(customMethod.IsAsync)
                {
                    output.Write("async ");
                }
                else if(customMethod.IsUnsafe)
                {
                    output.Write("unsafe ");
                }
            }
        ...
        private void GenerateConstructor(CodeConstructor e, CodeTypeDeclaration c)
        {
            ...
            OutputStartingBrace();
            Indent++;
            var cc = c as CodeGenerateSystem.CodeDom.CodeTypeDeclaration;
            if (cc != null)
                GenerateStatements(cc.ConstructStatements);
            GenerateStatements(e.Statements);
            Indent--;
            Output.WriteLine("}");
        }
...
#pragma warning disable 618 // Load with evidence is obsolete - this warning is passed on via the options.Evidence property
                ...
                {
                    results.CompiledAssembly = Program.LoadImageSkipIntegrityCheck(assemblyBuff, symbolsBuff, options.Evidence);
                }
#pragma warning restore 618
            }
            finally
        ...
        CompilerResults ICodeCompiler.CompileAssemblyFromDom(CompilerParameters options, CodeCompileUnit e)
        {
            ...
            {
#if !FEATURE_PAL
                var impersonation = WindowsIdentity.Impersonate(new IntPtr(0));
#endif
                try
                {
                              ...
                              ref string outputFile, ref int nativeReturnValue, string trueArgs)
        {
            ...
            {
                var cmd = "\"" + fullname + "\" " + arguments;
                if (trueArgs != null)
                    cmd = "\"" + fullname + "\" " + trueArgs;
                nativeReturnValue = Executor.ExecWaitWithCapture(cmd, Environment.CurrentDirectory, options.TempFiles, ref outputFile, ref errorFile);
            }
            else
            {
        ...
        private CompilerResults FromDomBatch(CompilerParameters options, CodeCompileUnit[] ea)
        {
            ...
            try
            {
                WindowsImpersonationContext impersonation = WindowsIdentity.Impersonate(new IntPtr(0));
                try
                {
#endif // !FEATURE_PAL
