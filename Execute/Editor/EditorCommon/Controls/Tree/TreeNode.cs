...
{
    public sealed class PropertyInTreeNodeAttribute : Attribute { }

	public sealed class TreeNode : DynamicObject, INotifyPropertyChanged
	{
		...
		private ITreeModel _tag;
		public ITreeModel Tag
		{
			get { return _tag; }
		}
			...
			switch (e.Action)
			{
				case NotifyCollectionChangedAction.Add:
					if (e.NewItems != null)
					{
						...
						foreach (ITreeModel obj in e.NewItems)
						{
        ...
        {
            if(!mProperties.ContainsKey(binder.Name))
            {
                mProperties.Add(binder.Name, value.ToString());
            }
            return true;
        }
        public override bool TryGetMember(GetMemberBinder binder, out object result)
        {
            return mProperties.TryGetValue(binder.Name, out result);
        }
    }
}
